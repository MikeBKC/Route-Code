...
void cwmp_agent_start_session(cwmp_t * cwmp)
{
    ...
    http_dest_t *  dest;
    http_dest_create(&dest, cwmp->acs_url, cwmp->pool);
	...
	session->dest = dest;
        ...
        while (!session_close)
        {
            ...
            case CWMP_ST_START:
                ...
                if (cwmp_session_connect(session) != CWMP_OK)
                {
		...
		int newtaskres = cwmp_agent_run_tasks(cwmp,&reboot);
...
//        return CWMP_ERROR;
    ...
    if (TRstrcmp(method, CWMP_RPC_GETPARAMETERVALUES) == 0)
    {
    ...
    else if (TRstrcmp(method, CWMP_RPC_GETRPCMETHODS) == 0)
    {
...
int cwmp_agent_clear_download_file(void)
{
    doSystem("rm -fr /tmp/download.img");
    return 0;
}
...
extern int formUpdateFirmware(char* filename);
int cwmp_agent_run_tasks(cwmp_t * cwmp,int *reboot)
{
	void * data;
	int tasktype = 0;;
	int ok = CWMP_NO;
			...
			case TASK_DOWNLOAD_TAG:
				{
                    ...
                    if(faultcode == CWMP_OK)
                    {
                        ...
                        {
                            /* 软件升级 */
                            faultcode = formUpdateFirmware("/tmp/download.img");
                            if(faultcode == CWMP_OK)
                            {
                                *reboot = 1;
                            }
                            ...
                            /*配置导入*/
                            faultcode = formUploadSettings("/tmp/download.img");
                        }else{
                            faultcode = 9001;
                        }
                        cwmp_agent_clear_download_file();
                    }else{
					    cwmp_log_error("download file error.\n");
                    }

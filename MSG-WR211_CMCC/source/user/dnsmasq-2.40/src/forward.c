...
#ifdef P1010
extern int difftime_p1010(time_t time1, time_t time0);
struct in6_pktinfo
  {
...
struct frec *get_new_frec(time_t now, int *wait)
{
...
#ifdef P1010
    else if (!oldest || difftime_p1010(f->time, oldest->time) <= 0)
#else
...
#ifdef P1010
  if (oldest && ((int)difftime_p1010(now, oldest->time)) >= TIMEOUT)
#else
  if (oldest && ((int)difftime(now, oldest->time)) >= TIMEOUT)
    ...
    { 
	 ...
	 record instead */
#ifdef P1010
      if (difftime_p1010(now, oldest->time) < 2*TIMEOUT && 

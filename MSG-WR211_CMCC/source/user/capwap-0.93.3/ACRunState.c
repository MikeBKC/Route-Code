	...
	switch(controlVal.messageTypeValue) {
#if defined(URCP_IPC) || defined (URCP_MODULE)
		case CW_MSG_TYPE_VALUE_URCP_REQUEST:
		{
			CWParseUrcpPDURequest(WTPIndex, (msgPtr->msg)+(msgPtr->offset), controlVal.msgElemsLen);
			if (timerSet) {
		...
		default: 
			...
			if( (controlVal.messageTypeValue==CW_MSG_TYPE_VALUE_DISCOVERY_REQUEST) ||
			    (controlVal.messageTypeValue==CW_MSG_TYPE_VALUE_JOIN_REQUEST))
			{
			    CWLog("--> receive msgType:%d in Run State... we close thread",controlVal.messageTypeValue);
			    /*AP可能重启了*/
			    CWCloseThread();
			}
			else{
...
#ifdef URCP_IPC
                CWThreadMutexLock(&gWTPs[WTPIndex].urcpMutex);
#endif
#ifdef CW_NO_DTLS
...
#ifdef URCP_IPC
				CWThreadMutexUnlock(&gWTPs[WTPIndex].urcpMutex);
#endif
				return CW_FALSE;
			}
#ifdef URCP_IPC

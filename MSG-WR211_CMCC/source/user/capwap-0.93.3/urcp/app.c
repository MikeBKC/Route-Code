...
static uint16 gRfType = RF_FLAG_24G;
#endif
...
uint16 receive_number_channel_5g(cmd_hdr_t* cmd, urcp_dev_t** dev)
{
    uint16 ret=URCP_OK;
    uint16 *channel_head = NULL, *channel_end = NULL;
	...
	if (0 == ntohs( *channel_head))
	{
	    (*dev)->wl_info_5G.channel_head = ntohs(0);
	    (*dev)->wl_info_5G.channel_end = ntohs(*channel_end);
	}
	else
	{
	    (*dev)->wl_info_5G.channel_head = ntohs(*channel_head);
	    (*dev)->wl_info_5G.channel_end = ntohs(0);
	}
    }
    else
    {
...
uint16 get_discover(cmd_hdr_t *cmd, uchar **app, urcp_infor_t* urcp_info)
{
   ...
   // MibProfileType mibType = MIB_PROF_FAT_FIT_SWITCH;
    ...
    URCP_DEBUG(("%s\n", __func__)); 
    db_loud(URCP_MODULE_ID_APP, "%s: mac: %s\n", __func__, str_macaddr(urcp_info->mac));
    cmd_rev = (cmd_discover_t*)(cmd);
#ifndef URCP_AP
    cmd_snd = (cmd_discover_t*)(*app);
#endif
    /*数据部分*/
#ifdef URCP_AP
    db_loud(URCP_MODULE_ID_APP, "    state: %d\n", read_state());
    if(ntohl(cmd_rev->send_flag) & DEV_TYPE_AC)
    {
	state = read_state();
	if((ntohl(cmd_rev->recv_flag) & DEV_TYPE_FIT_AP) && (state == AP_STATE_DISCOVERY) || (state == AP_STATE_UNMANGEDED))
	{
	    if((urcp_info->fromL3 == 1) || (gSocketL3==-1) || (ap_infor.current_timer>2)) {
		ac_ip_mac_set(urcp_info->mac, urcp_info->urcp_header->ip, urcp_info->fromL3);
	    }
	}
    }
#if 0
...
#if t
	    cmd_snd -> send_flag = htonl(DEV_TYPE_FAT_AP | DEV_TYPE_WIRELESS_5G);
	...
	else
	{
		...
		if (ap_infor.ap_auth == 1) {
			...
			if (0 == mac_cmp(ap_infor.ac_infor.mac, urcp_info->mac)) {
				DBCK();
				cmd_snd -> send_flag = htonl(DEV_TYPE_FIT_AP|DEV_FLAG_MANAGE|DEV_FLAG_AUTHED|DEV_TYPE_WIRELESS_5G);
//				cmd_snd -> send_flag = htonl(DEV_TYPE_FIT_AP|DEV_FLAG_MANAGE|DEV_FLAG_AUTHED);
			}
		} else {
			DBCK();
			cmd_snd -> send_flag = htonl(DEV_TYPE_FIT_AP|DEV_FLAG_MANAGE|DEV_TYPE_WIRELESS_5G);
...
#endif
    *dev = dev_get(urcp_info->mac,urcp_info->vid, ntohl(urcp_info->urcp_header->ip), val, urcp_info->fromL3);
...
uint16 receive_ap_update_fireware(cmd_hdr_t *cmd, urcp_dev_t **dev, urcp_infor_t* urcp_info)
{
    ...
    db_trace(URCP_MODULE_ID_APP, "get update fireware response: %d\n", ntohl(cmd_hdr_gen->value));
    if(read_state()==AP_STATE_SOFEUPDATE_REQ)
    {     
...
uint16 get_ap_update_fireware(cmd_hdr_t *cmd_buf, uchar **app_data, urcp_infor_t* urcp_info)
{
    ...
    db_trace(URCP_MODULE_ID_APP, "%s request for image\n", str_macaddr(urcp_info->mac));
    ProfInstLowHigh(mibType, &max, &min);

    dev = dev_get(urcp_info->mac, urcp_info->vid, ntohl(urcp_info->urcp_header->ip), (DEV_TYPE_FIT_AP|DEV_FLAG_MANAGE), urcp_info->fromL3);
...
#if 0
			    sprintf(tp, "cp %s%s %s%s_%02x%02x%02x%02x%02x%02x", tmp_path, prof->sw_name, TFTP_ROOT, 
				    IMAGE_FILE,urcp_info->mac[0],urcp_info->mac[1],urcp_info->mac[2],urcp_info->mac[3],urcp_info->mac[4],urcp_info->mac[5]);
#endif

                            strncpy(dev->dev_model, prof->sw_model, (DEV_TYPE_SIZE-1));
                            strncpy(dev->hw_version, prof->hw_version, (HW_VERSION_SIZE-1));

			    strcat(tmp_path, prof->sw_name);
			    if(access(tmp_path, 0)!= -1)
			    {
				value = 1u;
				memset(fileName,0,sizeof(fileName));
				sprintf(fileName, "%s_%s.bin",prof->sw_model, prof->hw_version);
			        sprintf(tp, "%s%s", TFTP_ROOT, fileName);
				if(access(tp, 0) == -1)
				{
				    if(symlink(tmp_path, tp)!=-1) {
					db_trace(URCP_MODULE_ID_APP,"need to update\n");
				    }
				    else {
					value = 0u;
					db_trace(URCP_MODULE_ID_APP,"symlink error\n");
				    }
				}
			    }
#ifndef URCP_REL
			    else {
				db_trace(URCP_MODULE_ID_APP, "Failed to open image file\n");
			    }
#endif
			}
#ifndef URCP_REL
			else {
			    db_trace(URCP_MODULE_ID_APP,"dev ptr is NULL\n");
			}
#endif
		}
	    }
...
{
    db_trace(URCP_MODULE_ID_APP, "get update profile response: %d\n", ntohl(((cmd_hdr_gen_t*)cmd)->value));
...
static void config_temp_convert_config_file(ApConfTempProfile *prof, ServiceZoneProfile *profSz, int32 sn)
{
    ...
    memset(tp, 0, sizeof(tp));
    sprintf(tp, "%s%s_%u", TFTP_ROOT, CONFIG_FILE, (uint32)sn);
    fp = fopen(tp, "w");
    if(fp != NULL)
    {
	...
	DBCK();
		...
		*/
		...
		/* WPA1_PSK_AND_WPA2_PSK */
			...
			sprintf(ftmp, "name=\"%s\"><active>Yes</active><ssidIsolate>%s</ssidIsolate><limitType>%u</limitType><limitDown>%u</limitDown><limitUp>%u</limitUp><securMode>3</securMode><wepSecurity><kNum1><key_state_action>No</key_state_action></kNum1><kNum2><key_state_action>No</key_state_action></kNum2><kNum3><key_state_action>No</key_state_action></kNum3><kNum4><key_state_action>No</key_state_action></kNum4></wepSecurity><wpaSecurity><radiusPort>0</radiusPort></wpaSecurity><wpaPskSecurity><sharePasswd>%s</sharePasswd><updataTime>%d</updataTime></wpaPskSecurity></case></fitApSZ></config>", profSz->head.name, profSz->ssidIsolate ? "Yes" : "No", profSz->limit_type, profSz->limit_down, profSz->limit_up ,profSz->wpaPskSecurity.sharePasswd, profSz->wpaPskSecurity.updataTime);
#else
		...
		/* NONE */
			...
			memset(ftmp, 0, sizeof(ftmp));
#if (SZ_SHARE_LIMIT_AC == FYES)
			sprintf(ftmp, "name=\"%s\"><active>Yes</active><ssidIsolate>%s</ssidIsolate><limitType>%u</limitType><limitDown>%u</limitDown><limitUp>%u</limitUp><wepSecurity><kNum1><key_state_action>No</key_state_action></kNum1><kNum2><key_state_action>No</key_state_action></kNum2><kNum3><key_state_action>No</key_state_action></kNum3><kNum4><key_state_action>No</key_state_action></kNum4></wepSecurity><wpaSecurity><radiusPort>0</radiusPort></wpaSecurity><wpaPskSecurity /></case></fitApSZ></config>", profSz->head.name, profSz->ssidIsolate ? "Yes" : "No", profSz->limit_type, profSz->limit_down, profSz->limit_up);
#else
...
uint16 get_ap_update_profile(cmd_hdr_t* cmd_buf, uchar** app_data, urcp_infor_t* urcp_info)
{
    ...
    sprintf(sn_name, "%u.xml", (uint32)(ap_sysinfo->product_sn));
    dir=opendir(tmp_path);
	DBCK("tmp_path = %s, sn_name = %s", tmp_path, sn_name);
    db_trace(URCP_MODULE_ID_APP, "%s request for profile\n", str_macaddr(urcp_info->mac));
	...
	if ((found == 0u) && (noInVlan == 0u)){
		...
		if ((strcmp(ap_sysinfo->product_type, vlan_ap_info[vid].ap_info.product_type) == 0)
			...
			DBCK();
			sprintf(sn_name, "%u.xml", vlan_ap_info[vid].ap_info.product_sn);
			...
			if(dir != NULL)
			{
				while ( (ptr = readdir(dir)) != NULL)
				{
					if (strcmp (ptr->d_name, sn_name) == 0)
					{
						...
						found = 1u;
						memset(tp, 0, sizeof(tp));
						sprintf(tp, "cp %s%s %s%u.xml -f", tmp_path, sn_name, tmp_path, ap_sysinfo->product_sn);
						DBCK("%s", tp);
						system(tp);
						break;
					}
    ...
    {
	memset(tp, 0, sizeof(tp));
	sprintf(tp, "cp %s%s %s%s_%u -f", tmp_path, sn_name, TFTP_ROOT, CONFIG_FILE, ap_sysinfo->product_sn);
	DBCK("%s", tp);
	URCP_DEBUG(("CMD:%s\n", tp));
	system(tp);
	found = 0u;
...
uint16 receive_ap_auth(cmd_hdr_t *cmd, urcp_dev_t **dev, urcp_infor_t* urcp_info)
{
    ...
    set_ap_authorize(1u);          
    db_trace(URCP_MODULE_ID_APP, "get auth response\n");
    if(read_state() == AP_STATE_UNAUTH) {
        change_state(AP_STATE_SOFEUPDATE_REQ);   
    }
...
void urcp_ap_affirm(uchar *mac, int32 fromL3)
{
	uint16 id;
	uint32 len;
	urcp_hdr_t *header;
	...
	urcp_send(0u, &(buffer[0]), len, fromL3);
...
void urcp_dev_table_ap_affirm()
{
	...
	for (i = 0; i < DEV_TABLE_SIZE; i++) {
		...
		if (0 != mac_cmp(tmp->mac, mac)) {
			...
			urcp_ap_affirm(tmp->mac, tmp->fromL3);
		}
	}
...
static int getChannelAp(int *newchannel)
{
    FILE *fp;
    char line[128];
    char parm1[12],chan[12];
    char channel[8] = {0};

    fp  = popen("iwconfig","r");
    if(fp == NULL)
    {   
        printf("popen iwconfig error\n");
        return -1;
    }
    ...
    {   
        sscanf(line,"%s %s",parm1,chan);
        if(strncmp(chan,"Channel=",8)==0)
        {   
            strncpy(channel,&chan[8],2);
            break;
        }   
        memset(line,0,sizeof(line));    
        memset(chan,0,sizeof(chan));
...
{   
    ...
    if(rf == RF_FLAG_5G){
	cmd -> type = htonl(TRAP_AP_CHANNEL_NEW_5G);
	index = 1;
    }
    else {
#endif
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    if(rf == RF_FLAG_5G){
	fp = popen("iwpriv rai0 get Channel", "r");
    }
    else {
#endif
	...
	if ( fp != NULL)
	{
	    ...
	    fgets(result, sizeof(result), fp);
#endif
        ret = getChannelAp(&channel);
//	    ret = sscanf(result,"Channel:%hd", &channel);
	    if ((ret == 0) && (channel > 0))
	    {
		/*AP CHANNEL*/
		if (prof->mBaseCf.AutoChannelSelect != 0) 
...
{   
    ...
    cmd = (cmd_hdr_t*)(*app_data);
    /*构建hello回复命令*/
    cmd->type = htonl(CMD_AP_ROAMING_TH_5G);

    tmp = (uint32 *)(*app_data + sizeof(cmd_hdr_t));

    if(prof != NULL)
    {	
	DBCK("dbm = %d", prof->dbm_5g);
	*tmp = htonl(prof->dbm_5g);
	/*length*/
	cmd->length = sizeof(cmd_hdr_t) + sizeof(uint32);
	cmd->cmd_status = htons(URCP_OK);
    }
...
{
   // int i, idx=FIT_AP_WL_24G_PF_START;
    int i;
    uint16 j=0;
    char *tmp;
    uint16 ret = URCP_OK;
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES) 
    if(urcp_info != NULL) {
	rf_type = urcp_info->rf;
    }
    if(rf_type==RF_FLAG_5G){
	mibType=MIB_PROF_FIT_AP_SZ_5G;
	idx = FIT_AP_WL_5G_PF_START;
	default_index = 1;
	if_wl = "rai";
	db_info(URCP_MODULE_ID_APP, "%s: 5G\n", __func__);
    }
#endif
#if 0
    for (i=0; i<4; i++)
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES) 
    if(rf_type==RF_FLAG_5G){
	cmd -> type = htonl(TRAP_AP_SSID_5G);
    }
    else {
#endif
...
//	    get_switch_if_name(if_name);
	...
	else
	{
		...
		DBCK("i = %d", i);
		prof = (FitApSzProfile*)ProfGetInstPointByIndex(mibType,i);
		profW = (WirelessProfile*)ProfGetInstPointByIndex(mibTypeW,(i+idx));
		if((prof != NULL) && (prof->head.active == TRUE))
		{
			DBCK();
			...
			DBCK("szone:%s, SSID=%s, i = %d\n", prof->head.name, profW->mBaseCf.SSID1, i);
//			printf("szone:%s, SSID=%s, i = %d\n", prof->head.name, profW->mBaseCf.SSID1, i+idx);
//		    get_switch_if_name(if_name);
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
            sprintf(if_name,"%s%d",if_wl, i);
#else
	...
	if (rf_type == RF_FLAG_5G)
	{    
	    profW = (WirelessProfile*)ProfGetInstPointByIndex(mibTypeW,1);
	}
	else
	{    
	    profW = (WirelessProfile*)ProfGetInstPointByIndex(mibTypeW,0);
 ...
 */
static uint32 get_wl_hosts(char *ifName)
{
    int s;
    struct iwreq iwr;
    uint32 host_no = 0u;
...
{   
    ...
    cmd_hdr_t* cmd;
    char *rf_ra = "ra0";
    uint32 cmd_type = TRAP_AP_CLIENTS;

    URCP_DEBUG(("%s()\n", __func__));
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    if ((urcp_info != NULL) && (urcp_info->rf == RF_FLAG_5G))
    {
	rf_ra = "rai0";
	cmd_type = TRAP_AP_CLIENTS_5G;
    }
#endif 
    cmd = (cmd_hdr_t*)(*app_data);
    /*构建hello回复命令*/
    cmd -> type = htonl(cmd_type);
    cmd -> cmd_status = htons(URCP_OK);

    tmp = (uint32*)(*app_data + sizeof(cmd_hdr_t));

    /*AP active clients*/
    max = get_wl_hosts(rf_ra);
    URCP_DEBUG(("AP clients = %d\n", max));
    *tmp = htonl(max);
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
uint16 trap_get_ap_clients_5g(cmd_hdr_t *cmd_buf, uchar **app_data, urcp_infor_t* urcp_info)
{   
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    uint16 rf = RF_FLAG_24G;
#endif
	...
	/*AP active clients*/
	max = get_wl_hosts("rai0");
	db_loud(URCP_MODULE_ID_APP, "%s: 5G clients %d\n", __func__, max);
    }
    else {
#endif
...
#if (CAPWAP_CONFIG_AP == FYES)
    if ( (isChange == 1u) && (prof->dhcp_en == 1))
    {
	chang_ip_type(NEED_CHANGE_DHCP_IP);
    }
#endif
    scmd -> cmd_status = htons(URCP_OK);
    /*构建hello回复命令*/
...
uint16 set_gateway_config(cmd_hdr_t *cmd, uchar** app_data)
{
	...
	config_backup_state(NEED_BACKUP);
#if (CAPWAP_CONFIG_AP == FYES)
    chang_ip_type(NEED_CHANGE_STATIC_IP);
#endif
    scmd -> cmd_status = htons(ret);
    /*构建hello回复命令*/
    scmd -> type = cmd->type;
...
uint16 set_ap_channel_config_5g(cmd_hdr_t *cmd, uchar** app_data)
{
	...
	DBCK();
	    prof->mBaseCf.AutoChannelSelect = 2;
		prof->mBaseCf.Channel = channel_no;
		system("iwpriv rai0 set AutoChannelSel=2");
	}
	else
	{
	DBCK();
	    prof->mBaseCf.AutoChannelSelect = 0;
		prof->mBaseCf.Channel = channel_no;
		sprintf(str, "iwpriv rai0 set Channel=%u", channel_no);
		system(str);
	}
    }
...
uint16 receive_air_probe(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
	...
	while(len<l)
	{
	    ...
	    {
		wl_dev=dev_get(tmp->mac, urcp_info->vid, 0u, DEV_TYPE_WIRELESS, urcp_info->fromL3);
		if(wl_dev!=NULL)
		{
...
uint16 trap_get_wireless_mode(cmd_hdr_t *cmd_buf, uchar **app_data, urcp_infor_t* urcp_info)
{
	...
	else if(md==9)
	{
		...
		else if(md==AND_11A_11N) {
		    md = AP_MODE_11AN;
		}
		else if(md==AND_11AC_11AN_11A) {
		    md = AP_MODE_11AC_AN_A;
		}
		else if(md==AND_11AC_11AN) {
		    md = AP_MODE_11AC_AN;
		}
	    }
	}
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
        if( rf==RF_FLAG_5G){
	    cmd -> type = htonl(TRAP_WIRELESS_MODE_5G);
	}
	else{
#endif
	...
	*app_data += cmd -> length;
    ...
    else if((prof != NULL) && (prof->head.active == 0))/*无线功能处于关闭状态*/
    {
    cprintf("------------line = %d----func =%s----\n",__LINE__,__func__);
	md = 0xFFFFFFFFu;
	cmd = (cmd_hdr_t*)(*app_data);
	/*构建hello回复命令*/
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
...
uint16 receive_wireless_mode(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
	...
	if( (md==AP_MODE_11BGN) || (md==AP_MODE_11B) || (md==AP_MODE_11G)
	    ...
	    || (md==AP_MODE_11AN) || (md==AP_MODE_11GN)
            || (md==AP_MODE_11AC_AN) || (md==AP_MODE_11AC_AN_A))
	{
	    (*dev)->wl_info.ap_flag |= md;
	    URCP_DEBUG(("%s: %d\n", __func__, md));
	    db_loud(URCP_MODULE_ID_APP, "%s: mode: %d\n", __func__, md);
	}
    }
    else
    {
...
uint16 set_ap_config_template(cmd_hdr_t *cmd, uchar** app_data)
{
	...
	if((prof1 != NULL) && (prof1->config == PROF_DISABLE)) /*no local*/
	{
	    /*save config*/
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
        if(urcp_conf_temp->wlFre == RF_FLAG_24G){
	    ProfBackupByIndex(mibType, PROF_CHANGE_EDIT, 0, &profhead);
	    prof = (WirelessProfile*)ProfGetInstPointByIndex(mibType, 0);
	}
	else {
            db_loud(URCP_MODULE_ID_APP,"%s: set 5G template\n", __func__); 
	    prof = (WirelessProfile*)ProfGetInstPointByIndex(mibType, 1);
...
//		    prof->mBaseCf.NoForwarding = FUN_DISABLE;
		...
		/*无线频段*/
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		if((urcp_conf_temp->wlFre == RF_FLAG_5G) && ((urcp_conf_temp->mode == 5u) || (urcp_conf_temp->mode == 6u)))
		{
		    prof->mBaseCf.VChanWidth = ntohl(urcp_conf_temp->VChanWidth);
		}
#endif
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		else if(urcp_conf_temp->wlFre == RF_FLAG_5G){
		    if (urcp_conf_temp->mode == 3u) {
			prof->mBaseCf.WirelessMode = ONLY_11A;
		    }
		    else if (urcp_conf_temp->mode == 4u) {
			prof->mBaseCf.WirelessMode = AND_11A_11N;
		    }
		    else if (urcp_conf_temp->mode == 5u) {
			prof->mBaseCf.WirelessMode = AND_11AC_11AN_11A;
		    }
		    else if (urcp_conf_temp->mode == 6u) {
			prof->mBaseCf.WirelessMode = AND_11AC_11AN;
		    }
		    else 
		    {
		...
		else if(urcp_conf_temp->wlFre == RF_FLAG_24G)
		{
		    prof->mBaseCf.WirelessMode = AND_11G_11N;
		    prof->mBaseCf.BasicRate = 15;
		}
...
uint16 set_sz_name_config(cmd_hdr_t *cmd, uchar** app_data)
{
		...
		MibProfileType mibTypeS = MIB_PROF_FIT_AP_SZ;
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		MibProfileType mibTypeS5g = MIB_PROF_FIT_AP_SZ_5G;
		FitAp5gSzProfile *profS5g = NULL;
#endif
		...
		tmp = (uchar*)cmd;
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		rf = ntohs(cmd->cmd_status);
                if( (rf==RF_FLAG_24G) || (rf==RF_FLAG_5G) || (rf==RF_FLAG_24G_5G)) {
		    gRfType = rf;
		}
		else {
                    URCP_ERROR(("%s: error rf type: 0x%04x\n", __func__, rf));
		    return URCP_FATAL_ERROR;
		}
#endif
		len = ntohs(cmd->length) - sizeof(cmd_hdr_t);
		...
		{
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		    if( (rf==RF_FLAG_24G) || (rf==RF_FLAG_24G_5G) ){
			sprintf(tmpStr, "wireless%d",(sz_num + FIT_AP_WL_24G_PF_START));
                        prof = (WirelessProfile *)ProfGetInstPointByIndex(mibType, (sz_num+FIT_AP_WL_24G_PF_START));
		    ...
		    if( (rf==RF_FLAG_5G) || (rf==RF_FLAG_24G_5G) ){
			sprintf(tmpStr, "wireless%d",(sz_num + FIT_AP_WL_5G_PF_START));
			prof = (WirelessProfile *)ProfGetInstPointByIndex(mibType, (sz_num+FIT_AP_WL_5G_PF_START));
			profS5g = (FitAp5gSzProfile*)ProfGetInstPointByIndex(mibTypeS5g, sz_num);
			ProfSetNameByPoint(prof, tmpStr);
			ProfSetNameByPoint(profS5g, tmp2);
			URCP_DEBUG(("\n< %s %d > Wireless head.name:%s FitAp head.name:%s\n",__func__, __LINE__, prof->head.name, profS5g->head.name));
			prof->head.active = TRUE;
			profS5g->head.active = TRUE;
			profS5g->client = 0u;
		    }
#else
		...
		for (i=sz_num; i<4; i++)
		{
		    ...
		    if( (rf==RF_FLAG_24G) || (rf==RF_FLAG_24G_5G) )
		    {
                        prof = (WirelessProfile *)ProfGetInstPointByIndex(mibType, (i+FIT_AP_WL_24G_PF_START));
			prof->head.active = FALSE;
		    ...
		    if( (rf==RF_FLAG_5G) || (rf==RF_FLAG_24G_5G) )
		    {
			prof = (WirelessProfile *)ProfGetInstPointByIndex(mibType, (i+FIT_AP_WL_5G_PF_START));
			prof->head.active = FALSE;
			memset(prof->head.name, '\0', sizeof(prof->head.name));
			profS5g = (FitAp5gSzProfile *)ProfGetInstPointByIndex(mibTypeS5g, i);
			memset(profS5g->head.name, '\0', sizeof(profS5g->head.name));
			profS5g->head.active = FALSE;
			profS5g->client = 0u;
		    }
#else
...
uint16 set_sz_share_limit_config(cmd_hdr_t *cmd, uchar** app_data)
{
    ...
    len = ntohs(cmd->length) - sizeof(cmd_hdr_t);
    if(len == sizeof(sz_share_limit))
    {
	memcpy(sz_share_limit,((uchar *)cmd + sizeof(cmd_hdr_t)), len);
	for (i = 0; i<4; i++)
	{
//	    printf("%s, type:%u, down=%u, up=%u\n", __func__, ntohl(sz_share_limit[i].limit_type), ntohl(sz_share_limit[i].limit_down), ntohl(sz_share_limit[i].limit_up));
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
	    rf = ntohs(cmd->cmd_status);
	    if( (rf==RF_FLAG_24G) || (rf==RF_FLAG_24G_5G) )
	    {
		profS = (FitApSzProfile *)ProfGetInstPointByIndex(mibTypeS, i);
		if ((profS != NULL) && (profS->head.active == TRUE))
		{
		    profS->limit_type = ntohl(sz_share_limit[i].limit_type);
		    profS->limit_down = ntohl(sz_share_limit[i].limit_down);
		    profS->limit_up = ntohl(sz_share_limit[i].limit_up);
		}
	    }
	    if( (rf==RF_FLAG_5G) || (rf==RF_FLAG_24G_5G) )
	    {
		profS5g = (FitAp5gSzProfile *)ProfGetInstPointByIndex(mibTypeS5g, i);
	    	if ((profS5g != NULL) && (profS5g->head.active == TRUE))
		{
		    profS5g->limit_type = ntohl(sz_share_limit[i].limit_type);
		    profS5g->limit_down = ntohl(sz_share_limit[i].limit_down);
		    profS5g->limit_up = ntohl(sz_share_limit[i].limit_up);
		}
	    }
#else
	    profS = (FitApSzProfile *)ProfGetInstPointByIndex(mibTypeS, i);	
...
uint16 set_sz_ssid_config(cmd_hdr_t *cmd, uchar** app_data)
{
    ...
    MibProfileType mibType = MIB_PROF_WIRELESS;
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    uint16 rf = ntohs(cmd->cmd_status);
#endif
    memset(str, 0, sizeof(str));
    scmd = (cmd_hdr_t*)(*app_data);
    tmp = (uchar*)cmd;
	...
	if( (rf==RF_FLAG_24G) || (rf==RF_FLAG_24G_5G) )
	{
	    prof = (WirelessProfile *)ProfGetInstPointByIndex(mibType, (num + FIT_AP_WL_24G_PF_START));
	    strncpy(prof->mBaseCf.SSID1, tmp2, UTT_SSID_LENTH+1);
	}
	if( (rf==RF_FLAG_5G) || (rf==RF_FLAG_24G_5G) )
	{
	    prof = (WirelessProfile *)ProfGetInstPointByIndex(mibType, (num + FIT_AP_WL_5G_PF_START));
	    strncpy(prof->mBaseCf.SSID1, tmp2, UTT_SSID_LENTH+1);
	}
#else
...
uint16 set_sz_ssid_isolate_config (cmd_hdr_t *cmd, uchar** app_data)
{
    ...
    URCP_DEBUG(("\n< %s %d >num:%d\n",__func__,__LINE__,num));
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    if( (gRfType == RF_FLAG_24G) || (gRfType == RF_FLAG_24G_5G) )
    {
	prof = (FitApSzProfile *)ProfGetInstPointByIndex(mibType, i);
	prof->ssidIsolate = num;
    }
    if( (gRfType == RF_FLAG_5G) || (gRfType == RF_FLAG_24G_5G) )
    {
	prof_5g = (FitAp5gSzProfile *)ProfGetInstPointByIndex(MIB_PROF_FIT_AP_SZ_5G, i);
	prof_5g->ssidIsolate = num;
    }
#else
	prof = (FitApSzProfile *)ProfGetInstPointByIndex(mibType, i);
	prof->ssidIsolate = num;
...
uint16 set_ap_roaming_th(cmd_hdr_t *cmd, uchar** app_data)
{
    ...
    if(isChange == 1u)
    {
        sprintf(str, "iwpriv ra0 set KickStaRssiLow=-%u;iwpriv ra1 set KickStaRssiLow=-%u;iwpriv ra2 set KickStaRssiLow=-%u;iwpriv ra3 set KickStaRssiLow=-%u", dbm, dbm, dbm, dbm);
        system(str);
        sprintf(str, "iwpriv ra0 set AssocReqRssiThres=-%u;iwpriv ra1 set AssocReqRssiThres=-%u;iwpriv ra2 set AssocReqRssiThres=-%u;iwpriv ra3 set AssocReqRssiThres=-%u", dbm, dbm, dbm, dbm);
        system(str);
    }
...
uint16 set_ap_roaming_th_5g(cmd_hdr_t *cmd, uchar** app_data)
{
    ...
    if(isChange == 1u)
    {
		sprintf(str, "iwpriv rai0 set KickStaRssiLow=-%u; iwpriv rai1 set KickStaRssiLow=-%u; iwpriv rai2 set KickStaRssiLow=-%u; iwpriv rai3 set KickStaRssiLow=-%u 1>/dev/null 2>&1", dbm, dbm, dbm, dbm);
		DBCK("%s", str);
		system(str);
		sprintf(str, "iwpriv rai0 set AssocReqRssiThres=-%u; iwpriv rai1 set AssocReqRssiThres=-%u; iwpriv rai2 set AssocReqRssiThres=-%u; iwpriv rai3 set AssocReqRssiThres=-%u 1>/dev/null 2>&1", dbm, dbm, dbm, dbm);
		DBCK("%s", str);
		system(str);
    }
...
uint16 set_sz_wep_config(cmd_hdr_t *cmd, uchar** app_data)
{
	...
	{
	    prof_5g = (FitAp5gSzProfile *)ProfGetInstPointByIndex(MIB_PROF_FIT_AP_SZ_5G, i);
	    prof_5g->wepSecurity.auMethod = ntohl(num.auMethod);
	    prof_5g->wepSecurity.kType = ntohl(num.kType);
	    prof_5g->wepSecurity.kNum1.key_state_action = ntohl(num.kNum1.key_state_action);
	    strncpy(prof_5g->wepSecurity.kNum1.passwd, num.kNum1.passwd, UTT_PAS_KEY_VALUE_LEN+1);
	    prof_5g->wepSecurity.kNum1.kFmt = ntohl(num.kNum1.kFmt);
	    prof_5g->wepSecurity.kNum2.key_state_action = ntohl(num.kNum2.key_state_action);
	    strncpy(prof_5g->wepSecurity.kNum2.passwd, num.kNum2.passwd, UTT_PAS_KEY_VALUE_LEN+1);
	    prof_5g->wepSecurity.kNum2.kFmt = ntohl(num.kNum2.kFmt);
	    prof_5g->wepSecurity.kNum3.key_state_action = ntohl(num.kNum3.key_state_action);
	    strncpy(prof_5g->wepSecurity.kNum3.passwd, num.kNum3.passwd, UTT_PAS_KEY_VALUE_LEN+1);
	    prof_5g->wepSecurity.kNum3.kFmt = ntohl(num.kNum3.kFmt);
	    prof_5g->wepSecurity.kNum4.key_state_action = ntohl(num.kNum4.key_state_action);
	    strncpy(prof_5g->wepSecurity.kNum4.passwd, num.kNum4.passwd, UTT_PAS_KEY_VALUE_LEN+1);
	    prof_5g->wepSecurity.kNum4.kFmt = ntohl(num.kNum4.kFmt);
	}
#else
	prof = (FitApSzProfile *)ProfGetInstPointByIndex(mibType, i);
	prof->wepSecurity.auMethod = ntohl(num.auMethod);
...
uint16 set_sz_wpa_config(cmd_hdr_t *cmd, uchar** app_data)
{
	...
	{
	    prof_5g = (FitAp5gSzProfile *)ProfGetInstPointByIndex(MIB_PROF_FIT_AP_SZ_5G, i);
	    prof_5g->wpaSecurity.wVersion = ntohl(num.wVersion);
	    prof_5g->wpaSecurity.sAlg = ntohl(num.sAlg);
	    prof_5g->wpaSecurity.radiusIp = num.radiusIp;
	    prof_5g->wpaSecurity.radiusPort = ntohl(num.radiusPort);
	    strncpy(prof_5g->wpaSecurity.radiusPasswd, num.radiusPasswd, UTT_RADIUS_KEY_LEN);
	    prof_5g->wpaSecurity.updataTime = ntohl(num.updataTime);
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
void changeToWirelessProfile5G(void)
{
    int i;
    FitAp5gSzProfile *prof = NULL;
    MibProfileType mibType = MIB_PROF_FIT_AP_SZ_5G;
    WirelessProfile *profW=NULL, *tmp = NULL;
    MibProfileType mibTypeW = MIB_PROF_WIRELESS;
    
    for (i=0; i<4; i++)
    {
	prof = (FitAp5gSzProfile *)ProfGetInstPointByIndex(mibType, i);
	if ((prof != NULL) && (prof->head.active == TRUE))
	{
	    profW = (WirelessProfile *)ProfGetInstPointByIndex(mibTypeW, (i + FIT_AP_WL_5G_PF_START));
...
void changeToWirelessProfile(void)
{
	...
	{
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
	    profW = (WirelessProfile *)ProfGetInstPointByIndex(mibTypeW, (i + FIT_AP_WL_24G_PF_START));
#else
	    profW = (WirelessProfile *)ProfGetInstPointByIndex(mibTypeW, (i+1));
#endif
	    memset(&(profW->mSafeCf),0,sizeof(profW->mSafeCf));
...
void wl_24G_init(void)
{
	...
	for (i=0; i<4; i++)/*这里用4代替sz_num，使MAC地址过滤能够生效，参数要重新设置*/
	{
		...
		if ((prof->limit_type == 0) || (prof->limit_type == 1) || (prof->limit_type == 2))
		{
...
void wl_5G_init(void)
{
	...
	if (prof->head.active == TRUE)
	{
	    sprintf(str, "iwpriv rai%d set VLANID=%u;iwpriv rai%d set NoForwarding=%u;", i, prof->vid, i, prof->ssidIsolate);
	    system(str);
	    if(prof->ssidIsolate)
	    {
		sprintf(str, "iwpriv rai%d set NoForwardingBTNBSSID=%u;", i, prof->ssidIsolate);
		system(str);
	    }
	    ...
	    if ((prof->limit_type == 0) || (prof->limit_type == 1) || (prof->limit_type == 2))
	    {
		sprintf(str, "iwpriv rai%d set StaRate=%u:%u:%u", i, prof->limit_type , prof->limit_down, prof->limit_up);
		system(str);
	    }
#endif
	}
    ...
    profBase = (ApBasicConfProfile *)ProfGetInstPointByIndex(MIB_PROF_AP_BASIC_CONF, 0);
    if ((profBase != NULL) && (profBase->dbm_5g != 0)) 
    {
...
uint16 set_sz_psk_config(cmd_hdr_t *cmd, uchar** app_data)
{
	...
	{
	    prof_5g = (FitAp5gSzProfile *)ProfGetInstPointByIndex(MIB_PROF_FIT_AP_SZ_5G, i);
	    prof_5g->wpaPskSecurity.wVersion = ntohl(num.wVersion);
	    prof_5g->wpaPskSecurity.sAlg = ntohl(num.sAlg);
	    strncpy(prof_5g->wpaPskSecurity.sharePasswd, num.sharePasswd, UTT_RADIUS_KEY_LEN);
	    prof_5g->wpaPskSecurity.updataTime = ntohl(num.updataTime);
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
            system("iwpriv rai0 set BYPASS=1");
#endif
        }else if( mode == 0){
            system("iwpriv ra0 set BYPASS=0");
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
            system("iwpriv rai0 set BYPASS=0");
#endif
        }
    }
    scmd -> cmd_status = htons(URCP_OK);
...
void szone_auto_updata_to_ap_5g(urcp_dev_t *dev)
{
    int i;
    urcp_ap_join_sz_msg_t urcp_ap_join_sz_msg;
    memset(&urcp_ap_join_sz_msg,0,sizeof(urcp_ap_join_sz_msg));
    for(i=0;i<dev->wl_info_5G.szone_info.szone_num;i++){
        if( i !=0 ){
	        strcat(urcp_ap_join_sz_msg.sz_name, ",");
        }
	    strcat(urcp_ap_join_sz_msg.sz_name, dev->wl_info_5G.szone_info.szone[i]);
    }
//    printf("auto updata sz_name: %s %s\n",urcp_ap_join_sz_msg.sz_name,__func__);
	memcpy(urcp_ap_join_sz_msg.general_msg.dst_mac, dev->mac, MAC_LEN);
	urcp_ap_join_sz_msg.general_msg.opt_type = MISC_SET_GENERAL;/* 设置消息类型 */
...
uint16 trap_receive_ssid(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
    ...
    if((*dev)->wl_info.szone_info.szone_changed == 1){
	    ...
	    if ((*dev)->wl_info_5G.szone_info.szone_changed != 1)
	    {
...
uint16 trap_receive_ssid_5g(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
	...
	(*dev)->wl_info_5G.joined_sz_num = (uint16)ntohl(cmd_ssid->value);
	ssid = (ssid_t*)(cmd_ssid+1);
#if SZONE_AUTO_UPDATA
	uttSemP(SEM_SZONE_UPDATA, 0);
#if 0
	if((*dev)->wl_info_5G.szone_info.auto_updata != 1){
	    (*dev)->wl_info_5G.szone_info.szone_changed = 0;
	}
#endif
	if((*dev)->wl_info_5G.szone_info.szone_changed == 1)
	{
	    if((*dev)->wl_info_5G.szone_info.count > 0)
	    {
		(*dev)->wl_info_5G.szone_info.count -= 1;
	    }
	    else
	    {
		szone_auto_updata_to_ap_5g(*dev);
		(*dev)->wl_info_5G.szone_info.r_times += 1;
		switch((*dev)->wl_info_5G.szone_info.r_times)
		{
		    case 0:
			break;
		    case 1:
			(*dev)->wl_info_5G.szone_info.count = 2;
			break;
		    case 2:
			(*dev)->wl_info_5G.szone_info.count = 4;
			break;
		    case 3:
			(*dev)->wl_info_5G.szone_info.count = 8;
			break;
		    case 4:
			(*dev)->wl_info_5G.szone_info.szone_changed = 0;
			(*dev)->wl_info_5G.szone_info.r_times = 0;
			(*dev)->wl_info_5G.szone_info.count = 0;
			break;
		    default:
			(*dev)->wl_info_5G.szone_info.r_times = 0;
			(*dev)->wl_info_5G.szone_info.count = 0;
	...
	{
	    memset((*dev)->wl_info_5G.szone_info.szone,0,sizeof((*dev)->wl_info_5G.szone_info.szone));
	    (*dev)->wl_info_5G.szone_info.szone_num = val;
	}
#endif
	for(i=0;i<val;i++)
	{
	    memset((*dev)->wl_info_5G.ssid_info[i].ssid,0,sizeof((*dev)->wl_info_5G.ssid_info[i].ssid));
	    URCP_DEBUG(("%s,line=%d,val=%d\n", __func__,__LINE__,val));
#if (SSID_ENCODE_TYPE == FYES)
	    profSZ = (ServiceZoneProfile *)ProfGetInstPointByName(MIB_PROF_SERVICE_ZONE, (ssid + i)->szone);
	    if(profSZ != NULL)
		...
		if(ap_ssid != NULL)
		{
		    strncpy((*dev)->wl_info_5G.ssid_info[i].ssid, ap_ssid, UTT_SSID_LENTH);
		}
		else
		{
		    strncpy((*dev)->wl_info_5G.ssid_info[i].ssid, tmp, UTT_SSID_LENTH);
		}
	    }
	    else
	    {
		URCP_DEBUG(("%s,line=%d\n", __func__,__LINE__));
		strncpy((*dev)->wl_info_5G.ssid_info[i].ssid, (ssid + i)->ssid, UTT_SSID_LENTH);
	    }
	    if(ap_ssid != NULL)
	    {
		free(ap_ssid);
	    ...
	    URCP_DEBUG(("%s ,%d,ssid:%s\n", __func__,__LINE__, (*dev)->wl_info_5G.ssid_info[i].ssid));
	    strncpy((*dev)->wl_info_5G.ssid_info[i].szone, (ssid + i)->szone, UTT_SZONE_LENTH);
#if SZONE_AUTO_UPDATA
	    if((*dev)->wl_info_5G.szone_info.szone_changed == 0)
	    {
		strncpy((*dev)->wl_info_5G.szone_info.szone[i], (ssid + i)->szone, UTT_SZONE_LENTH);
	    }
#endif
	    memcpy((*dev)->wl_info_5G.ssid_info[i].mac, (ssid + i)->mac, MAC_LEN);
	}
#if SZONE_AUTO_UPDATA
	uttSemV(SEM_SZONE_UPDATA, 0);
#endif
...
uint16 trap_receive_ap_channel_new_5g(cmd_hdr_t* cmd, urcp_dev_t **dev, urcp_infor_t* urcp_info)
{
    uint16 ret=URCP_OK;
    URCP_DEBUG(("%s\n", __func__));
    ret = receive_number_channel_5g(cmd, dev); 
    return ret;
}
#endif
...
uint16 trap_receive_clients_5g(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
	...
	/*复制数据*/
	tmp = (cmd_hdr_gen_t *)cmd;
	ptr = (uint32*) &((*dev)->wl_info_5G.clients);
	client = ntohl(tmp->value);
	*ptr = client;
	URCP_DEBUG(("%s: %d\n", __func__, *ptr));
    }
...
uint16 trap_receive_wireless_mode(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
	...
	if( (md==AP_MODE_11BGN) || (md==AP_MODE_11B) || (md==AP_MODE_11G)
	    || (md==AP_MODE_11N) || (md==AP_MODE_11A) || (md==AP_MODE_11BG)
	    || (md==AP_MODE_11AN) || (md==AP_MODE_11GN) || (md==0xFFFFFFFFu))
	{
	    (*dev)->wl_info.ap_flag = md;
	    URCP_DEBUG(("%s: %d\n", __func__, md));
	}
...
uint16 trap_receive_wireless_mode_5g(cmd_hdr_t* cmd, urcp_dev_t** dev, urcp_infor_t* urcp_info)
{
	...
	if( (md==AP_MODE_11BGN) || (md==AP_MODE_11B) || (md==AP_MODE_11G)
	    || (md==AP_MODE_11N) || (md==AP_MODE_11A) || (md==AP_MODE_11BG)
	    || (md==AP_MODE_11AN) || (md==AP_MODE_11GN) || (md == AP_MODE_11AC_AN_A) || (md == AP_MODE_11AC_AN) || (md == 0xFFFFFFFFu))
	{
	    (*dev)->wl_info_5G.ap_flag = md;
	    URCP_DEBUG(("%s: %d\n", __func__, md));
	}
    }
    else
...
uint16 trap_syslog_sta_info(cmd_hdr_t *cmd_buf, urcp_dev_t **dev, urcp_infor_t *urcp_info)
{
	...
	if (ntohl(cmd->sta_status) == AP_STA_CONNECT)
	{
	    db_info(URCP_MODULE_ID_STA_LOG,"%s,connect ap_mac:%s", __func__, str_macaddr(urcp_info->mac));
	    db_info(URCP_MODULE_ID_STA_LOG, " sta_mac:%s\n", str_macaddr(cmd->sta_mac));
	    ac_sta_add_up_log(cmd->sta_mac, urcp_info->mac);
	}
	else
	{
	    db_info(URCP_MODULE_ID_STA_LOG,"%s,disconnect ap_mac:%s", __func__, str_macaddr(urcp_info->mac));
	    db_info(URCP_MODULE_ID_STA_LOG, " sta_mac:%s\n", str_macaddr(cmd->sta_mac));
	}
#endif
...
uint16 ap_disconnect_sta(cmd_hdr_t* cmd, uchar** app)
{
     ...
     for(i=0; i<num; i++)
     {
         if(isValidMac(data) == 1) {
           ...
           system(buf);
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
           sprintf(buf, "iwpriv rai0 set DisConnectSta=%02x:%02X:%02X:%02X:%02X:%02X",
                   data[0],data[1],data[2],data[3],data[4],data[5]);
           system(buf);
#endif
           data += MAC_LEN;
...
uint16 trap_get_ap_rf_rtx(cmd_hdr_t *cmd_buf, uchar **app_data, urcp_infor_t* urcp_info)
{
    ...
    uint32 cmd_type = TRAP_AP_RF_RTX_INFO;
    char *rf_ra = "ra";
    ...
    {
	rf_ra = "rai";
	cmd_type = TRAP_AP_RF_RTX_INFO_5G;
    }
#endif
    cmd = (cmd_hdr_t*)(*app_data);
    /*构建hello回复命令*/
    ...
    if (!get_ap_RTX_info(p_tx_p, p_tx_b, p_rx_p, p_rx_b, rf_ra))
    {
	*p_tx_p = htonl(*p_tx_p);
	*p_tx_b = htonl(*p_tx_b);
	*p_rx_p = htonl(*p_rx_p);
...
uint16 trap_get_ap_rf(cmd_hdr_t *cmd_buf, uchar **app_data, urcp_infor_t* urcp_info)
{
    uint16 ret = URCP_OK;
    cmd_hdr_t *cmd = NULL;
    uint16 rate = 0, power = 0, bea = 0;
    uint16 *p_rate = NULL, *p_power = NULL, *p_bea = NULL, *p_chanw = NULL;
    MibProfileType mibType = MIB_PROF_WIRELESS;
    WirelessProfile *prof= NULL;
    char *rf_ra = "ra";
    uint32 cmd_type = TRAP_AP_RF_INFO;
    int index = 0;
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    if ( (urcp_info != NULL) && (urcp_info->rf == RF_FLAG_5G))
    {
	rf_ra = "rai";
	cmd_type = TRAP_AP_RF_INFO_5G;
	index = 1;
    }
#endif 
    cmd = (cmd_hdr_t*)(*app_data);
...
uint16 trap_receive_ap_rf_rtx_info( cmd_hdr_t *cmd, urcp_dev_t **dev, urcp_infor_t* urcp_info )
{
    ...
    db_info( URCP_MODULE_ID_MAIN, "%s, %d, %d, %d, %d\n", __func__, ntohl(*p_tx_p), ntohl(*p_tx_b), ntohl(*p_rx_p), ntohl(*p_rx_b));
    return ret;
}
#endif
...
uint16 trap_receive_ap_rf_rtx_info_5g( cmd_hdr_t *cmd, urcp_dev_t **dev, urcp_infor_t* urcp_info )
{
    ...
    if ((ntohs(cmd->length) == sizeof(cmd_hdr_t) + 4*sizeof(uint32)) && (dev != NULL) && (*dev != NULL))
    {
    	(*dev)->wl_info_5G.tx_packets = ntohl(*p_tx_p);
	(*dev)->wl_info_5G.tx_bytes = ntohl(*p_tx_b);
	(*dev)->wl_info_5G.rx_packets = ntohl(*p_rx_p);
	(*dev)->wl_info_5G.rx_bytes = ntohl(*p_rx_b);
    }
    else
    {
	ret = URCP_ANY_ERROR;
...
uint16 trap_receive_ap_rf_info_5g( cmd_hdr_t *cmd, urcp_dev_t **dev, urcp_infor_t* urcp_info )
{
    ...
    if ((ntohs(cmd->length) == sizeof(cmd_hdr_t) + 4*sizeof(uint16)) && (dev != NULL) && (*dev != NULL))
    {
	(*dev)->wl_info_5G.bit_rate = ntohs(*p_rate);
	(*dev)->wl_info_5G.chan_width = ntohs(*p_chanw);
	(*dev)->wl_info_5G.power_new = ntohs(*p_power);
	(*dev)->wl_info_5G.beacon = ntohs(*p_bea);
    }
    else
    {
	ret = URCP_ANY_ERROR;
...
//		}
    ...
    if ((index == max) && (prof_free != NULL) && (1 == mode) && (repeat_flag != 1))
    {
	/*新建实例*/
...
//    ApBlackWhiteListProfile *profBl = NULL, *bl_free = NULL;;
	...
	for (i = min + start_index; i < max; i++) 
	{
		...
		flag = 1;
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		strncpy(ssid_info[ssid_num].ssid, profWl->mBaseCf.SSID1, UTT_SSID_LENTH);
		ssid_info[ssid_num].id = ssid_num;
		ssid_num++;
#else
...
//		printf("%s,ssid:%s\n", __func__, profWl->mBaseCf.SSID1);
	...
	flag = 0;
	sz_num = 0;
	ProfInstLowHigh(mibTypeFAS_5G, &max, &min);
	for (i = min; i < max; i++) 
	{
	    profFAS_5G = (FitAp5gSzProfile *)ProfGetInstPointByIndex(mibTypeFAS_5G, i);
	    if ((profFAS_5G != NULL) && (TRUE == profFAS_5G->head.active) && ((ProfInstIsFree((Pointer)profFAS_5G)) == 0))
	    {   
		strncpy(ssid_info[i].sz, profFAS_5G->head.name, UTT_SZONE_LENTH);
		sz_num++;
		flag = 1;
	    }   
	}  
	...
	for (i = min; i < max; i++) 
	{
	    ...
	    {   
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
		strncpy(ssid_info[sz_num].sz, profFAS->head.name, UTT_SZONE_LENTH);
		sz_num++;
#else
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
static int enabled_mac_filter_5g(int mode, int ra_num, char *mac_all)
{
    char str[100] = "";
    sprintf(str, "iwpriv rai%d set ACLClearAll=1", ra_num);
    system(str);
    memset(str, 0, sizeof(str));
    sprintf(str, "iwpriv rai%d set AccessPolicy=%d", ra_num, mode);
    system(str);
    system(mac_all);
//    printf("%s,mac_all:%s\n",__func__, mac_all);
    return 0;
...
void serivce_zone_mac(void)
{
...
#if defined(WIRELESS_5G) && (WIRELESS_5G==FYES)
    MibProfileType mibTypeFAS_5G = MIB_PROF_FIT_AP_SZ_5G;
    FitApSzProfile *profFAS_5G = NULL;
	...
	if ((profFAS_5G != NULL) && (TRUE == profFAS_5G->head.active) && ((ProfInstIsFree((Pointer)profFAS_5G)) == 0))
	{   
	    strncpy(sz[sz_num], profFAS_5G->head.name, UTT_SZONE_LENTH);
	    sz_num++;
	}   
    }   
	...
	mode = 0;
	memset(mac_all, 0, sizeof(mac_all));
	sprintf(mac_all, "iwpriv rai%d set ACLAddEntry=\"", i);
	ProfInstLowHigh(mibType, &max, &min);
	for (index = min; index < max; index++)
	{
	    prof = (ApMacFilterProfile *)ProfGetInstPointByIndex(mibType, index);
	...
	enabled_mac_filter_5g(mode, i, mac_all); 
    }

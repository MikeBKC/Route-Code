...
void printWTPList(struct WTPInfo *wtpList, int nWTPs)
{
	...
	printf("*---------------------------------------------------------------------------------------------------*\n");
	printf("| %5s | %-16s | %-16s | %5s | %5s | %5s\n", "WTPId", "WTPName" ,"Model", "SN", "SWVersion", "HWVersion");
	printf("*---------------------------------------------------------------------------------------------------*\n");
	printf("| %5s | %-30s |\n", "-1", "all");

	if (wtpList != NULL) {
		for (i = 0; i < nWTPs; i++) {
			printf("| %5d | %-16s | %-16s | %5d | %5d | %5d |\n", wtpList[i].wtpId, wtpList[i].name,
				wtpList[i].modelName, wtpList[i].sn, wtpList[i].swVersion, wtpList[i].hwVersion
			);
		}
	}

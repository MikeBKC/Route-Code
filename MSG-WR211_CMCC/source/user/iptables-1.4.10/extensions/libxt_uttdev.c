...
" [!] --is-lan-in			arrived on a bridge device\n");
	...
	printf(
"uttdev match options:\n"
" [!] --uttdev-in inputname[+]	is a packet come from inputname ([+] for wildcard)\n"
...
" --is-salewifi-in              is a packet come from sale wifi\n"
#endif
);
}
              ...
              const void *entry, struct xt_entry_match **match)
{
	...
	switch (c) {
		...
		xtables_parse_interface(argv[optind-1], info->uttindev,
		...
		*flags |= XT_UTTDEV_OP_WANTOLAN;
		...
		case '6':
		info->bitmask |= XT_UTTDEV_OP_WIFISALEIN;

		*flags |= XT_UTTDEV_OP_WIFISALEIN;
		break;
...
static void uttdev_check(unsigned int flags)
{
	...
	if (flags & XT_UTTDEV_OP_WIFISALEIN) {
	    para_cnt++;
	}
...
uttdev_print(const void *ip, const struct xt_entry_match *match, int numeric)
{
	...
	if (info->bitmask & XT_UTTDEV_OP_WANTOLAN)
		printf(" --wan-to-lan ");
	if (info->bitmask & XT_UTTDEV_OP_WIFISALEIN)
		printf(" --is-salewifi-in ");

#endif
	if (info->bitmask & XT_UTTDEV_OP_IN)
		printf("%s --uttdev-in %s",
		(info->invert & XT_UTTDEV_OP_IN) ? " !":"", info->uttindev);
#if 0
	if (info->bitmask & XT_UTTDEV_OP_OUT)
		printf("%s --uttdev-out %s",
		(info->invert & XT_UTTDEV_OP_OUT) ? " !":"", info->uttoutdev);
#endif
	printf(" ");
}

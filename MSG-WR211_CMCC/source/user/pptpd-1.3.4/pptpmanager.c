 ...
 *
 * $Id: pptpmanager.c,v 1.5 2013/11/01 10:47:03 guo.deyuan Exp $
 */
 ...
 * 作者： Jianqing.Sun 
 ********************************************************************/ 
void slot_set_count(int num)
{
    slot_count = num;
    SWORD_SYSLOG(LOG_NOTICE, "%s------%d, slot_count = %d\n", __func__, __LINE__, slot_count);
    return;
}
/********************************************************************
 * 函数： utt_pptp_srv_ip_adjust
 * 功能： 调整ip，若现在要用的ip恰好是上次的固定ip，则调整firstOpen
 * 创建： 2012-10-11
 * 参数： msg        -     收到的消息结构体
 *	  firstOpen  -     这次要用的ip的序号
 * 返回： -1         -     不需要调整firstOpen
 *	  其它       -     firstOpen调整后的序号
 * 输出： 略
 * 作者： Jianqing.Sun 
 ********************************************************************/ 
static int utt_pptp_srv_ip_adjust(utt_msg_pptp_srv *msg, int firstOpen)
{
    int flag = -1;
    struct in_addr addr;
    char tmp[16];
    int i = 0, j = 0;
    struct slot *sloti = NULL;
    struct slot *slotj = NULL;
    if(msg->msg_data.pptp_ass_ip != msg->msg_data.ppp_ass_ip)
    {
	memset(tmp, 0, sizeof(tmp));
	strcpy(tmp, slot_get_remote(0));
	if(inet_aton(tmp, &addr) != 0)
	{
	    SWORD_SYSLOG(LOG_NOTICE, "%s------%d, ntohl(msg->msg_data.pptp_ass_ip) = %x, ntohl(msg->msg_data.ppp_ass_ip) = %x, ntohl(addr.s_addr) = %x\n", __func__, __LINE__, ntohl(msg->msg_data.pptp_ass_ip), ntohl(msg->msg_data.ppp_ass_ip), ntohl(addr.s_addr));
	    i = ntohl(msg->msg_data.pptp_ass_ip) - ntohl(addr.s_addr);
	    j = ntohl(msg->msg_data.ppp_ass_ip) - ntohl(addr.s_addr);
	    if(j == firstOpen)
	    {/*若现在要用的ip恰好是上次的固定ip，则调整firstOpen*/
		flag = i;
	    }
	    SWORD_SYSLOG(LOG_NOTICE, "%s------%d, i = %d, j = %d\n", __func__, __LINE__, i, j);
	    sloti = &slots[i];
	    slotj = &slots[j];
	    slotj->pid = sloti->pid;
	    sloti->pid = 0;
...
#define PPPOE_SERVER_BIND_IP_FILE "/etc/ppp/pptp-bind-ip"
/********************************************************************
 * 函数： isBindIp
 * 功能： 判断要分配的ip是否为固定ip
 * 创建： 2012-10-11
...
static int isBindIp(int i)
{
	...
	{/*直到文件结尾*/
		...
		inet_aton(tmp, &addr);
		addr.s_addr += i << 24;
		SWORD_SYSLOG(LOG_NOTICE, "%s------%d, i = %d, tmp = %s, addr.s_addr = %x, *((unsigned int *)bufIp) = %x\n", __func__, __LINE__, i, tmp, addr.s_addr, *((unsigned int *)bufIp));
		if (addr.s_addr == *((unsigned int *)bufIp)) 
		{/*此ip是固定ip*/
		    ret = 1;
		    break;
		}
 ...
 * 作者： Jianqing.Sun 
 ********************************************************************/ 
int existNonBindIp(void)
{
    int i = 0, isBind = 0, flag = -1;
    for(i = 0; i < slot_count; i++) 
    {
	struct slot *slot = &slots[i];
	SWORD_SYSLOG(LOG_NOTICE, "%s------%d, i = %d, slot->pid = %d\n", __func__, __LINE__, i, slot->pid);
	if (slot->pid == 0) 
	{
	    isBind = isBindIp(i);/*此序号对应ip是否被绑定*/
	    SWORD_SYSLOG(LOG_NOTICE, "%s------%d, slot_count = %d, i = %d, isBind = %d, slot->pid = %d\n", __func__, __LINE__, slot_count, i, isBind, slot->pid);
	    if(isBind == 0)
	    {/*此ip没有被绑定*/
 ...
 * 作者： Jianqing.Sun 
 ********************************************************************/ 
int slot_find_empty(void)
{
    int flag = -1;

    flag = existNonBindIp();/*查找是否存在非固定空闲ip*/
    SWORD_SYSLOG(LOG_NOTICE, "%s------%d, flag = %d\n", __func__, __LINE__, flag);
    if(flag == -1)
    {/*若不存在非固定空闲ip则分配固定ip*/
	flag = slot_find_by_pid(0);
    }
    return flag;
}
#else
...
static void sigchld_responder(int sig)
{
  ...
  while ((child = waitpid(-1, &status, WNOHANG)) > 0) {
	...
	syslog(LOG_DEBUG, "[VPN] %s-%d: find slot %d, set pid 0.",__func__,__LINE__,i);
#endif
	if (pptp_debug) syslog(LOG_DEBUG, "MGR: Reaped child %d", child);
      } else {
...
int pptp_manager(int argc, char **argv)
{
...
#ifdef PPTP_IP_BIND
	/* 获得消息队列 */
	key = ftok(UTT_MSG_PPTP_SRV_FILE, 0);
	if (key != -1) {
	    msgid = msgget(key, 0666 | IPC_CREAT);
	    SWORD_SYSLOG(LOG_NOTICE, "%s------%d, msgid = %d\n", __func__, __LINE__, msgid);
	}
#endif
#if (VPN_CONTROL == FYES)
    /*共享内存初始化*/
	...
	while (1) {
		...
		if (pptp_delegate) {
			...
			SWORD_SYSLOG(LOG_NOTICE, "%s------%d, firstOpen = %d\n", __func__, __LINE__, firstOpen);
#if (VPN_CONTROL == FYES)
			syslog(LOG_DEBUG, "[VPN] %s-%d: find empty slot %d, vpn count %d ",__func__,__LINE__,firstOpen,nvramProfile->vpnProfile.count);
#endif
			if (firstOpen == -1) {
		...
		if (FD_ISSET(hostSocket, &connSet)) {	/* A call came! */
#ifdef PPTP_IP_BIND
		    /* 
		     * 接收来自pppd的消息，判断是否存在pptp server分配IP地址与pppd分配IP地址冲突 
		     * 有冲突，作调整
		     * 接收的是上次ppp拨号时传来的消息
		     */
		    if (msgrcv(msgid, &msg, sizeof(utt_msg_pptp_srv_data), 0, IPC_NOWAIT) != -1) 
		    {
			index = utt_pptp_srv_ip_adjust(&msg, firstOpen);
			if(index != -1)
			{
			    firstOpen = index;
			}
			...
			if (clientSocket == -1) {
				...
				{
					syslog(LOG_NOTICE, "VPN Notice: No free vpn connection session - no more vpn item can connect!");
					goto dos_exit;
				}
#endif
...
#if (VPN_CONTROL == FYES)
						doSystem("vpnHandle minus ");
						syslog(LOG_DEBUG, "[VPN] %s-%d: find empty slot %d, set pid %d ",__func__,__LINE__,firstOpen,ctrl_pid);
#endif
					}
					break;
				}

...
//NOTE: You may need to change this name in your platform according to you ethernet name.
#define IFNAME			"eth2.2"	//TODO: wlan0
...
uint32_t platform_wifi_get_ip(char ip_str[PLATFORM_IP_LEN])
{
    ...
    strncpy(ip_str, inet_ntoa(sin->sin_addr), PLATFORM_IP_LEN);
    close( sock_fd );

    printf("local ip_str = %s, ip_num = 0x%8x\n", ip_str, ntohl(sin->sin_addr.s_addr));

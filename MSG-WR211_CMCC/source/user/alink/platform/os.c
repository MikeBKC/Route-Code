...
uint32_t platform_get_time_ms(void)
{
	struct timeval tv = { 0 };
	uint32_t time_ms;
        static uint32_t time_ms_start = 0;

	gettimeofday(&tv, NULL);

        if(!time_ms_start) {
            time_ms_start = tv.tv_sec * 1000 + tv.tv_usec / 1000;
        }

	time_ms = tv.tv_sec * 1000 + tv.tv_usec / 1000 - time_ms_start;

	return time_ms;
}

uint32_t platform_get_time_sec(void)
{
        ...
        if(error) {
            printf("read sysinfo fail!\n");
            assert(0);
        }
        return (unsigned int)s_info.uptime;
}
...
{
    //nothing need to do in linux platform.
}


char *platform_get_chipid(char chipid[PLATFORM_CID_LEN])
{
    return strncpy(chipid, "12345678", PLATFORM_CID_LEN);
}

char *platform_get_os_version(char os_ver[PLATFORM_OS_VERSION_LEN])
{
    return strncpy(os_ver, "ROUTER_IOT", PLATFORM_OS_VERSION_LEN);
}
...
{
    strncpy(name_str, "UTT", STR_SHORT_LEN);

...
if [ $# != 2 ];then
    echo "Usage: l2tp-up.sh <ppp name> <channel name>"
    exit;
fi
...
chnl_name=`echo $chnl_name |sed "s/l2tp_//g"`
# vpn free session minus 1
vpnHandle minus l2tpclient $chnl_name 
lanIp=`uttcli getbynm l2tpClient $chnl_name remoteLanIp`
lanNm=`uttcli getbynm l2tpClient $chnl_name remoteLanNetmask`
#pptp_client=`nvram_get 2860 PptpClients |sed "s/;/\n/g" |grep "^${chnl_name},"`
echo l2tp_client  >/dev/ttyS0
...
if [ $VPN_PROXY == $FYES ]; then
    iptables -t nat -D POSTROUTING -j pptp_default_nat
    iptables -t nat -A POSTROUTING -j pptp_default_nat
    iptables -t nat -D pptp_default_nat -o $1 -j MASQUERADE
    iptables -t nat -A pptp_default_nat -o $1 -j MASQUERADE
    vpnMode=`uttcli getbynm l2tpClient $chnl_name vpnMode`

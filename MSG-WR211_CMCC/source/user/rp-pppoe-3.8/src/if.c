...
"$Id: if.c,v 1.4 2013/05/27 11:36:52 sun.jianqing Exp $";
...
#ifdef PPPOE_SERVER_LOG
extern void utt_syslog(int pri, const char *fmt, ...);
#else
#define utt_syslog(pri, fmt, args...) 
#endif
...
etherType(PPPoEPacket *packet)
{
	...
	utt_syslog(LOG_ERR, "Invalid ether type 0x%x", type);
    }
    return type;
}
...
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr)
{
    ...
    utt_syslog(LOG_INFO, "Interface=%.16s HWaddr=%02X:%02X:%02X:%02X:%02X:%02X Device=%.32s Buffer size=%d",
	   ifname,
...
receivePacket(int sock, PPPoEPacket *pkt, int *size)
{
	...
	utt_syslog(LOG_ERR, "Truncated bpf packet header: len=%d", bpfSize);
	clearPacketHeader(pkt);		/* resets bpfSize and bpfOffset */
	return 0;
    }
    memcpy(&hdr, bpfBuffer + bpfOffset, sizeof(hdr));
    if (hdr.bh_caplen != hdr.bh_datalen) {
	utt_syslog(LOG_ERR, "Truncated bpf packet: caplen=%d, datalen=%d",
    ...
    if (seglen > bpfSize) {
	utt_syslog(LOG_ERR, "Truncated bpf packet: seglen=%d, bpfSize=%d",
	       seglen, bpfSize);
	clearPacketHeader(pkt);		/* resets bpfSize and bpfOffset */
	return 0;
    }
	...
	*size = data.len;
     ...
     */
    /*
     * 3520G去tag头,8个字节，同样适用1725平台（三层交换机 )
     * by dhyu
     */
    {
	int delLen;
	...
	/* 剥离switch添加的tag,返回值为剥离的tag长度 */
	delLen = sw_frame_header_pull((char *)pkt, &pd, &header);
	if (delLen != 0) {
	    /* 调整长度 */
	    *size -= delLen;
	    /* 将删除头后的数据拷贝到 pkt */
    ...
    int ii;
    FILE *fd1;
    fd1 = fopen("/etc/record.txt", "w+");
    utt_syslog(LOG_INFO, "%s: recv; size = %d", __FUNCTION__, *size);
    if (fd1 != NULL) {
	utt_syslog(LOG_INFO, "%s: ttt sizeof(PPPoEPacket) = %d", __FUNCTION__, sizeof(PPPoEPacket));
	utt_syslog(LOG_INFO, "%s: %x %x %x %x %x %x", __FUNCTION__, ((char *)pkt)[0], ((char *)pkt)[1], ((char *)pkt)[2], ((char *)pkt)[3], ((char *)pkt)[4], ((char *)pkt)[5]);
    for (ii = 0; ii < 23; ii++) {
	fprintf(fd1, "%2x ", ((char *)pkt)[ii]);
	if (((ii+1) % 16) == 0) {
	    fprintf(fd1, "%c", '\n');
	}
    }

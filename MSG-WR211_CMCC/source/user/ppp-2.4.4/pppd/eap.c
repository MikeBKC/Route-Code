...
#define RCSID	"$Id: eap.c,v 1.1.1.1 2011/08/17 11:10:38 brwang Exp $"
...
extern char pptp_name[32];
static char * getWanName()
{
   char *wanName=NULL;
   if(strlen(pptp_name) >0) {
       wanName=pptp_name;
   }else{
       if(strcmp(devnam, "eth2.2")==0){
           wanName="WAN1";
       }else if (strcmp(devnam, "eth2.3")==0){
           wanName="WAN2";
       }else if (strcmp(devnam, "ttyUSB0")==0){
           wanName="3G";
       }else if (strcmp(devnam, "apcli0") == 0){
           wanName="APClient";
       }else {
           wanName=devnam;
       }
...
int len;
{
	...
	error("SYS_CHANGE EAP: %s peer reports authentication failure",getWanName());
	auth_withpeer_fail(esp->es_unit, PPP_EAP);
}

...
static void help(void)
{
	...
	"--l7classified: if conntrack has been classified, the rule return matched. MUST be used with --l7proto unknown\n"
	);
}
...
static const struct option opts[] = {
	...
	{ .name = "l7dir",   .has_arg = 1, .val = 'd' },
        { .name = "l7classified",   .has_arg = 0, .val = 'c' },
	{ .name = NULL }
};
...
static char ** readl7dir(char * dirname)
{
	...
	{
            perror("scandir");
	    xtables_error(OTHER_PROBLEM, "Couldn't open dir %s\n", dirname);
	}
        else 
	{
            	while(n--) 
...
static void parse_layer7_protocol(const char *s, struct xt_layer7_info *info)
{
	...
	else                  dir = "/etc_ro/l7-protocols";  /* dufault path modified by lei.minghuan from /etc/... */
      ...
      int numeric)
{
	...
	/* Êä³öclassifiedÐÅÏ¢ */
	if (((struct xt_layer7_info *)match->data)->classified) {
	    printf("classified ");
	}

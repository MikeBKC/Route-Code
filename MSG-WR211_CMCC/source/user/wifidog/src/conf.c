...
/* $Id: conf.c,v 1.7.18.1 2015/06/23 15:57:34 hou.bo Exp $ */
/** @file conf.c
...
static const struct {
	...
	{ "otherserver",		oOtherAuthServer},
	{ "otherhostname",		oOtherAuthHostname },
	{ "otherserverdata",		oOtherAuthServerData },
...
/*
 * config_init_from_WebAuthProfile
 * 从WebAuthProfile配置中初始化wifidog全局配置
 * 开启wifidog是调用初始化配置
 * guo.deyuan
 * */
void 
config_init_from_WebAuthProfile()
{
    ...
    {
	if(profG->wifiData.noticeTime > 0) {				    /*到期推送时间 单位:分钟*/
	    config.noticeTime = profG->wifiData.noticeTime;
	}
	if((profG->wifiData.noticeUrl) > 0) {				    /*到期通告URL*/
	    strncpy(config.noticeUrl, profG->wifiData.noticeUrl, MAX_URL_LEN);
	}
    }
#endif
...
config_init(void)
{
	...
	wan = getWanIfNamePPP(1);
	if(wan != NULL) {
	config.external_interface = safe_strdup(wan);
	} else {
	config.external_interface = NULL;
	}
	config.gw_id = DEFAULT_GATEWAYID;
	config.gw_interface = safe_strdup(getLanIfName());
	...
	config.Auth_time = DEFAULT_AUTHTIME;
	config.freeAuth_time = DEFAULT_FREEAUTHTIME;
#if (WIFIDOG_USER_SUCCESS_URL == FYES)
	config.successUrl = NULL;
#endif
#if (WIFIDOG_USER_SECRET_KEY == FYES)
	config.secret_key =safe_strdup(SECRET_KEY_VALUE);
#endif
...
static void
parse_other_auth_server(FILE *file, const char *filename, int *linenum)
{
    char *other_host = NULL,
	 *data=NULL,
	 line[MAX_BUF],
	 *p1,
	 *p2;
    int  opcode ;
    t_secondary_auth_server	*sec_server=NULL,*sec_tmp=NULL;
	...
	if (strlen(p1) > 0) {
	    ...
	    *p2 = '\0';
	    ...
	    switch (opcode) {
		case oOtherAuthHostname:
		    other_host = safe_strdup(p2);
		    break;
		case oOtherAuthServerData:
		    data = safe_strdup(p2);
		    break;
    ...
    sec_server = safe_malloc(sizeof(t_secondary_auth_server));
    if(sec_server != NULL) {
	/* Fill in struct */
	memset(sec_server, 0, sizeof(t_secondary_auth_server)); /*< Fill all with NULL */
	sec_server->hostname = other_host;
	sec_server->data = (void*)data;
	if (config.secondary_auth_server == NULL) {
	    config.secondary_auth_server = sec_server;
	} else {
	    for (sec_tmp = config.secondary_auth_server; sec_tmp->next != NULL;
		    sec_tmp = sec_tmp->next);
	    sec_tmp->next = sec_server;
	}
    }
}
/** @internal
			...
			if ((strncmp(s, "#", 1)) != 0) {
				...
				case oAuthServer:
					parse_auth_server(fd, filename,
							&linenum);
					break;
				case oOtherAuthServer:
					parse_other_auth_server(fd, filename,
							&linenum);
					break;
...
#if (WIFIDOG_USER_SECRET_KEY == FYES)
	config_notnull(config.secret_key, "secret_key");
#endif

...
static void phRegRoute(struct hostent *lphost, int total,char *interface)
#else
static void phRegRoute(struct hostent *lphost, int total)
#endif
{
    static int times = 0;
    char buf[255];
	...
	LOG(1) ("ip route flush table 100\n");
#endif
    }
    else if (times == 1)
	times++;
    ...
    if (lphost == NULL || total <= 0)
    {
	LOG(1) ("Error no router for registering\n");
	return;
    }
...
#if (DDNS_ADVANCE == FYES)
	sprintf(buf, "config-Aphddnsroute.sh %s %s", 
		my_inet_ntoa(((struct in_addr *)lphost->h_addr_list[i])->s_addr),interface);
#else
	sprintf(buf, "config-phddnsroute.sh %s",
		my_inet_ntoa(((struct in_addr *)lphost->h_addr_list[i])->s_addr));
#endif
	system(buf);
	LOG(1) ("%s\n", buf);
    }
...
#if (DDNS_ADVANCE == FYES)
BOOL phConnect(int m_hSocket, char * lpszHostAddress, unsigned short nHostPort, int *nAddressIndex, char *szSelectedAddress,char *interface)
#else
...
#if (DDNS_ADVANCE == FYES)
	phRegRoute(lphost, total,interface);	
#else
	phRegRoute(lphost, total);
#endif

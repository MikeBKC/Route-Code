 ...
 *
 * $Id: sock.c,v 1.5 2013/04/03 18:22:56 hou.bo Exp $
 */
 ...
 */
	while (ringqLen(rq) > 0) {
		...
		/*750次尝试后 不再发送。这里不直接修改内核的超时时间
		  /proc/sys/net/ipv4/tcp_retries2 是因为可能影响到其他 如vpn*/
		if(sp->uttSendTimeOut > 750) return -1;
		bytesWritten = socketDoOutput(sp, (char*) rq->servp, len, &errCode);
		if (bytesWritten < 0) {
			if (errCode == EINTR) {
#if (DEBUG_GOAHEAD == FYES)
			    goahead_debug_print("%s:%s-%d errCode:%d\n", __FILE__, __func__, __LINE__, errCode);
#endif
			    continue;
			} else if (errCode == EWOULDBLOCK || errCode == EAGAIN) {
			    sp->uttSendTimeOut++;
			    /*睡下 防止不停的send。导致系统负载过高*/
			    usleep(100000);
#if (defined (WIN) || defined (CE))
				if (sp->flags & SOCKET_BLOCK) {
					int		errCode;

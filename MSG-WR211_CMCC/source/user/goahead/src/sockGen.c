 ...
 *
 * $Id: sockGen.c,v 1.7 2013/04/02 17:40:16 sun.jianqing Exp $
 */
...
int socketOpenConnection(char *host, int port, socketAccept_t accept, int flags)
{
	...
	struct timeval uttSkTimeout = {60,0}; /*设置超时时间*/ 
	int uttSkSndSize = 32768;/*32k*/

#if((WEB_BIND_ANY_SRC == FNO) && (MULTI_LAN == FNO))
	char				ipaddr[18];
#endif
#ifdef WF_USE_IPV6
	int					gaierr;
 ...
 */
	...
	if (host == NULL) {
#if((WEB_BIND_ANY_SRC == FYES) || (MULTI_LAN == FYES))
	    sockaddr.sin_addr.s_addr = INADDR_ANY;
#else
	    getIfIp(getLanIfName(),ipaddr);
 ...
 */
		...
		/*mantis 0009804 tcp非正常断开导致goahead挂起。*/
		/*发送超时*/
		setsockopt(sp->sock, SOL_SOCKET, SO_SNDTIMEO, (char *)&uttSkTimeout,sizeof(uttSkTimeout));
		/*发送缓冲区大小*/
		setsockopt(sp->sock, SOL_SOCKET,SO_SNDBUF, (char *)&uttSkSndSize,sizeof(uttSkSndSize));
#if 0
		int skLen = sizeof(int);
		getsockopt(sp->sock, SOL_SOCKET,SO_SNDBUF, (char *)&uttSkSndSize, &skLen);
		printf("%s %d %d \n", __func__, __LINE__, uttSkSndSize);
#endif
#ifdef WF_USE_IPV6
...
#if 0
			printf("socketReady call socketSelect\n");
#endif
			socketSelect(sid, 0);
			return 1;
		}
...
#if 0
			printf("socketSelect have event occur\n");
#endif
			index = sp->sock / (NBBY * sizeof(fd_mask));
			bit = 1 << (sp->sock % (NBBY * sizeof(fd_mask)));

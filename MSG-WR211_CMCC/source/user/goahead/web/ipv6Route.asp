...
var strSecondMenuFocus="<%write(ipv6RouteConfig);%>";
<% outputIpv6RouteConfigConfig();%>
<%getWanIfCount();%>
...
for(i=0;i<totalrecs;i++){
	...
	RouteEnablesw[i]=(RouteEnablews[i]=="1"?"<%write(com_startUse);%>":"<%write(com_forbidden);%>");
}
//end
...
function EditConfig(val,i){
	if(val==0){ 
		Ipv6Route_List.id.value="";
		if(eval(max_totalrecs-totalrecs)<1)	{
			alert("<%write(com_editAlert);%>");
			return;
		}
	}
	else Ipv6Route_List.id.value=RouteNames[i];
	Ipv6Route_List.action="ipv6Route_edit.asp";
	Ipv6Route_List.submit();
}
function DeleteConfig(index){
	if(confirm("<%write(com_deleteTip);%>")){
		Ipv6Route_List.delstr.value=RouteNames[index];
		Ipv6Route_List.action="/goform/ConfigIpv6RouteConfigDel";
	    Ipv6Route_List.submit();
	}
}
function delAll(){
	if(confirm("<%write(com_deleteAllTip);%>")){
		Ipv6Route_List.action="/goform/ConfigIpv6RouteConfigDelAll";
		Ipv6Route_List.submit();
	}
}
-->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <td width="15"></td>
  <!--////内容-->
  <td><script language="javascript">showSecondTitle(_("<%write(ipv6_RouteList);%>"));</script>
  <table width="100%" border="0" height="450" cellspacing="0" cellpadding="0" align="center" class="table_bg">
   <script language="javascript">showerror(errorstr,80);</script>
  <tr valign="top">
    <td ><form name="Ipv6Route_List" method="post" action="/goform/ConfigIpv6RouteConfigDel">
...
<tr>
   ...
   <table id=grouplist style="display" width="80%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td colspan="4" height="20">&nbsp;</td></tr>
		...
		<script language="javascript">document.write(_("<%write(com_select);%>"));</script></div></td>
			 ...
			 <td width="45" valign="top" align="right"><input type="submit" value="<%write(com_delete);%>" class="input-button" onClick="return(genData(Ipv6Route_List,0,frame12,RouteNames,'<%write(rou_routeInfo);%>'))" id="del"></td>
			  </tr>
		      </table></td>

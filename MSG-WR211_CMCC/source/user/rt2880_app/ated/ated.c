...
#if (WIRELESS == FYES)
	ProfInit();
	prof = (WirelessProfile *)ProfGetInstPointByIndex(MIB_PROF_WIRELESS, 0);

	opmode = prof->mBaseCf.OperationMode;
	if (opmode == 1) {
		SystemProfile *profSys = (SystemProfile *)ProfGetInstPointByIndex(MIB_PROF_SYSTEM, 0);
#if (UTT_BRIDEG_MODE_FLAG == FYES) 
		if (profSys && profSys->brideg_mode_flag == 1) {
			memcpy(ifr.ifr_name, "br1", 7);
		} else { 
#endif
#ifdef CONFIG_UTT_LAN_IFNAME
	    memcpy(ifr.ifr_name, CONFIG_UTT_LAN_IFNAME, 7);
#else

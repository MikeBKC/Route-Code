#!/bin/sh
#
# $Id: config-mdhcp.sh,v 1.3 2014/01/17 11:50:17 chen.kun Exp $
#
...
fname="/etc/udhcpd_vlan.conf"
fbak="/etc/udhcpd_vlan.conf_bak"
...
#        rm -f $pidfile
#    fi
    if [ -e "/etc/udhcpd.$3.tmp" ]; then
        cat /etc/udhcpd.$3.tmp >> $fname
    fi
	;;
#    touch $leases > null 2>&1
#    echo "lease_file $leases" >> $fname;;
#    rm -f /etc/udhcpd.$3.tmp
#    udhcpd $fname;;
#    reset_all_phys $2;;
  "-k")
    if [ -e ${pidfile} ]; then

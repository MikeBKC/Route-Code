...
var WrlessEnables="";var ssids="";var broadcastEnables=0;var pskPsswds="";
<%aspoutDynConfig();%>
<% aspoutWlanWizard_Sui(); %>
	//关闭
		//开关
$(function(){
	parent.art.dialog.list["func_box"].title("<%write(wwsui_wlanset);%>");
	var enabledWifi =WrlessEnables;
	var switch_b = $(".on-off").children("button");
	initEnabledButton(switch_b,enabledWifi);
	$("#wifiname").val(ssids);
	$("#wifipwd").val(pskPsswds);
	$("#wifipwdis").val(pskPsswds);
...
function formsave(){
    ...
    f.action="/goform/formWlanWizard_Sui";
    f.submit();
    /*
    $.ajax({
	type:"post",
...
function jump()
{
        window.parent.open_windows(null,"guide_wifi_5g",null);
}
</script>
...
<div class="warp">
    ...
    <br>
<div class="tabs_link"><span id="5gjump" name="5gjump" style="display:none" onClick="jump();"><%write(wwsui_skip24g);%> >></span>&nbsp;&nbsp;&nbsp;&nbsp;<span onClick="window.parent.art.dialog.list['func_box'].close();"><%write(com_exitset);%> >></span></div> 
    <br>
     <div class="tabs_button"><input type="button" class="btn" onClick="window.parent.open_windows(null,'internet',null);" value="<%write(com_previous);%>"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="btn"  value="<%write(com_save);%>" onClick="save();"/></div>
     </form>
     <br>
    </div>
</div>

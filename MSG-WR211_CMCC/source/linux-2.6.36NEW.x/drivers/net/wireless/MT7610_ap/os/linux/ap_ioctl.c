...
{ RTPRIV_IOCTL_APCLI_STATUS,
  IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | 1024 ,
  "get_apc_st"},
#endif
#ifdef CONFIG_WIFI_CHANNEL_SCORE
{ RTPRIV_IOCTL_WIFI_SCORE,
  IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | 1024 ,
  "get_wifi_score"},
#endif
...
/*				struct iw_range range; */
		...
		case SIOCGIWPRIV:
			...
			break;
#ifdef CONFIG_RT_IOCTL_GET_SUPPORT
               case RTPRIV_IOCTL_GET:
                        {
                                if( access_ok(VERIFY_READ, wrqin->u.data.pointer, wrqin->u.data.length) == TRUE)
                                        Status = RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GET, 0, NULL, 0);
                         }
                         break;
#endif
...
/* end of modification */
		...
		case RTPRIV_IOCTL_SSID_STA_INFO:
			RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_GET_SSID_STA_INFO, 0, NULL, 0);
			break;
#endif
...
#endif /* DBG */
#ifdef CONFIG_MT7610_APCLI_CONN_STATUS
		case RTPRIV_IOCTL_APCLI_STATUS:
			if (strstr(wrqin->ifr_ifrn.ifrn_name,"apclii") == NULL) {
			    Status = RTMP_IO_EINVAL;
			} else {
			    RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_APC_STATUS, 0, NULL, 0);
			}
			break;
#endif
#ifdef CONFIG_WIFI_CHANNEL_SCORE
                case RTPRIV_IOCTL_WIFI_SCORE:
                        if (strstr(wrqin->ifr_ifrn.ifrn_name,"rai0") == NULL) {
                            Status = RTMP_IO_EINVAL;
                        } else {
                            RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_WIFI_CHANNEL_SCORE, 0, NULL, 0); 
                        }   
                        break;
#endif
		default:

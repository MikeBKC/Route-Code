...
#ifdef RT_IOCTL_CONFIG_GET
#define UTT_CONFIG_LEN 4096
/* 
    ==========================================================================
    Description:
        Get BYPASS MODE
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	PSTRING		msg;
	INT		    Status=TRUE;
    INT         max_len = UTT_CONFIG_LEN;
    POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
#if defined (RT_IOCTL_ASSOC_THRESHOLD_GET) || defined (CONFIG_UTT_AP_RATE_LIMITING)
    UCHAR           apidx = pObj->ioctl_if;
#endif
#ifdef RT_IOCTL_ASSOC_THRESHOLD_GET
    CHAR AssocReqRssiThres;
    CHAR KickStaRssiLow;
#endif
#ifdef CONFIG_UTT_AP_RATE_LIMITING
    UttLimitConf *limit=NULL;
    UINT down, up;
#endif
     ULONG beacon, val;

	os_alloc_mem(NULL, (PUCHAR *)&msg, max_len);

	memset(msg, 0 , max_len);
	sprintf(msg,"%s","\n\n");

#ifdef RT_IOCTL_ASSOC_THRESHOLD_GET
    AssocReqRssiThres = pAd->ApCfg.MBSSID[apidx].AssocReqRssiThreshold;
    sprintf(msg+strlen(msg),"%-20s%-d\n","AssocReqRssiThres:",AssocReqRssiThres);
    KickStaRssiLow = pAd->ApCfg.MBSSID[apidx].RssiLowForStaKickOut;
    sprintf(msg+strlen(msg),"%-20s%-d\n","KickStaRssiLow:",KickStaRssiLow);
#endif

#ifdef RT_IOCTL_BYPASS_GET
    USHORT BYPASS;
	BYPASS = pAd->CommonCfg.BYPASS;
	sprintf(msg+strlen(msg),"%-20s%-u\n","BYPASS:",BYPASS);
#endif

#ifdef RT_IOCTL_CHANNEL_GET
    ULONG Channel;
    RTMP_DRIVER_CHANNEL_GET(pAd, &Channel);
	sprintf(msg+strlen(msg),"%-20s%-u\n","Channel:",Channel);
#endif

#ifdef RT_IOCTL_UTTMAXSTA_GET
    UCHAR uttMaxStaNum;
	uttMaxStaNum = pAd->CommonCfg.uttMaxStaNum;
	sprintf(msg+strlen(msg),"%-20s%-u\n","uttMaxStaNum:",uttMaxStaNum);
#endif

#ifdef RT_IOCTL_TXPOWER_GET
    ULONG TxPower;
    TxPower = pAd->CommonCfg.TxPowerPercentage;
	sprintf(msg+strlen(msg),"%-20s%-u\n","TxPower:",TxPower);
#endif

    beacon = pAd->CommonCfg.BeaconPeriod;                                                                                      
    sprintf(msg+strlen(msg),"%-20s%-lu\n","BeaconPeriod:", beacon);
  
    val = pAd->RalinkCounters.OneSecReceivedByteCount;                                                                         
    sprintf(msg+strlen(msg),"%-20s%-lu\n","RxRate:", val);
    
    val = pAd->RalinkCounters.OneSecTransmittedByteCount;                                                                      
    sprintf(msg+strlen(msg),"%-20s%-lu\n","TxRate:", val);
#ifdef CONFIG_UTT_AP_RATE_LIMITING
	sprintf(msg+strlen(msg),"%-20s\n","Rate-limiting:");
	sprintf(msg+strlen(msg),"  %5s%8s%18s%9s%16s%9s\n","ApIdx", "Policy", "Download(kbit/s)", "Burst", "Upload(kbit/s)", "Burst");
	for(apidx=0;apidx<pAd->ApCfg.BssidNum; apidx++)
	{
	    limit = &(pAd->ApCfg.MBSSID[apidx].limitConf);
	    sprintf(msg+strlen(msg),"%7d", apidx);
	    if(limit->limitPolicy==UTT_BANDWIDTH_DISABLED){
	        sprintf(msg+strlen(msg),"%8s", "0");
	    }
	    else if(limit->limitPolicy==UTT_BANDWIDTH_SHARED){
	        sprintf(msg+strlen(msg),"%8s", "share");
	    }
	    else {
	        sprintf(msg+strlen(msg),"%8s", "alone");
	    }
	    down = (HZ*(limit->dRate))>>7;
	    up = (HZ*(limit->uRate))>>7;
	    sprintf(msg+strlen(msg),"%18u%9u%16u%9u\n", down, (limit->dmaxCredit)>>7, 
		    up, (limit->umaxCredit)>>7);
	}
#endif
    pIoctlCmdStr->u.data.length = strlen(msg);
    Status = !copy_to_user(pIoctlCmdStr->u.data.pointer, msg, pIoctlCmdStr->u.data.length);

    DBGPRINT(RT_DEBUG_TRACE, ("Get_Config_Proc - pIoctlCmdStr->u.data.length = %d\n", pIoctlCmdStr->u.data.length));
...
/* 
    ...
    ==========================================================================
*/
INT	Set_RateLimiting_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
        UINT policy=0, dRate=0, uRate=0;
        char *data=NULL;
	UCHAR idx;
	UttLimitConf     *conf=NULL;

	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	data = rstrtok(arg,":");
	if(data!=NULL) {
	    idx = pObj->ioctl_if;
	    conf = &(pAd->ApCfg.MBSSID[idx].limitConf);
	    policy = simple_strtol(data, 0, 10);
	    if(policy != 0)
	    {
		data = rstrtok(NULL,":");
		if(data!=NULL) { /*µ¥Î»Îªkbit/s*/
		    dRate = simple_strtol(data, 0, 10);
		    data = rstrtok(NULL,":");
		    if(data!=NULL) {
			uRate = simple_strtol(data, 0, 10);
			conf->dRate = (dRate<<7)/HZ; /*dRate(kbit)*1024/8 to Bps*/
			conf->uRate = (uRate<<7)/HZ;
			conf->dmaxCredit = (dRate<<7)>>2;
			conf->umaxCredit = (uRate<<7)>>2;
		    }
		}
	    }
	    if(policy>UTT_BANDWIDTH_ALONE) {
		policy = UTT_BANDWIDTH_ALONE;
	    }
	    conf->limitPolicy = policy;
	}
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	pIoctlCmdStr->u.data.length = strlen(msg);
	Status = !copy_to_user(pIoctlCmdStr->u.data.pointer, msg, pIoctlCmdStr->u.data.length);
    *(pIoctlCmdStr->u.data.pointer + pIoctlCmdStr->u.data.length) = '\0';

	DBGPRINT(RT_DEBUG_TRACE, ("Get_BYPASS_Proc - pIoctlCmdStr->u.data.length = %d\n", pIoctlCmdStr->u.data.length));
	...
	IN	PSTRING			arg)
{
	...
	pAd->CommonCfg.BYPASS = simple_strtol(arg, 0, 10);
	
	DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d) Set_BYPASS_Proc::(VLAN_VID=%d)\n", pObj->ioctl_if, pAd->CommonCfg.BYPASS));
...
/* 
    ==========================================================================
    Description:
        Get uttMaxStaNum MODE
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    ==========================================================================
*/
INT	Get_UTTMAXSTANUM_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	STRING		msg[64];
...
/* 
    ==========================================================================
    Description:
        Set uttMaxStaNum MODE
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    ==========================================================================
*/
INT	Set_UTTMAXSTANUM_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	
	pAd->CommonCfg.uttMaxStaNum = simple_strtol(arg, 0, 10);
	printk("uttMaxStaNum = %d\n", pAd->CommonCfg.uttMaxStaNum);
	
	DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d) Set_UTTMAXSTANUM_Proc::(VLAN_VID=%d)\n", pObj->ioctl_if, pAd->CommonCfg.uttMaxStaNum));
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    *(pIoctlCmdStr->u.data.pointer + pIoctlCmdStr->u.data.length) = '\0';
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Get_Channel_Proc - pIoctlCmdStr->u.data.length = %d\n", pIoctlCmdStr->u.data.length));
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    *(pIoctlCmdStr->u.data.pointer + pIoctlCmdStr->u.data.length) = '\0';
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Get_TxPower_Proc - pIoctlCmdStr->u.data.length = %d\n", pIoctlCmdStr->u.data.length));
...
#ifdef CONFIG_RT_IOCTL_SSID_STA_INFO_GET
VOID RTMPIoctlGetSsidStaInfo(
	IN PRTMP_ADAPTER pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    INT i;
    USHORT apIdx;
    MAC_TABLE_ENTRY *pEntry;
    RT_SSID_STA_INFO_TABLE *pSsidTab = NULL;
    RT_SSID_STA_INFO_ENTRY *pDst = NULL;

	/* allocate memory */
    os_alloc_mem(NULL, (UCHAR **)&pSsidTab, sizeof(RT_SSID_STA_INFO_TABLE));
    if (pSsidTab == NULL)
    {
        DBGPRINT(RT_DEBUG_ERROR, ("%s: Allocate memory fail!!!\n", __FUNCTION__));
        return;
    }
    NdisZeroMemory(pSsidTab, sizeof(RT_SSID_STA_INFO_TABLE));
	...
	if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
	{
            apIdx = pAd->MacTab.Content[i].apidx;
            if ( (apIdx < pAd->ApCfg.BssidNum) && (pSsidTab->Num<MAX_NUMBER_OF_MAC)){
		pDst = &(pSsidTab->Entry[pSsidTab->Num]);
		getRate(pEntry->HTPhyMode, &(pDst->Rate));
	        COPY_MAC_ADDR(pDst->StaMac, &pAd->MacTab.Content[i].Addr);
		pDst->Rssi0 += pEntry->RssiSample.AvgRssi0;
		pDst->Rssi1 += pEntry->RssiSample.AvgRssi1;
		pDst->Rssi2 += pEntry->RssiSample.AvgRssi2;
		pDst->ConnectedTime = pEntry->StaConnectTime;
		pDst->TxStat.word = pEntry->HTPhyMode.word;
		pDst->TxBytes = pEntry->TxBytes;
		pDst->RxBytes = pEntry->RxBytes;
		pDst->RxPackets = pEntry->RxPackets.u.LowPart;
		pDst->TxPackets = pEntry->TxPackets.u.LowPart;
		pDst->TxDataRate = pEntry->TxDataRate;
		pDst->RxDataRate = pEntry->RxDataRate;
                pDst->ApIdx = pEntry->apidx;
...
#if 0
	pSsidTab->Num = pAd->ApCfg.BssidNum;

    for(i=0;i<pAd->ApCfg.BssidNum;i++){
        memcpy(pSsidTab->Entry[i].Bssid, pAd->ApCfg.MBSSID[i].Bssid, MAC_ADDR_LEN);
        memcpy(pSsidTab->Entry[i].Ssid, pAd->ApCfg.MBSSID[i].Ssid, MAX_LEN_OF_SSID);
        pSsidTab->Entry[i].StaNum = 0u;
    }
    ...
    for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
	{
		if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i]) && (pAd->MacTab.Content[i].Sst == SST_ASSOC))
		{
	        ...
	        if ( apIdx < pAd->ApCfg.BssidNum ){
		        COPY_MAC_ADDR(pSsidTab->Entry[apIdx].StaInfo[pSsidTab->Entry[apIdx].StaNum].StaMac, &pAd->MacTab.Content[i].Addr);
                 pSsidTab->Entry[apIdx].StaNum += 1;
             }
 		}
 	}
#endif
	wrq->u.data.length = sizeof(RT_SSID_STA_INFO_TABLE);
	if (copy_to_user(wrq->u.data.pointer, pSsidTab, wrq->u.data.length))
	{
		DBGPRINT(RT_DEBUG_TRACE, ("%s: copy_to_user() fail\n", __FUNCTION__));
	}

	if (pSsidTab!= NULL)
		os_free_mem(NULL, pSsidTab);
}
#endif

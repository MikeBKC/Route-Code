	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	if((pAd->ApCfg.ApCliTab[index].CtrlCurrState == APCLI_CTRL_CONNECTED)
		...
		if ( IS_ENTRY_APCLI(pEntry)
			...
			&& (pEntry->PortSecured == WPA_802_1X_PORT_SECURED))
			{
				if ((pAd->ApCfg.ApCliTab[index].remoteAuthIsWep == 0) && pAd->ApCfg.ApCliTab[index].wdev.WepStatus == Ndis802_11WEPDisabled) {/*两端均无wep加密*/
				    bConnect = TRUE;
				    break;
				}
				if ((pAd->ApCfg.ApCliTab[index].remoteAuthIsWep == 0) && pAd->ApCfg.ApCliTab[index].wdev.WepStatus == Ndis802_11WEPEnabled) {
				    bConnect = FALSE;
				    break;
				}
	    ...
	    if (bConnect == TRUE){
		sprintf(msg+strlen(msg), "apclii%d=%d\n", index,1);/*已连接*/
	    } else {
		sprintf(msg+strlen(msg), "apclii%d=%d\n", index,2);/*连接中*/
	    }
	} else {

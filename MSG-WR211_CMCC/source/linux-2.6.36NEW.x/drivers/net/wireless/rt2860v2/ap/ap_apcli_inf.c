...
#ifdef UTT_ADDED
#if 1
static struct net_device_stats *RT28xx_get_ether_stats_ff(
...
#else
                //KickTxCount
		pStats->rx_packets = (pAd)->ApCfg.ApCliTab[0].RxPacket;
		pStats->tx_packets = (pAd)->ApCfg.ApCliTab[0].TxPacket;
#endif

		pStats->rx_bytes = (pAd)->ApCfg.ApCliTab[0].RxByte;
		pStats->tx_bytes = (pAd)->ApCfg.ApCliTab[0].TxByte;
...
struct net_device_stats *RT28xx_get_ether_stats_ff(
    IN  struct net_device *net_dev)
{
	RTMP_ADAPTER *pAd = NULL;
	struct net_device_stats *pStats;
...
#else
		pStats->rx_packets = pAd->RalinkCounters.RxCount;
		pStats->tx_packets = pAd->RalinkCounters.TxCount;
#endif
...
#ifdef UTT_ADDED
	netDevOpHook.get_stats = RT28xx_get_ether_stats_ff;
#endif

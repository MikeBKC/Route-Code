...
#ifdef CONFIG_RT2860V2_APCLI_CONN_STATUS
		pApCliEntry->wepAuthIsPass = TRUE;
#endif
	}
#endif /* MAC_APCLI_SUPPORT */
	pRxD->MyBss = 1;				
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
    ...
    sprintf(msg, "\n");
    for(index = 0; index < MAX_APCLI_NUM; index++) {
	if((pAd->ApCfg.ApCliTab[index].CtrlCurrState == APCLI_CTRL_CONNECTED)
		&& (pAd->ApCfg.ApCliTab[index].SsidLen != 0))	{
		...
		if ( IS_ENTRY_APCLI(pEntry)
			...
			{
				if ((pAd->ApCfg.ApCliTab[index].remoteAuthIsWep == 0) && pAd->ApCfg.ApCliTab[index].WepStatus == Ndis802_11WEPDisabled) {/*两端均无wep加密*/
				    bConnect = TRUE;
				    break;
				}
				if ((pAd->ApCfg.ApCliTab[index].remoteAuthIsWep == 0) && pAd->ApCfg.ApCliTab[index].WepStatus == Ndis802_11WEPEnabled) {
				    bConnect = FALSE;
				    break;
				}
				if ((pAd->ApCfg.ApCliTab[index].remoteAuthIsWep == 1)) {
				    if (pAd->ApCfg.ApCliTab[index].wepAuthIsPass) {
					bConnect=TRUE;
				    } else {
					bConnect=FALSE;
				    }
...
#ifdef UTT_ADDED
                        //added by brwang
                        pApCli[apcliIndex].TxPacket++;
                        pApCli[apcliIndex].TxByte += (RTPKT_TO_OSPKT(skb_p))->len ;
#endif
			/* transmit the packet */
			return Func(skb_p);
		}

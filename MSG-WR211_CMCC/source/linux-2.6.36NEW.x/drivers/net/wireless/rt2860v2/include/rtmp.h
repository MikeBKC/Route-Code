...
#define UTT_BANDWIDTH_DISABLED 0
#define UTT_BANDWIDTH_SHARED   1
#define UTT_BANDWIDTH_ALONE    2
/**
 *
 *精细化限速内核配置结构
 */
typedef struct _UttLimitRunStat{
        unsigned int                   ucredit;
        unsigned int                   dcredit;
        unsigned int                   ulastjiffies;
        unsigned int                   dlastjiffies;
}UttLimitRunStat;

typedef struct _UttLimitConf{
//   struct list_head list;
//    rwlock_t lock;  
    unsigned int uRate; /*上传速率*/
    unsigned int dRate; /*下载速率*/

    unsigned int umaxCredit; /*上传速率*/
    unsigned int dmaxCredit; /*下载速率*/

    unsigned int limitPolicy;/*共享或独立*/
...
#if 0
    bool isOnTimeHook;/*表示该配置是否挂载到了时间生效钩子*/
    bool timeEnable;/*表示该配置时间是否生效*/
#endif
}UttLimitConf;
#endif
...
/*==================================================== */
	...
	BOOLEAN fPsPollEnable;
	UCHAR fPsPollTimerRunning; 	 /* check PS-POLL timer running */
	RALINK_TIMER_STRUCT fPsPollTimer;	 /* A timer which check PS-POLL  flag */
#endif
#ifdef CONFIG_UTT_AP_RATE_LIMITING
	UttLimitRunStat  runStat;
#endif
...
#define MAX_EXCEPTION_NUM  20
#define MAX_REFUSE_TIMES   2

typedef struct _utt_exception_sta {
	UCHAR			HdrAddr[MAC_ADDR_LEN];
	ULONG           now;
	UINT            refuse_times;
} utt_exception_sta;

typedef struct _utt_exception_group {
	utt_exception_sta utt_except_sta[MAX_EXCEPTION_NUM];
	NDIS_SPIN_LOCK  utt_ecpt_lock;
} utt_exception_group;

extern utt_exception_group utt_except_group;
#endif
#endif  /* __RTMP_H__ */

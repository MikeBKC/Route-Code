...
#ifdef CONFIG_MAX_WLAN_CLIENT_NUM
#define MAX_NUMBER_OF_MAC			(CONFIG_MAX_WLAN_CLIENT_NUM+1)/* if MAX_MBSSID_NUM is 8, this value can't be larger than 211 */
#else
...
typedef struct _RT_SSID_STA_INFO_ENTRY {
    ...
    USHORT StaNum;
    RT_STA_INFO StaInfo[MAX_STA_NUM];
} RT_SSID_STA_INFO_ENTRY;
typedef struct _RT_SSID_STA_INFO_TABLE {
    ULONG Num;
    RT_SSID_STA_INFO_ENTRY Entry[MAX_NUMBER_OF_SSID];
} RT_SSID_STA_INFO_TABLE;
#endif

typedef struct _RT_SSID_STA_INFO_ENTRY {
    ULONG                   Rate;   /*802.11 phy rate, 无线接入的实际速率*/
    UINT32                  ConnectedTime;
    UINT32                  TxBytes;
    UINT32                  TxDataRate;
    UINT32                  RxBytes; 
...
typedef struct _RT_SSID_STA_INFO_TABLE {
    UINT32                  Num;
    RT_SSID_STA_INFO_ENTRY  Entry[MAX_NUMBER_OF_MAC];
} RT_SSID_STA_INFO_TABLE;
#endif
#ifdef DOT11_N_SUPPORT
#ifdef TXBF_SUPPORT

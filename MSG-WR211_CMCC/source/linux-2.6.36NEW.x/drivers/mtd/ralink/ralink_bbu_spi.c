...
#define LOCK_UNID_OFFSET 8
int raspi_flashid_utt(char *buf,unsigned int len,unsigned int flag)
{
    if(flag == 0)
    {
	bbu_spic_trans(OPCODE_RDID, 0, buf, 1, len, SPIC_READ_BYTES);
    }
    else
    {
	bbu_mb_spic_trans(OPCODE_UNIQUEID, 0, buf, 1, len, SPIC_READ_BYTES);
    }
    return 0; 
}

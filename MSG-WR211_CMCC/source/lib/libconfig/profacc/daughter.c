...
#endif
#if (RAISECOM_CMCC == FYES)
        sysProfile->brideg_mode_flag = 1;
#else
        sysProfile->brideg_mode_flag = 0;
#endif
...
//#if ((UTT_NV_1800N == FYES) || (UTT_NV_510V2 == FYES))
...
#elif(IP_3_254 == FYES)
		interfaceProfile->ether.sta.ip = htonl(0xc0a803fe);
#else
...
#elif (IP_3_254 == FYES)
                dhcpSProfile->ipStart = htonl(0xc0a80364);
                dhcpSProfile->ipEnd = htonl(0xc0a803c8);
#else
                dhcpSProfile->ipStart = htonl(0xc0a80164);
                dhcpSProfile->ipEnd = htonl(0xc0a801c8);
#endif
                dhcpSProfile->netmask = htonl(0xffffff00);
#if (UTT_NV_1800N != FYES && IP_3_254 == FYES)
		dhcpSProfile->pDns = htonl(0xc0a803fe);;
                dhcpSProfile->gateway = htonl(0xc0a803fe);
#else
	...
	case MIB_PROF_WIRELESS:
...
#else
            sprintf(wirelessProfile->mBaseCf.SSID1,"ChinaNet-2.4G-%02X%02X", newmac[4],newmac[5]);
#endif
#endif
...
#if (MSG_WR201 == FYES || RAISECOM_211 == FYES)
		...
		if (instindex == 1) {
...
#if (WRITE_NEWMAC == FYES)
#if (RAISECOM_CMCC == FYES)
            sprintf(wirelessProfile->mBaseCf.SSID1,"CMCC-5G-%02X%02X", newmac[4],newmac[5]);
#else
            sprintf(wirelessProfile->mBaseCf.SSID1,"ChinaNet-5G-%02X%02X", newmac[4],newmac[5]);
#endif
#endif
		    wirelessProfile->head.active = TRUE;
		    wirelessProfile->mBaseCf.apWorkMode = 1;
            ...
            wirelessProfile->mBaseCf.WirelessMode = 14;
            wirelessProfile->mBaseCf.VChanWidth = 1;
#else
...
#endif
	    strncpy(shortCutMenuProfile->appName[3],"learnConfig",50);
#if (APP_ELINKC == FYES)

/*
 * $Id: mibinpppoe.h,v 1.1.1.1 2011/08/17 11:11:56 brwang Exp $
 *
...
typedef struct pppoeOptions {
    ...
    char                               serverName[ MAX_HOST_NAME_LENGTH + 1 ];
    char                               serviceName[ MAX_HOST_NAME_LENGTH + 1 ];
#if (PPPOE_SRV == FYES)
#define					BIND_NUM	4
    Boolean				auto_bind_mac;
    Byte			        bind_mac[BIND_NUM][ETHER_ADDR_LEN];
    Uint16				acc_session;
    IpAddress				clientIp;
#if (PPPOE_ACCOUNT == FYES)
    PPPOE_ACC_MODE			acc_mode;
    Uint32				acc_tmstart;
    Uint32				acc_tmstop;
    Uint32				acc_hours;
    Uint32				save_acc_hours;
    Uint32				save_rx_mbytes;
    Uint32				save_tx_mbytes;
    Uint32				acc_rx_mbytes;
    Uint32				acc_tx_mbytes;
#endif
#endif
}
PppoeOptions;

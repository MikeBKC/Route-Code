...
char *utHex2str(char *in, int iLen, char *out)
{
    int i;

    for (i=0; i<iLen; i++)
    {
	sprintf(out+i*2, "%.2X", in[i]&0xFF);
    }

    return out;
}
char *utstr2Hex(char *in, int iLen, char *out)
{
    int i=0;
    char tStr[3];            //临时字串     
    char hexStr[3];                 //转变后字符 

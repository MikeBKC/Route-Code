...
void md5_mac(UCHAR *key, ULONG key_len, UCHAR *data, ULONG data_len, UCHAR *mac)
{
	...
	HMD5Init(&context);
	HMD5Update(&context,	key, key_len);
	HMD5Update(&context,	data, data_len);
	HMD5Update(&context,	key, key_len);
	HMD5Final(mac, &context);
}
...
void hmac_md5(UCHAR *key, ULONG key_len, UCHAR *data, ULONG data_len, UCHAR *mac)
{
	...
	if (key_len	> 64) {
		...
		HMD5Init(&ttcontext);
		HMD5Update(&ttcontext, key, key_len);
		HMD5Final(tk, &ttcontext);
	...
	/* perform inner MD5 */
	HMD5Init(&context);					 /*	init context for 1st pass */
	HMD5Update(&context,	k_ipad,	64);	 /*	start with inner pad */
	HMD5Update(&context,	data, data_len); /*	then text of datagram */
	HMD5Final(mac, &context);			 /*	finish up 1st pass */

	/* perform outer MD5 */
	HMD5Init(&context);					 /*	init context for 2nd pass */
	HMD5Update(&context,	k_opad,	64);	 /*	start with outer pad */
	HMD5Update(&context,	mac, 16);		 /*	then results of	1st	hash */
	HMD5Final(mac, &context);			 /*	finish up 2nd pass */
}
//#define __BIG_ENDIAN
#ifdef __BIG_ENDIAN
...
#define	MD5Step(f, w, x, y, z, data, t, s) ( w	+= f(x,	y, z) +	data + t, w = (CYCLIC_LEFT_SHIFT(w, s)) & 0xffffffff, w +=x )
 ...
 *
 *  Note:
 *      Called after HMD5Init or HMD5Update(itself)   
 */
VOID HMD5Update(MD5_CTX *pCtx, UCHAR *pData, ULONG LenInBytes)
{
    ...
    if (temp) 
    {
        ...
        HMD5Transform(pCtx->Buf, (ULONG *)pCtx->Input);
 ...
 *  	
 *  Note:
 *      Called by HMD5Update or HMD5Final
 */
...
{  
	...
	{ 
    	...
    	0xf7537e82,	0xbd3af235,	0x2ad7d2bb,	0x4819ab51

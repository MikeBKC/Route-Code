...
function showThreeMenu() { //描绘三级菜单函数
    ...
    for (i = 2; i < arrayThreeMenu[1] + 2; i++) {
		...
		if(arrayThreeMenu[i][0].length>=30)tabWidth=180; 
            ...
            else document.write('<td width="3" align="right" class="three_bottom"><img src="../<% write(com_imgthreel); %>"></td><td width="'+tabWidth+'" align="center"  class="three_both"><a onclick="openThreeMenu(' + i + ')" target="main"  style="cursor :pointer; color:408080">' + arrayThreeMenu[i][0] + '</a></td><td width="3"  class="three_bottom"><img src="../<% write(com_imgthreer); %>"></td><td width="6" class="three_bottom">&nbsp; </td>');
        }
    }
    document.write('');
...
function re_checkPassword1(cntrl1, maxlen, allownull) {
    ...
    if (pas1.indexOf(",") != -1 || pas1.indexOf(";") != -1 || pas1.indexOf("；") != -1 || pas1.indexOf("%") != -1 || pas1.indexOf("\"") != -1 || pas1.indexOf("\'") != -1 || pas1.indexOf("\\") != -1 || 　pas1.indexOf(" ") != -1 || pas1.indexOf("&") != -1 || pas1.indexOf("#") != -1 || pas1.indexOf(">") != -1)
	{
...
function checkPassword1(cntrl1, maxlen, allownull, alertFlag) {
    ...
    } else if (pas1.indexOf(",") != -1 || pas1.indexOf(";") != -1 || pas1.indexOf("；") != -1 || pas1.indexOf("%") != -1 || pas1.indexOf("\"") != -1 || pas1.indexOf("\'") != -1 || pas1.indexOf("\\") != -1 || 　pas1.indexOf(" ") != -1 || pas1.indexOf("&") != -1 || pas1.indexOf("#") != -1 || pas1.indexOf(">") != -1) {
        retips = "<%write(com_passwd);%> <% write(JSAlert582); %>";
    }
    if (retips != '') {
...
function dynamicShowList(index,ctrl){
	...
	if(recsPerPage[index]==10 ){
		height=65+20*recsPerPage[index]+"px";	
	}else if(recsPerPage[index]==20){
		height=85+20*recsPerPage[index]+"px";	
	}else if(recsPerPage[index]==30){
		height=105+20*recsPerPage[index]+"px";
	}else if(recsPerPage[index]==50){
		height=145+20*recsPerPage[index]+"px"
	}else{
		height=250+20*recsPerPage[index]+"px";	
	}
...
function dspList(index, list, tableWidth, arrayTitle, titleCount, arrayData, chkbox, editable) {
    ...
    if (chkbox) list.document.write('<td nowrap="nowrap" width="21" height="20">&nbsp;</td>');
    for (i = 0; i < titleCount; i++) {
        if (sortable[index][i] == 1) {
            list.document.write('<td nowrap="nowrap" height="21" align="center" class="style5"><a href="javascript:parent.sortData(' + index.toString() + ',parent.' + listheadname[index] + ',parent.' + listname[index] + ',' + i + ',parent.arrayData' + (index + 1) + ')" class="ablack">' + arrayTitle[i] + '</a></td>');
        } else {
            list.document.write('<td nowrap="nowrap" height="21" align="center" class="style5">' + arrayTitle[i] + '</td>');
        }
    }
    if (editable) list.document.write('<td align="center" class="style5"> <%write(com_edit); %></td>');//第一行 编辑
...
function is3GPort(index,wanCount) {
	if (((index == 2) && (platform == "nv510W"))||  ((index == 3) && (platform == "nv515W"))||((index==wanCount-1) && ((platform == "nv518W-Plus")||(platform == "nv518W-Lite"))) || ((index == 3) && (platform == "nvD908W"))) {
		return true;
	} else {
		return false;
	}
...
function isAPCliPort(index, wanCount) {
    ...
    if (((index == 3) && (platform == "nv510W")) || ((index == wanCount) && (platform == "nv520W"))||((index == wanCount) && (platform == "U1000N"))|| ((index == wanCount) && (platform == "nv518W")) || ((index == wanCount) && (platform == "nv518W-VPN")) || ((index == wanCount) && (platform == "nv512W"))|| ((index == wanCount) && (platform == "nv842W"))|| ((index == wanCount) && (platform == "nv518W-Plus"))|| ((index == wanCount) && (platform == "nv518W-Lite")) || ((index == wanCount) && (platform == "nvA750W"))||((index == wanCount)&&(platform == "nvA755W")) || ((index == wanCount) && (platform == "nvA650W")) || ((index == wanCount) && (platform == "nv750W"))||(index == wanCount && (platform == "nv1220GW")) ||(index == wanCount && (platform == "nv750GW")) || ((index == wanCount) && (platform == "nv520Wv3"))|| ((index == wanCount) && (platform == "nv842Wv3")) || ((index == wanCount) && (platform == "nvA650W_INT"))|| (index == wanCount && (platform == "u1000Nv2")) || ((index == wanCount) && (platform == "nvA655W"))||((index == wanCount)&&(platform == "nvA658W")))   {
		return true;
	} else {
		return false;
	}
...
/*显示保存成功提示信息*/
function showWaitTips(flag, content)
{
		content ="正在保存...";
		window.parent.art.dialog({
		    title:false,
...
/*确认框*/
function showConfirm(content,okFunc,cancelFunc){
	if(content){
		window.parent.art.dialog({
		title:false,
		width:'300px',
...
/*升级框*/
function showUpdate(okFunc,cancelFunc){

		window.parent.art.dialog({
		title:false,
		width:'700px',
		height:'auto',
		content:' <div style="width:600px; margin:0 auto; text-align:center;position: relative;">'+
				'<br>'+
...
//				        		'<button type="button" class="btn" id="update_button" style="margin: 20px 50px;">升级</button>'+
//				        	'</td>'+
//				        '</tr>'+
				   ' </table>'+
				   ' </form>'+

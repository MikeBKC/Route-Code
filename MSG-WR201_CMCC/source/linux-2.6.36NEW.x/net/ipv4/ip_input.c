...
#ifdef CONFIG_PERFORMANCE_OPTIMIZATION
	if (!isSkbComeFromLan(skb)) {/*非lan口进来的，根据目的地址查询*/
	    int xrg = 0;
...
int ip_rcv(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (isSkbComeFromLan(skb)) {/*lan口进来的，根据源地址地址查询,查不到则创建*/
#if (CONFIG_UTT_MAC_LIMIT == FYES)
...
ip_hash_init:
		...
		{		    
			...
			memcpy(stats->mac, eth->h_source, ETH_ALEN);/*对iphash表的mac地址赋值*/
		    } else {/* 获取错误，将统计mac清0 */
			memset(stats->mac, 0, ETH_ALEN);
		    }
		    uttTraverseIpNodeInit(skb->ipHashNode); /*遍历各模块，初始化ip hash节点配置中的优先级字段*/
		}
		else
		{

...
/*
 * nfColliDetecTimeOut	-	超时发送DHCP discover包
 *
 * 输入参数：
 * data			-	此处不需要传入参数，但函数必须有一个参数，
 *				因此一直为0
 */
static void nfColliDetecTimeOut(u_long data)
{
	...
	mod_timer(&nfColliDetecTimer, jiffies + UTT_COLLISION_DETECTION_POLL_INTERVAL);/*重新激活定时器*/
	return;
}
/**
 * 轮询初始化函数入口
...
static void nfColliDetecPollInit(void)
{
    ...
    nfColliDetecTimer.expires = jiffies + UTT_COLLISION_DETECTION_POLL_INTERVAL;   /* 定时器超时时的节拍数*/
    nfColliDetecTimer.data = 0;                /* 给定时器处理函数传入参数0*/
    nfColliDetecTimer.function = nfColliDetecTimeOut;   /* 定时器超时时调用的函数*/
    add_timer(&nfColliDetecTimer);/*激活定时器*/
    return;
}

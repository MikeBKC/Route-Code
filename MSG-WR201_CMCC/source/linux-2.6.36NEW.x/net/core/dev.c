...
static int __netif_receive_skb(struct sk_buff *skb)
{
	...
	skb->utt_iif = skb->dev->utt_ifindex;
	/*
	...
	if(orig_dev->utt_ifindex != 0) {
#if 0
            if(orig_dev->qos_pertick_rxbytes > 0) 
#endif
            {
		orig_dev->qos_pertick_rxbytes -= ((int)skb->len);
//                uttEtherIfInfo[orig_dev->utt_ifindex].qos_pertick_rxbytes = orig_dev->qos_pertick_rxbytes;
            }
	}

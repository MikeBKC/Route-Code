...
#include "rtl8367rb/rtl8367b_asicdrv_phy.h"
#endif
...
extern int _8367_dump(void);
u32 mii_mgr_read(u32 phy_addr, u32 phy_register, u32 *read_data)
{                         
         u32 volatile status = 0;       
         u32 rc = 0;       

         rc = rtl8367b_getAsicPHYReg(phy_addr, phy_register, read_data);
//	 printk("%s: phy: %d, reg: %d, val: %d, rc: %d\n", __func__, phy_addr, phy_register, *read_data, rc);
         if(rc==0) {
             return 1;                  
         }
         else {
...
{
     rtl8367b_setAsicPHYReg(phy_addr, phy_register, write_data);
     return 0;
}
#elif defined (CONFIG_RALINK_RT6855) || defined (CONFIG_RALINK_RT6855A)

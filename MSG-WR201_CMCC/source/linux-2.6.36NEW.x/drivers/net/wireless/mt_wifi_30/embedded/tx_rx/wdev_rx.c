...
#if 0
#define DEBUG_UTT_APC_LED 1
#endif
#ifdef DEBUG_UTT_APC_LED
#define UTT_APCLED_DEBUG(fmt, argx...) printk(fmt, ##argx)
#else
#define UTT_APCLED_DEBUG(fmt, argx...)
#endif
...
#define APC_LED_UPDATE_INTERVAL 2*HZ /*apclient灯更新周期*/
#define APC_LED_NORX_TIMEOUT 10*HZ /*无流量超时关灯*/
#define APC_LED_BLINK_INTERVAL HZ/2 /*apclient 连接中，灯闪烁间隔*/
#define APC_GPIO_LED_ONOFF(gpio,on) \
{\
    if(on) {\
	(*((volatile u32 *)(RALINK_SYSCTL_BASE+0x620)))&= ~(0x1<<gpio); \
	(*((volatile u32 *)(RALINK_SYSCTL_BASE+0x600)))|= 0x1<<gpio; \
    }\
    else{\
	(*((volatile u32 *)(RALINK_SYSCTL_BASE+0x620)))|= (1<<gpio); \
    }\
}

#define APC_LED_ON(gpio) APC_GPIO_LED_ONOFF(gpio,1);
#define APC_LED_OFF(gpio) APC_GPIO_LED_ONOFF(gpio,0);
static void uttApcLedTimerFunc(unsigned long data);
static unsigned long apcLedNextJiff;
static UINT32 apcConnectSt = 0; /*apcliet 连接状态*/
static UINT32 uttApcBlinkLedCount = 0;/*apclient连接中，灯闪烁，当前亮灯的个数*/
static UINT32 uttApcRssiLedCount = 0;/*apclient接受信号强度指示灯亮的个数*/

/*初始化定时器，但不执行*/
DEFINE_TIMER(uttSignalLEDTimer, uttApcLedTimerFunc, 0, 0);

extern void uttApcLedInit(void)
{
    apcLedNextJiff = jiffies;
    return;
}
static void uttApcLed(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
    STA_TR_ENTRY* pRptStaEntry;
    PAPCLI_STRUCT pApCliEntry = &pAd->ApCfg.ApCliTab[0];

    apcConnectSt = 0;
    if((pApCliEntry->CtrlCurrState == APCLI_CTRL_CONNECTED) && (pApCliEntry->SsidLen != 0)) 
    {/*判断apclient连接状态*/

	if((pEntry != NULL) && IS_ENTRY_APCLI(pEntry) && (pEntry->Sst == SST_ASSOC)) {

	    pRptStaEntry = &pAd->MacTab.tr_entry[pEntry->wcid];

	    if (pRptStaEntry->PortSecured == WPA_802_1X_PORT_SECURED)
	    {

		if ((pApCliEntry->remoteAuthIsWep == 0) && pApCliEntry->wdev.WepStatus == Ndis802_11WEPDisabled) {/*两端均无wep加密*/
		    apcConnectSt = 1;	
		}
#if 0
		if ((pApCliEntry->remoteAuthIsWep == 0) && pApCliEntry->wdev.WepStatus == Ndis802_11WEPEnabled) {
		    apcConnectSt = 2;	
		}
#endif
		if ((pApCliEntry->remoteAuthIsWep == 1)) {
		    if (pApCliEntry->wepAuthIsPass) {
			apcConnectSt = 1;	
		    }
#if 0
		    else {
...
#endif
    //	printk("===================     apcConnectSt  ================ %d\n",apcConnectSt);

    /*更新信号强度指示*/
    if(((LONG)(pEntry->RssiSample.LastRssi[0] - pAd->BbpRssiToDbmDelta) >= (-50))||(((LONG)(pEntry->RssiSample.LastRssi[1]-pAd->BbpRssiToDbmDelta)>=(-50))))//点亮3盏信号灯 
    {
	uttApcRssiLedCount = 3;
    }
    else if(((LONG)(pEntry->RssiSample.LastRssi[0]-pAd->BbpRssiToDbmDelta)>=(-65))||(((LONG)(pEntry->RssiSample.LastRssi[1]-pAd->BbpRssiToDbmDelta)>=(-65))))//点亮2盏信号灯 
    {
	uttApcRssiLedCount = 2;
    }
    else/*只要有包来就亮一个灯*/
    {
	uttApcRssiLedCount = 1;
    }

    if(apcConnectSt ==1)
    {/*连接成功，直接更新灯*/
	UTT_APCLED_DEBUG("%s %d jiffies:%u apclient connected and update rssi led\n", __func__, __LINE__, (unsigned int)jiffies);
	if(uttApcRssiLedCount == 3)
	{
	    APC_LED_ON(0);
	    APC_LED_ON(2);
	    APC_LED_ON(3);
	}
	else if(uttApcRssiLedCount == 2)
	{
	    APC_LED_OFF(0);
	    APC_LED_ON(2);
	    APC_LED_ON(3);
	}
	else /*if(uttApcRssiLedCount==1)*/
	{
	    APC_LED_OFF(0);
	    APC_LED_OFF(2);
	    APC_LED_ON(3);
    ...
    /*若定时器未启动，则启动定时器
     *这里不加锁，多核情况可能导致timer_pendig判断过后，添加的定时器执行时间，被这里的mod_timer修改
     不过考虑到apclient led灯的时间要求不高，这里不加锁，以追求性能*/
    if(!timer_pending(&uttSignalLEDTimer))
    {
	UTT_APCLED_DEBUG("%s %d jiffies:%u restart timer\n", __func__, __LINE__, (unsigned int)jiffies);
	mod_timer(&uttSignalLEDTimer,jiffies+HZ);   
    }
}

/**
 *  此函数执行，证明有数据接受
 *  1.apclient未连接则闪烁。表示持续进行连接尝试
 *  2.不管apclient何种状态，超时应关灯。因为rssi靠收包计算。则若异常断开（rootap断电），则无rx，必须超时关灯
 *
 * */
static void uttApcLedTimerFunc(unsigned long data)
{
    if( time_after (jiffies, apcLedNextJiff + APC_LED_NORX_TIMEOUT))
    {/*长时间没有数据接受，则灭掉所有的灯.且定时器不再执行*/
	APC_LED_OFF(0);
	APC_LED_OFF(2);
	APC_LED_OFF(3);
	UTT_APCLED_DEBUG("%s %d jiffies:%u apclient no rx data, timeout and off led\n", __func__, __LINE__, (unsigned int)jiffies);
    }
    else
    {
	if(apcConnectSt == 0)
	{/*连接中*/
	    uttApcBlinkLedCount++;
	    if(uttApcBlinkLedCount > uttApcRssiLedCount)
	    {
		APC_LED_OFF(0);
		APC_LED_OFF(2);
		APC_LED_OFF(3);
		uttApcBlinkLedCount = 0;
	    }
	    else
	    {
		if(uttApcBlinkLedCount == 1)
		{
		    APC_LED_ON(3);
		}
		else if(uttApcBlinkLedCount == 2)
		{
		    APC_LED_ON(2);
		}
		else if(uttApcBlinkLedCount == 3)
		{
		    APC_LED_ON(0);
		}
	    }
	    UTT_APCLED_DEBUG("%s %d jiffies:%u apclient connecting,restart next blink timer\n", __func__, __LINE__,(unsigned int)jiffies);
	    mod_timer(&uttSignalLEDTimer,jiffies+APC_LED_BLINK_INTERVAL);   
	}
	else
	{
	    UTT_APCLED_DEBUG("%s %d jiffies:%u apclient connectend,restart next no rx timeout timer\n", __func__, __LINE__, (unsigned int)jiffies);
	    mod_timer(&uttSignalLEDTimer,jiffies+APC_LED_NORX_TIMEOUT);   
	}
    }
}
#endif
...
VOID dev_rx_data_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
...
#ifdef AIRKISS_SUPPORT
    if ((pAd->bAK_Monitor == TRUE) && (ak_get_status() != AIRKISS_STATUS_COMPLETE) ) 
	ak_rcv_frame_handle(pAd, (PHEADER_802_11) pRxBlk->pData, pRxBlk->DataSize);
#endif
    ...
    if(IS_ENTRY_APCLI(pEntry))
    {
	if( unlikely(time_after(jiffies,apcLedNextJiff)))
	{/*定时更新下apclient信号强度指示灯，以减小损耗*/
	    uttApcLed(pAd, pEntry);
	    apcLedNextJiff = jiffies + APC_LED_UPDATE_INTERVAL;
	}
    }
#endif

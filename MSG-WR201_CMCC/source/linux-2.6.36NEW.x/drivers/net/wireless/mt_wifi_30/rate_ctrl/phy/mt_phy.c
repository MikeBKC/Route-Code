...
INT MTSmartCarrierSense(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():Enter ---> BssNr=%d, miniRSSI=%d, TotalByteCount=%d RxByteCount=%d TxByteCount=%d EDCCA=%d RxRatio=%d\n", __FUNCTION__,
		...
		&& (pAd->MacTab.Size > 0))
	{
		...
		else if ((pAd ->SCSCtrl.SCSMinRssi > -74) && (RxRatio < 90)) 
		{
			if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_LOW)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): CSC=L\n", __FUNCTION__));
			}
		}	
		else
		{
			if (pAd->SCSCtrl.SCSStatus != SCS_STATUS_DEFAULT)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): CSC=H (Default)\n", __FUNCTION__));				
			}
		}				
	}
	else	

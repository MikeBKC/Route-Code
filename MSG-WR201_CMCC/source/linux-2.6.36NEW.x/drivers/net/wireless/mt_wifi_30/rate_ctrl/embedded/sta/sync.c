	...
	IN PVOID SystemSpecific3)
{
	...
	*/
    ...
    //    airkiss_status_t ak_status = ak_get_status();
    //    if (ak_status == AIRKISS_STATUS_CHANNEL_LOCKED || ak_status == AIRKISS_STATUS_COMPLETE )
    //        return;         
    //}
#endif
...
#ifdef AIRKISS_SUPPORT
            if (pAd->bAK_Monitor == TRUE && pAd->MlmeAux.Channel == 0 && pAd->ScanCtrl.Channel == 0) {
                airkiss_status_t ak_status = ak_get_status();
                if (ak_status != AIRKISS_STATUS_CHANNEL_LOCKED && ak_status != AIRKISS_STATUS_COMPLETE) {
                    pAd->MlmeAux.Channel = FirstChannel(pAd);
                    pAd->ScanCtrl.Channel = FirstChannel(pAd);
                    //ak_debug(("mlmeaux channel (%d), scanctrl channel (%d) \n", pAd->MlmeAux.Channel, pAd->ScanCtrl.Channel));
                }
            }            
#endif

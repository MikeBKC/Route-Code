...
extern void ralink_gpio_led_onoff(int gpio, int on);
#define __LED_ON(gpio) ralink_gpio_led_onoff(gpio,1);
#define __LED_OFF(gpio) ralink_gpio_led_onoff(gpio,0);
static unsigned int SIGNAL_LED_ST = 0;/*0 表示初始状态， 1 表示点亮三个信号灯，2 表示点亮两个信号灯 ，3 表示点亮一个信号灯 ，4 表示信号差没有点亮信号灯 ， 5 表示在定时器中灭掉信号灯*/
#define RptTimeOutCount 5
struct timer_list uttSignalLEDTimer;
#endif
...
#ifdef CONFIG_RALINK_MT7628_RPT1603
static void uttRptTimerfunc(unsigned long data)
{
    if(SIGNAL_LED_ST == 5)
    {
	__LED_OFF(0);
	__LED_OFF(2);
	__LED_OFF(3);
	SIGNAL_LED_ST = 0 ;//设置为初始状态	
    }
    else
    {
	SIGNAL_LED_ST = 5 ;//定时器修改状态
        mod_timer(&uttSignalLEDTimer,jiffies+RptTimeOutCount*HZ);   
    }
}

static int utt_rpt_timer_init(void)
{
    init_timer(&uttSignalLEDTimer);
    uttSignalLEDTimer.expires = jiffies + RptTimeOutCount*HZ;
    uttSignalLEDTimer.function = uttRptTimerfunc;
    add_timer(&uttSignalLEDTimer);
    return 0;
}
#if 0
static void utt_rpt_timer_destory(void)
{
        del_timer(&uttSignalLEDTimer);
}
...
// TODO: shiang@PF#2, is this atheros protection still necessary here????
	...
	if(IS_ENTRY_APCLI(pEntry))
	{
	    if(SIGNAL_LED_ST == 0)
	    {
		printk("\nxrgadd line= %d,func=%s\n\n",__LINE__,__func__);
		utt_rpt_timer_init();	
	    }
	    if(((LONG)(pEntry->RssiSample.LastRssi[0]-pAd->BbpRssiToDbmDelta)>=(-65))||(((LONG)(pEntry->RssiSample.LastRssi[1]-pAd->BbpRssiToDbmDelta)>=(-65))))//点亮3盏信号灯 
	    {
		if(SIGNAL_LED_ST!=1)//检测状态是否变化
		{
		    SIGNAL_LED_ST = 1;
		    __LED_ON(0);
		    __LED_ON(2);
	    ...
	    else if(((LONG)(pEntry->RssiSample.LastRssi[0]-pAd->BbpRssiToDbmDelta)>=(-75))||(((LONG)(pEntry->RssiSample.LastRssi[1]-pAd->BbpRssiToDbmDelta)>=(-75))))//点亮2盏信号灯 
	    {
		if(SIGNAL_LED_ST!=2)
		{
		    SIGNAL_LED_ST = 2;
	    ...
	    else if(((LONG)(pEntry->RssiSample.LastRssi[0]-pAd->BbpRssiToDbmDelta)>=(-80))||(((LONG)(pEntry->RssiSample.LastRssi[1]-pAd->BbpRssiToDbmDelta)>=(-80))))//点亮1盏信号灯 
	    {
		if(SIGNAL_LED_ST!=3)
		{
		    SIGNAL_LED_ST = 3;
	    ...
	    //printk("\n\n ......RSSI =============%ld %ld %ld  mac:%x-%x-%x-%x-%x-%x\n",(LONG)(pEntry->RssiSample.LastRssi[0] - pAd->BbpRssiToDbmDelta),(LONG)(pEntry->RssiSample.LastRssi[1] - pAd->BbpRssiToDbmDelta),(LONG)(pEntry->RssiSample.LastRssi[2] - pAd->BbpRssiToDbmDelta),pEntry->Addr[0],pEntry->Addr[1],pEntry->Addr[2],pEntry->Addr[3],pEntry->Addr[4],pEntry->Addr[5]);   
	} 
#endif

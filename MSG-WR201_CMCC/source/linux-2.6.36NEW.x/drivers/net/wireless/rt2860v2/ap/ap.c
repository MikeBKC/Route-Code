	...
	IN PRTMP_ADAPTER pAd)
{
	...
	CHAR avgRssi;
#endif
#ifdef  UTT_WFDRV_PRINT
	UINT32 rx, tx;
	static UINT32 d=0u, retry=0u, failed=0u, RTS=0u, ACKfail=0u, dup=0u, FCSE=0u;
#endif
	...
	if((d & 0x1u) == 1u)
	{
	    printk("re:%u, f:%u, RTS:%u, ACK:%u, dup:%u,FCS:%u\n", pAd->WlanCounters.RetryCount.u.LowPart-retry,
		    pAd->WlanCounters.FailedCount.u.LowPart-failed, pAd->WlanCounters.RTSSuccessCount.u.LowPart-RTS,
		    pAd->WlanCounters.ACKFailureCount.u.LowPart-ACKfail, pAd->WlanCounters.FrameDuplicateCount.u.LowPart-dup,
		    pAd->WlanCounters.FCSErrorCount.u.LowPart-FCSE);
	retry = pAd->WlanCounters.RetryCount.u.LowPart;
	failed = pAd->WlanCounters.FailedCount.u.LowPart;
	RTS = pAd->WlanCounters.RTSSuccessCount.u.LowPart;
	ACKfail = pAd->WlanCounters.ACKFailureCount.u.LowPart;
	dup = pAd->WlanCounters.FrameDuplicateCount.u.LowPart;
	FCSE = pAd->WlanCounters.FCSErrorCount.u.LowPart;
	    rx = pAd->RalinkCounters.OneSecReceivedByteCount;
	    tx = pAd->RalinkCounters.OneSecTransmittedByteCount;

	    printk("apRate rx: ");
	    if(rx>=0x100000)
		printk("%d.%dM ", rx>>20u, rx&0xFFFFFu);
	    else if(rx>=0x400)
		printk("%d.%dK ", rx>>10u, rx&0x3FFu);
	    else
		printk("%dB ", rx);
	...
	for (i = 1; i < MaxWcidNum; i++) 
	{
		...
		{
		    if(pEntry->RxBytes >= pEntry->LastRxBytes){
			pEntry->RxDataRate = pEntry->RxBytes - pEntry->LastRxBytes;
		    }
		    else {
			pEntry->RxDataRate = 0xFFFFFFFFU - pEntry->LastRxBytes + pEntry->RxBytes;
		    }

		    if(pEntry->TxBytes >= pEntry->LastTxBytes){
			pEntry->TxDataRate = pEntry->TxBytes - pEntry->LastTxBytes;
		    }
		    else {
			pEntry->TxDataRate = 0xFFFFFFFFu - pEntry->LastTxBytes + pEntry->TxBytes;
		    }
#ifdef  UTT_WFDRV_PRINT

		if((d&0x1u) == 0u) {
		    printk("%02X%02X%02X- %02X%02X%02X Rate: rx ", pMacTable->Content[i].Addr[0],pMacTable->Content[i].Addr[1],pMacTable->Content[i].Addr[2],pMacTable->Content[i].Addr[3],pMacTable->Content[i].Addr[4], pMacTable->Content[i].Addr[5]);
		    tx = pEntry->TxDataRate;
		    rx = pEntry->RxDataRate;
		    if(rx>=0x100000)
			printk("%d.%dM ", rx>>20u, rx&0xFFFFFu);
		    else if(rx>=0x400)
			printk("%d.%dK ", rx>>10u, rx&0x3FFu);
		    ...
		    pEntry->LastRxBytes = pEntry->RxBytes;
		    pEntry->LastTxBytes = pEntry->TxBytes;
#endif
...
#ifdef UTT_ASSOC_THRESHOLD_PATCH
		else if ((pAd->ApCfg.MBSSID[pEntry->apidx].RssiLowForStaKickOut != 0) && 
			  ( (avgRssi=RTMPAvgRssi(pAd, &pEntry->RssiSample)) < pAd->ApCfg.MBSSID[pEntry->apidx].RssiLowForStaKickOut)) 
		{
			bDisconnectSta = TRUE;
			DBGPRINT(RT_DEBUG_WARN, ("Disassoc STA %02x:%02x:%02x:%02x:%02x:%02x , RSSI Kickout Thres[%d]-[%d]\n",
					PRINT_MAC(pEntry->Addr), pAd->ApCfg.MBSSID[pEntry->apidx].RssiLowForStaKickOut,
					avgRssi));

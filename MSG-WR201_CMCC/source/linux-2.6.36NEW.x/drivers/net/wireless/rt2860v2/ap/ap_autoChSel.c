...
static inline UCHAR calculateWifiScore(
	IN PRTMP_ADAPTER pAd
	)
{
	PBSSINFO pBssInfoTab = pAd->pBssInfoTab;
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++)
	{
		...
		*/
		...
		pChannelInfo->wifiChannelScore[channel_idx] = dirtyness;
		...
		pChannelInfo->wifiChannelScore[channel_idx] += falsecca;
		if (min_score > pChannelInfo->wifiChannelScore[channel_idx]) {
		    min_score = pChannelInfo->wifiChannelScore[channel_idx];
		    ch = pAd->ChannelList[channel_idx + 4].Channel;
		}
	}

	return ch;
}
VOID CollectWifiScore(
	IN PRTMP_ADAPTER pAd,
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	UCHAR ch = 0, i;
	ChannelSel_Alg Alg = ChannelAlgCCA;
	PSTRING msg;
	UINT32 TotalLen;

	TotalLen = sizeof(CHAR)*((pAd->ChannelListNum+1)*20);
	...
	*/
	...
	ch =  calculateWifiScore(pAd);
	for (i=0; i<pAd->ChannelListNum; i++)
	{
	    if (!pAd->pChannelInfo->SkipList[i]) {
		sprintf(msg+strlen(msg), "Channel[%d]=%lu\n", pAd->ChannelList[i].Channel,pAd->pChannelInfo->wifiChannelScore[i]);
	    }
	}

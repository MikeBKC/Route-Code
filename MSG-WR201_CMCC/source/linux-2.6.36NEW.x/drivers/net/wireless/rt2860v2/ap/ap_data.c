...
#if 0
void frame_dump(unsigned char* buffer, int len)
{
    int i;
	...
	*/
	...
	if (IS_ENTRY_CLIENT(pMacEntry) || (Wcid == MCAST_WCID))
	{
		...
		else
		{
			...
			if(pMbss != NULL) 
			{
			    policy = pMbss->limitConf.limitPolicy;
			    if((policy != UTT_BANDWIDTH_DISABLED) && (pMbss->limitConf.dRate != 0)) {
				if(policy == UTT_BANDWIDTH_SHARED)/*¹²Ïí*/ {
				    limit = &(pMbss->limitConf.runStat);
				}
				else {/*¶ÀÏí*/
				    limit = &(pMacEntry->runStat);
				}
				type = RTMP_GET_PACKET_PROTOCOL(pPacket);
				if(type==0x0800) {
				    len = SrcBufLen-34;
				}
				else {
				...
				write_lock(&(pMbss->limitConf.downlock));
				if(utt_down_ratelimit(limit, &(pMbss->limitConf), len)==0) {
				    write_unlock(&(pMbss->limitConf.downlock));
				    RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
				    return NDIS_STATUS_FAILURE;
				}
				write_unlock(&(pMbss->limitConf.downlock));
	...
	if (RTMPEqualMemory(pMacEntry->Addr, pAddr, MAC_ADDR_LEN))
	{
		...
		*/
        ...
        if (isActive == FALSE)
        {
			...
			if (pMacEntry->fPsPollEnable)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("%s(): : packet not send by HW then ignore other PS-Poll Aid[%d]!\n", __FUNCTION__, pMacEntry->Aid));
				RTMP_IRQ_UNLOCK(&pAd->irq_lock, IrqFlags);
				return;
			}
			else
			{
				ULONG psTime = 500;
				pMacEntry->fPsPollEnable = TRUE;
				if (pMacEntry->PsQueue.Head)
					psTime = 500 * 2;
					
				if (pMacEntry->fPsPollTimerRunning == TRUE)
				{
					RTMPModTimer(&pMacEntry->fPsPollTimer, psTime);
				}	
				else
				{
					pMacEntry->fPsPollTimerRunning = TRUE;	
			       	RTMPSetTimer(&pMacEntry->fPsPollTimer, psTime);				
				}				
				DBGPRINT(RT_DEBUG_TRACE, ("%s(): : receive PS-Poll then turn on fPsPollEnable[%d]  Aid[%d] fPsPollTimerRunning[%d] psTime[%d]!\n", 
					__FUNCTION__, pMacEntry->fPsPollEnable, pMacEntry->Aid, pMacEntry->fPsPollTimerRunning, psTime));	
			}
#endif
			if (pMacEntry->PsQueue.Head)
			{
...
#ifdef UTT_ADDED 
                        //added by brwang
                        (pAd)->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].RxPacket++;
                        (pAd)->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].RxByte += pRxBlk->DataSize;
#endif
		...
		*/
		switch (pHeader->FC.Type)
		{
			...
			case BTYPE_MGMT:
                ...
                {
                    DBGPRINT(RT_DEBUG_TRACE, ("Receive ASSOC_REQ & update RSSI...\n"));
                    Update_Rssi_Sample(pAd, &pAd->ApCfg.RssiSample, pRxWI);
                    pAd->ApCfg.NumOfAvgRssiSample ++;
                }
#endif

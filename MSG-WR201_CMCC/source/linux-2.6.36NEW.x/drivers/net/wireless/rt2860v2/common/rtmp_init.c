...
#define SERIAL_NUM 0x2c
	UINT32 serno = 0u, n = 0u;
	int ret, rdlen;
	u_char *buf = NULL;
	struct mtd_info *mtd;

#ifdef CONFIG_UTT_MAC_FC2FEF
	pAd->PermanentAddress[0] = 0xFC;
	pAd->PermanentAddress[1] = 0x2F;
	pAd->PermanentAddress[2] = 0xEF;
#else
	pAd->PermanentAddress[0] = 0x00;
	pAd->PermanentAddress[1] = 0x22;
	pAd->PermanentAddress[2] = 0xaa;
#endif
	pAd->PermanentAddress[3] = 0xFF;
	pAd->PermanentAddress[4] = 0xFF;
	pAd->PermanentAddress[5] = 0xFF;

	/*此处用mtd驱动中函数读取序列号，如果读取出错不return，接口继续启动*/
	mtd = get_mtd_device_nm("Bootloader");
	if (!IS_ERR(mtd)) {
		buf = kmalloc(mtd->erasesize, GFP_KERNEL);
		if (buf != NULL) {
			ret = mtd->read(mtd, SERIAL_NUM, 4, &rdlen, buf);
			if (rdlen != 4) {
			//	printk("%d in %s of %s, read error\n", __LINE__, __func__, __FILE__);
			}

			serno = *((int *)buf);
		//	printk("%d in %s of %s, serno = %x\n", __LINE__, __func__, __FILE__, serno);
			kfree(buf);
#ifdef CONFIG_CHINA_MAC_04714B
			pAd->PermanentAddress[0] = 0x04;
			pAd->PermanentAddress[1] = 0x71;
			pAd->PermanentAddress[2] = 0x4B;
#else
#ifdef CONFIG_UTT_MAC_FC2FEF
			pAd->PermanentAddress[0] = 0xFC;
			pAd->PermanentAddress[1] = 0x2F;
			...
			serno += 0x2711;
#ifdef CONFIG_UTT_8_BSSID_SERNO/*开启赚钱wifi，dat会有8个BSSID，需要是8的倍数*/
			serno <<= 3;
#else
			serno <<= 2;
#endif
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	do
	{
		...
		UAPSD_SP_AUE_Handle(pAd, pEntry, StaFifo.field.TxSuccess);
			...
			if (pEntry && pEntry->fPsPollEnable)
			{
				BOOLEAN Cancelled;
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s(): wcid[%d] pEntry->fPsPollEnable[%d] fPsPollTimerRunning[%d]\n", 
									__FUNCTION__, wcid, pEntry->fPsPollEnable, pEntry->fPsPollTimerRunning));
			}
#endif /* CONFIG_AP_SUPPORT */

#endif /*UTT_POWER_SAVING_PATCH*/
#ifdef CONFIG_STA_SUPPORT
...
/*	EDCA_PARM DefaultEdcaParm;*/
	...
	pAd->CommonCfg.BYPASS = 0;
#endif
#ifdef UTT_MAXSTANUM
	pAd->CommonCfg.uttMaxStaNum = 0;
#endif
#ifdef RTMP_RBUS_SUPPORT
	if (pAd->infType == RTMP_DEV_INF_RBUS)
	{
...
#ifdef CONFIG_AP_SUPPORT
{
...
#endif /* P2P_APCLI_SUPPORT */
	{
		...
		for(j = BSS0; j < pAd->ApCfg.BssidNum; j++)
		{
			...
			pAd->ApCfg.MBSSID[j].AssocReqRssiThreshold = 0;
			pAd->ApCfg.MBSSID[j].RssiLowForStaKickOut = 0;
#endif
#ifdef CONFIG_UTT_AP_RATE_LIMITING
		        NdisZeroMemory(&(pAd->ApCfg.MBSSID[j].limitConf), sizeof(pAd->ApCfg.MBSSID[0].limitConf));
		        pAd->ApCfg.MBSSID[j].limitConf.limitPolicy = UTT_BANDWIDTH_DISABLED;
			rwlock_init(&(pAd->ApCfg.MBSSID[j].limitConf.uplock));
			rwlock_init(&(pAd->ApCfg.MBSSID[j].limitConf.downlock));
#endif

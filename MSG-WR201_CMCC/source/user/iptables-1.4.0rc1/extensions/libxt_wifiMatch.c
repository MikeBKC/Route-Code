...
static void wifiMatch_check(unsigned int flags)
{
	...
	if (flags == 0)
		exit_error(PARAMETER_PROBLEM, "wifiMatch: no wifiMatch option specified");
...
{
	const struct xt_wifiMatch_info *info = (struct xt_wifiMatch_info *)match->data;
...
static struct xtables_match wifiMatch_match6 = {
	.family		= AF_INET6,
	.name		= "wifiMatch",
	.version	= IPTABLES_VERSION,
	.size		= XT_ALIGN(sizeof(struct xt_wifiMatch_info)),
...
void _init(void)
{
	...
	xtables_register_match(&wifiMatch_match6);
}

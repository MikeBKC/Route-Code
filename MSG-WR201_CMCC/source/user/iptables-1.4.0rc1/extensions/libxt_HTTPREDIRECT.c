...
{	
	...
	case '1' :
		...
		exit_error(PARAMETER_PROBLEM, "HTTPREDIRECT target: optarg[%s] len exceed %d", optarg, HTTP_R_LEN);
	    }
	...
	case '2':
		...
		exit_error(PARAMETER_PROBLEM, "HTTPREDIRECT target: optarg[%s] should be c,r or d", optarg);
	    }
	    break;
	default:
	    return 0;

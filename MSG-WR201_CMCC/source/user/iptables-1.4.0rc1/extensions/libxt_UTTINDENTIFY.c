...
{	
	...
	case '1' :
	    if (!optarg) {
		exit_error(PARAMETER_PROBLEM, "UTTINDENTIFY target: need arg\n");
	    }
	    /* ÊÇ·ñ°üº¬, */
	    if (!(secStr = strstr(optarg, UTTIND_ARG_SPE))) {
		exit_error(PARAMETER_PROBLEM, "UTTINDENTIFY target: arg should contain ,\n");
	    }
	    /* ×ª»»´íÎó */
	    priF = (u_int8_t) strtol(optarg, NULL, 10);
	    secF = (u_int32_t) strtoul(secStr + 1, NULL, 10);
	    if ((priF == 0) || (secF == 0)) {
		exit_error(PARAMETER_PROBLEM, "UTTINDENTIFY target: error arg\n");
	    }

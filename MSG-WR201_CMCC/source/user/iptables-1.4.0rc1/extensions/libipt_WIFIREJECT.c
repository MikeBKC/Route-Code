                        ...
                        const void *entry, struct xt_entry_target **target)
{
	...
	case '1':
		...
		exit_error(PARAMETER_PROBLEM, "WIFIREJECT target: optarg[%s] len exceed %d", optarg, HTTP_R_LEN);
	    }
	...
	case '2':
		...
		exit_error(PARAMETER_PROBLEM, "WIFIREJECT target: optarg[%s] len exceed %d", optarg, HTTP_SN_LEN);
	    }
	...
	case '3':
		...
		exit_error(PARAMETER_PROBLEM, "WIFIREJECT target: optarg[%s] len exceed %d", optarg, HTTP_DOMAIN_LEN);
	    }
	    strncpy(reject->auth_domain, optarg, HTTP_DOMAIN_LEN);
	    break;
	case '4':
	    if ((strlen(optarg) > HTTP_LOGIN_URL_LEN)) {
		exit_error(PARAMETER_PROBLEM, "WIFIREJECT target: optarg[%s] len exceed %d", optarg, HTTP_LOGIN_URL_LEN);
	    }
	    strncpy(reject->login_url, optarg, HTTP_LOGIN_URL_LEN);
	    break;
	case '5':
	    if ((strlen(optarg) > HTTP_SECRET_KEY_LEN)) {
		exit_error(PARAMETER_PROBLEM, "WIFIREJECT target: optarg[%s] len exceed %d", optarg, HTTP_SECRET_KEY_LEN);
	    }
	...
	default:
		/* Fall through */
		exit_error(PARAMETER_PROBLEM, "WIFIREJECT target: Invalid argument");
		return 0;
	}
	return 1;
}
...
static struct iptables_target reject_tg_reg = {
	.name		= "WIFIREJECT",
	.version	= IPTABLES_VERSION,
	.size		= IPT_ALIGN(sizeof(struct ipt_wifireject_info)),
	.userspacesize	= IPT_ALIGN(sizeof(struct ipt_wifireject_info)),

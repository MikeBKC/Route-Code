...
#if (DST == FYES)
int rfc1305print(uint32_t *data, struct ntptime *arrival, int dstFlag);
#else
...
static int alter=0; /* if alter interval */
static int set_clock_flag=0; /* record if has set clock already */
static unsigned int _timezone;
...
//_syscall1( int, adjtimex, struct timex *, txc);
...
#undef Data
	...
	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
		struct timeval tv_set;
                ...
                fp = fopen("/etc/TZ","r");
                if(fp != NULL){
                    fscanf(fp,"%d",&timezone);
                    fclose(fp);
                }
#endif
		/* it would be even better to subtract half the slop */
//		tv_set.tv_sec  = xmttime.coarse - JAN_1970 + 8*3600;
#if (DST == FYES)
		if (dstFlag) {
		    tv_set.tv_sec  = xmttime.coarse - JAN_1970 + _timezone + 60*60;
		} else {
		    tv_set.tv_sec  = xmttime.coarse - JAN_1970 + _timezone;
		}
#else
		tv_set.tv_sec  = xmttime.coarse - JAN_1970 + _timezone;
#endif
...
//		if (_syscall2(int,do_settimeofday,struct timeval *,&tv_set,struct timezone *,NULL)) {
...
int setup_transmit(int usd, char *host, short port)
{
	...
	sa_dest.sin_port=htons(port);
        printf("usd[%d],sin_addr[%x],len[%d], errno[%d]\n",usd, sa_dest.sin_addr, sizeof(sa_dest), errno );
...
#if (DST == FYES)
int primary_loop(int usd, int num_probes, int interval, int goodness, int dstFlag)
#else 
...
#endif
{
	...
	for (;;) {
...
#if (DST == FYES)
			error = rfc1305print(incoming_word, &udp_arrival_ntp, dstFlag);
#else
		...
		if (probes_sent >= num_probes && num_probes != 0) return 0;
	}
        return -1;
}
...
int main(int argc, char *argv[]) {
	...
	int cycle_time_backup;
	int probe_count=0;            /* default of 0 means loop forever */
	/* int debug=0; is a global above */
	int goodness=0;
	char hostname[16*3]={0};          /* support 3 hosts */
        char *hostnames[3]={0};
	int replay=0, hostNum=1;                 /* replay mode overrides everything */
#if (DST == FYES)
	int dstFlag = 0;
#endif
	for (;;) {
#if (DST == FYES)
		c = getopt( argc, argv, "c:" DEBUG_OPTION "g:h:i:z:lp:rsaf" );
#else
		c = getopt( argc, argv, "c:" DEBUG_OPTION "g:h:i:z:lp:rsa" );
#endif
		if (c == EOF) break;
		switch (c) {
			case 'c':
			...
			case 'g':
				...
				break;
			case 'h':
				strncpy(hostname, optarg, 47);
                                hostnames[0]=hostname;
                                if((hostnames[1]=strchr(hostname, ',')) != NULL){
                                    hostnames[1][0]='\0';
                                    hostnames[1]++;
                                    hostNum++;
                                    if((hostnames[2]=strchr(hostnames[1], ',')) != NULL){
                                        hostnames[2][0]='\0';
                                        hostnames[2]++;
                                        hostNum++;
                                    }
	...
	if (debug) {
		...
		live, udp_local_port, set_clock, alter, _timezone);
	}

	/* if alter true, back cycle_time */
	if (alter) {
	    cycle_time_backup=cycle_time;
	}
...
#endif
	    /* 
	     *if we have got ntp reply and have already set the time,
	     * we use the interval arg to send ntp request.
	     *if we first get ntp request, that means we have not set the time,
	     * we send ntp request every 1 sec. see mantis 0010845
	     */
	    if (alter) {
		if (set_clock_flag) {
		    cycle_time = cycle_time_backup;
		} else {
		    cycle_time = 1;
		}
	    }
            for(i = 1; i <= hostNum; i++) {
                if(strlen(hostnames[index]) < 7){
                    rc=-1;
                } else {
	            rc=setup_transmit(usd, hostnames[index], NTP_PORT);
                    if(rc>=0) {
#if (DST == FYES)
	                rc=primary_loop(usd, probe_count, cycle_time, goodness,dstFlag);
#else
	                rc=primary_loop(usd, probe_count, cycle_time, goodness);
#endif
                    }
                }
                if(rc<0) {
                    index=(index+1)%hostNum;
                } else {
                    break;
                }
            }

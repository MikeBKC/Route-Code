 ...
 *
 * $Id: pptpctrl.c,v 1.2 2013/03/12 16:06:41 hou.bo Exp $
 */
...
static void launch_pppd(char **pppaddrs, struct in_addr *inetaddrs)
{
	...
	if (*pppaddrs[0] || *pppaddrs[1]) {
		char pppInterfaceIPs[50];
		sprintf(pppInterfaceIPs, "%s:%s,%s", pppaddrs[0], pppaddrs[1],inet_ntoa(inetaddrs[1]));
		pppd_argv[an++] = pppInterfaceIPs;
	}
#endif

	syslog(LOG_INFO, "%s:%s-%d, noipparam = %d, pptp_logwtmp = %d\n", __FILE__, __func__, __LINE__, noipparam, pptp_logwtmp);
        if (!noipparam) {

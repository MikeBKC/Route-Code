...
#if (VPN_CONTROL == FYES)
extern void doSystem(const char* _Format, ...);
#endif
...
#if (VPN_CONTROL == FYES)
	openswan_log("[VPN] %s-%d: DPD timeout, restart connection name %s ",__func__,__LINE__,c->name);
	doSystem("vpnHandle plus ipsec %s ",c->name);
#endif
...
#endif
	if(strlen(c->name)!=0U){
	    //openswan_log("DPD: c->name=%s---------------------------------------",c->name);

...
#define ASSIGNED_TIME 1388534390 /*2014.01.01 00:00:00 秒数时间*/
...
typedef struct {
    ...
    int Auth_time;		/*@brief default client auth time 默认客户端认证时间(单位：秒)*/
    int freeAuth_time;		/*@brief deault client free auth time 默认免认证时间(单位：秒)*/
    int conns;			/*@brief the max connections that wifidog allow user connect */
    unsigned int tmpSN;
    unsigned int  macAuthEn;
#if (WIFIDOG_NOTICE == FYES)
    unsigned int noticeTime;			    /*到期通告时间，单位分钟*/
    char noticeUrl[MAX_URL_LEN+1];	    /*到期推送url*/
#endif

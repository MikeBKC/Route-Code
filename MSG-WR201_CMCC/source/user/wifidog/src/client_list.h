 ...
 * */
typedef enum {
    /*每个比特位代表一种类型，以下定义用于移位*/
    WIFIDOG_CLIENT_CTRL_TYPE_RATE_LIMIT=0,	    /*带宽控制*/
    WIFIDOG_CLIENT_CTRL_TYPE_EXPIRING_NOTICE,	    /*到期通告*/
} ControlType;
/*
 * 客户端认证尅性定义
 * */
typedef enum {
    WIFIDOG_CLIENT_AUTH_TYPE_UNKOWN=0,		    /*未知证类型*/
    WIFIDOG_CLIENT_AUTH_TYPE_FREE=1,		    /*免认证类型*/
    WIFIDOG_CLIENT_AUTH_TYPE_MAC_BLACK,		    /*MAC黑名单*/
    WIFIDOG_CLIENT_AUTH_TYPE_MAC_GREY,		    /*MAC灰名单*/
    WIFIDOG_CLIENT_AUTH_TYPE_MAC_WHITE,		    /*MAC白名单*/
    WIFIDOG_CLIENT_AUTH_TYPE_PHONE,		    /*手机认证*/
    WIFIDOG_CLIENT_AUTH_TYPE_WECHAT_URL,	    /*微信URL认证*/
    WIFIDOG_CLIENT_AUTH_TYPE_ACCOUNT,		    /*账号认证*/
#if (SWITCH_AUTH_MODE == FYES)
    WIFIDOG_CLIENT_AUTH_TYPE_SWITCH_NO_MAC_AUTH,	    /*三层交换机下认证(路由模式),无法获取mac，通过ip放通*/
#endif
} AuthType;
...
typedef struct	_t_client {
  struct	_t_client *next;        /**< @brief Pointer to the next client */
	...
	int	expire;			/**认证有效期*/
	int	runtime;
	int	free_auth_client;	/** 免认证用户*/
	int	need_notify;		/*服务端强制下线待通知用户*/
	...
	t_counters	counters;	/**< @brief Counters for input/output of
					     the client. */
	AuthType authType;		/*客户端的认证类型*/
	int ctrlType;			/*客户端的控制标志*/
	union {
#if (WIFI_RATE_LIMIT == FYES) 
	    /*带宽控制*/
	    int index;			/* ctrlType 为带宽控制时，即该客户端是带宽控制的，
					 * 此时index保存带宽限速全局变量的索引，其中保存此客户端所述的限速组，超时时会用到*/
#endif
	    void *reserve;
	}data;
} t_client;

...
**
**
** $Id: api.c,v 1.6.28.1 2015/07/13 20:01:51 hou.bo Exp $
**
*/
	...
	struct	timeval *timeout;
{
	...
	if (r == NULL) {
	    fprintf(stderr,"malloc client request error!\n");
		//server->lastError = -3;
		return(NULL);
	}
 ...
 * http 响应
 * 页面不缓存
 * 直接使用httpdOutPut 部分浏览器可能缓存，比如小米3自带浏览器
 * bhou
 */
void uttHttpdOutNoCache(request *r, const char *msg)
{

    httpdAddHeader(r, "Pragma: no-cache\nCache-Control: no-cache\nExpires:0\n");
    _httpd_sendHeaders(r, strlen(msg), 0);
    httpdOutput(r, msg);
}

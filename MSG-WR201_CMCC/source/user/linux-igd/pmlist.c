...
struct utt_Upnp_data uttSendData;
static void utt_sendDataToUpnpTool(int port);
...
int pmlist_PushBack(struct portMap* item)
{
	...
	{
            fp = fopen("/tmp/upnp","wb+");
            if(fp)
            {
                tmp = pmlist_Head;
                while(tmp) 
                {
                    fwrite(tmp,sizeof(struct portMap),1,fp);
                    tmp = tmp->next;
                }
                fclose(fp);
            }
...
int pmlist_AddPortMapping (int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	
	{
	  ...
	  trace(3, "%s", command);
          memset(&uttSendData, 0, sizeof(struct utt_Upnp_data));
          strncpy(uttSendData.action, "I", 0x10);
          strncpy(uttSendData.protocol, protocol, 0x10);
          strncpy(uttSendData.dport, externalPort, 0x10);
          strncpy(uttSendData.innerIp, internalClient, 0x10);
          strncpy(uttSendData.innerPort, internalPort, 0x10);
          for(i = 1; i <= MAX_WAN_IF_COUNT; i++) {
            utt_sendDataToUpnpTool(UPNP_DEMO_PORT_BASE + i);
          }
...
int pmlist_DeletePortMapping(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	
	{
	  ...
	  trace(3, "%s", command);
          memset(&uttSendData, 0, sizeof(struct utt_Upnp_data));
          strncpy(uttSendData.action, "D", 0x10);
          strncpy(uttSendData.protocol, protocol, 0x10);
          strncpy(uttSendData.dport, externalPort, 0x10);
          strncpy(uttSendData.innerIp, internalClient, 0x10);
          strncpy(uttSendData.innerPort, internalPort, 0x10);
...
static void utt_sendDataToUpnpTool(int port)
{
    ...
    bd = bind(sd, (struct sockaddr*)&cliAddr, stLen);
    if(bd < 0) {
    ...
    sn = sendto(sd, &uttSendData, sizeof(uttSendData), 0,

	...
	while (--argc > 0 && **++argv == '-') {
		while (*++*argv) {
			...
			case 'V': /*software version*/
				if ((--argc <= 0) ||
				    (opt_sw_ver = get_sys_ver(*++argv, 0)) < 0)
					usage ();
				printf("firmware version: %08x\n", opt_sw_ver);
				goto NXTARG;
			case 'H': /*hardware version*/
				if ((--argc <= 0) ||
				    (opt_hw_ver = get_sys_ver(*++argv, 1)) < 0)
					usage ();
			...
			case 'F': /*function mode*/
				...
				opt_fc = strtol(*++argv, NULL, 16);
				printf("fc mode: %08x\n", opt_fc);
				goto NXTARG;
			case 'U': /*firmware update control: license control is 1*/
				if (--argc <= 0) {
				    usage();
				}
				opt_update = strtol(*++argv, NULL, 16);
				printf("license ver: %08x\n", opt_update);
				goto NXTARG;
...
NXTARG:		;
	...
	opt_sw_ver    = opt_sw_ver << ((HWVER_1 + HWVER_2));
	hdr->systemVer= htonl(opt_sw_ver | opt_hw_ver);

	strncpy((char *)hdr->name, name, IH_NMLEN);

	hdr->ih_fc    = htons(opt_fc);
	hdr->ih_update    = opt_update;
	...
	int j;
	for (j = 0; j < 100; j++) {
		printf(" %02x,", *((unsigned char *)hdr + j));
		if ((j + 1) % 16 == 0) {
			printf("\n");
		}
	}
...
usage ()
{
	...
	fprintf (stderr, "          -A ==> set architecture to 'arch'\n"
			 ...
			 "          -F ==> set utt function mode\n"
			 "	    -U ==> set utt soft update(license control)"
		);
	exit (EXIT_FAILURE);
}

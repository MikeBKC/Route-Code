...
/*
 * kill_3g_pppd - 3G卡拔出后杀pppd进程
 *
 * 由于现在有了自动检测热插拔，因此没有必要在3G
 * 卡拔出后继续拨号
 *
 */
static void kill_3g_pppd(void)
{
	FILE *fp;
	char *p = NULL;
	char command[20];
	...
	if (fp != NULL) {/*执行成功*/
		memset(buffer, 0, 50);
		while (fgets(buffer, sizeof(buffer), fp) != NULL) {
			if ((p = strstr(buffer,"pppd file /etc/3g")) != NULL) {
				memset(pppd_pid, 0, 10);
				memset(command, 0, 20);
				sscanf(buffer, "%s", pppd_pid);
				SWORD_PRINTF("%s-%d:pppd_pid = %s\n", __func__, __LINE__, pppd_pid);
				sprintf(command, "kill %s", pppd_pid);
...
static void auto_dial_3g(char *Vendor_last, char *ProdID_last)
{
	...
	system("/sbin/3g_switch.sh");/*执行拨号脚本*/

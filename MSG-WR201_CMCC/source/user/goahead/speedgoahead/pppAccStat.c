...
static void pollTask(void)
{
...
#endif
#endif
#if (RAISECOM_211 == FYES && WIFI_SLEEP_FLAG == FYES)
    char Routetable[256];
    char Default[16] = {0};
    unsigned int gpioflag = 0;
#endif
	...
	if(xport_cnt == 300) { /*300s*/
        ...
        if(gpioflag == 0)
        {
            while(fgets(Routetable,sizeof(Routetable),fp) != NULL)
            {   
                sscanf(Routetable,"%7s %*s",Default);
                if(strcmp(Default,"default") == 0 && profSys->wifisleep != FUN_DISABLE)
                {   
                    gpioflag = 1;
                    system("gpio l 2 4000 0 0 0 0");
                }
            }
        }else{
            while(fgets(Routetable,sizeof(Routetable),fp) != NULL)

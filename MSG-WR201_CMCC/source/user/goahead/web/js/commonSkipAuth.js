...
alertFlag:0 根据情况产生alert提示框（button，submit触发事件产生提示。其他不产生）
		  ...
		  2 产生提示框
************/
function inputErro(obj, tips, alertFlag) {
...
function showThreeMenu() { //描绘三级菜单函数
    ...
    for (i = 2; i < arrayThreeMenu[1] + 2; i++) {
        if (arrayThreeMenu[i][2]) {
            if (arrayThreeMenu[i][3]) document.write('<td width="3" align="right" valign="bottom"><img src="../<% write(com_imgthreel); %>" height="27"></td><td width="120" class="three_top" align="center"><b>' + arrayThreeMenu[i][0] + '</b></td><td width="3" valign="bottom"><img src="../<% write(com_imgthreer); %>" height="27"></td><td width="6" class="three_bottom">&nbsp; </td>');
            else document.write('<td width="3" align="right" class="three_bottom"><img src="../<% write(com_imgthreel); %>"></td><td width="120" align="center"  class="three_both"><a onclick="openThreeMenu(' + i + ')" target="main"  style="cursor :hand; color:408080">' + arrayThreeMenu[i][0] + '</a></td><td width="3"  class="three_bottom"><img src="../<% write(com_imgthreer); %>"></td><td width="6" class="three_bottom">&nbsp; </td>');
        }
    }
    document.write('');
...
function re_checkSpecialName(cntrl, tips) {
    ...
    for (i = 0; i < name.length; i++) {
        if (allowChar.indexOf(name.substring(i, i + 1)) == -1) retips = "<% write(JSAlert20); %>";
    }
    return retips;
}
//功能：检查是否合法数字并返回提示信息
...
function checkName(cntrl, maxlen, tips, allownull) {
    ...
    } else if (name.indexOf(";") != -1 || name.indexOf(",") != -1 || name.indexOf("%") != -1 || name.indexOf("\"") != -1 || name.indexOf("&") != -1 || name.indexOf(" ") != -1 || name.indexOf("\'") != -1 || name.indexOf("\\") != -1 || name.indexOf("<") != -1 || name.indexOf(">") != -1) {
...
function checkPassword1(cntrl1, maxlen, allownull, alertFlag) {
    ...
    } else if (pas1.indexOf(",") != -1 || pas1.indexOf(";") != -1 || pas1.indexOf("；") != -1 || pas1.indexOf("%") != -1 || pas1.indexOf("\"") != -1 || pas1.indexOf("\'") != -1 || pas1.indexOf("\\") != -1 || 　pas1.indexOf(" ") != -1) {
        retips = "<% write(JSAlert19); %>";
    }
    if (retips != '') {
...
function dynamicShowList(index,ctrl){
	...
	if(recsPerPage[index]==10 ){
		height=60+20*recsPerPage[index]+"px";	
	}else if(recsPerPage[index]==20){
		height=80+20*recsPerPage[index]+"px";	
	}else if(recsPerPage[index]==30){
		height=100+20*recsPerPage[index]+"px";
	}else if(recsPerPage[index]==50){
		height=140+20*recsPerPage[index]+"px"
	}else{
		height=180+20*recsPerPage[index]+"px";	
	}
...
if(lineButton[index]==0){
	...
	if(index==0)
		var lineContent='每页显示行数&nbsp;<select id="frame'+(index+1)+'2List" name="frame'+(index+1)+'2List" onchange="parent.dynamicShowList(0,frame'+(index+1)+'2List)" style="width:50px;height="12px;"><option value="10" selected="selected">10</option><option value="30">30</option><option value="50">50</option></select>';
	else if(index==1)
		var lineContent='每页显示行数&nbsp;<select id="frame'+(index+1)+'2List" name="frame'+(index+1)+'2List" onchange="parent.dynamicShowList(1,frame'+(index+1)+'2List)" style="width:50px;height="12px;"><option value="10" selected="selected">10</option><option value="30">30</option><option value="50">50</option></select>';
}else if(lineButton[index]==20){
	if(index==0)
		var lineContent='每页显示行数&nbsp;<select id="frame'+(index+1)+'2List" name="frame'+(index+1)+'2List" onchange="parent.dynamicShowList(0,frame'+(index+1)+'2List)" style="width:50px;height="12px;"><option value="20" selected="selected">20</option><option value="50">50</option><option value="100">100</option></select>';
	else if(index==1)
    ...
    allcheckobj[index].checked = false;
		...
		if(lineButton[index]==10||lineButton[index]==20){
		if(index==0){
...
function dspList(index, list, tableWidth, arrayTitle, titleCount, arrayData, chkbox, editable) {
    ...
    list.document.write('	 <td nowrap="nowrap" class="style21"><table width="' + tableWidth + '"  border="0"  cellpadding="0" id="tableList" name="tableList" cellspacing="1" >');
    list.document.write('	<tr bgcolor="ffffff">');
    if (chkbox) list.document.write('<td nowrap="nowrap" width="20" height="20">&nbsp;</td>');
    for (i = 0; i < titleCount; i++) {
        if (sortable[index][i] == 1) {
            list.document.write('<td nowrap="nowrap" height="20" align="center" class="style5"><a href="javascript:parent.sortData(' + index.toString() + ',parent.' + listheadname[index] + ',parent.' + listname[index] + ',' + i + ',parent.arrayData' + (index + 1) + ')" class="ablack">' + arrayTitle[i] + '</a></td>');
        } else {
            list.document.write('<td nowrap="nowrap" height="20" align="center" class="style5">' + arrayTitle[i] + '</td>');
        }
    }
    if (editable) list.document.write('<td align="center" class="style5"> <%write(com_edit); %></td>');

	if (_strSecondMenuFocus == "<% write(PDB); %>") {//策略库页面
		list.document.write('<td align="center" class="style5"> <%write(com_updateStra); %></td>');
	}
    ...
    for (i = recbegin; i < recend; i++) { //i = recbegin 而不是 = recbegin + 1 因为FILLDATA函数中的参数和arrarData的索引 从0开始　
        ...
        if (i < recscount[index]) {
            ...
            if (chkbox) {
                list.document.write('<td nowrap="nowrap" height="20"><input type="checkbox" class="input-checkbox" name="chklist" value="on"></td>');
            }
            if (editable) //add by cxhu 090825
            list.document.write('<td nowrap="nowrap" height="20" align="center" class="style6"><a href="javascript:parent.EditConfig(1,' + i + ')" class="ablack">' + arrayData[0][i] + '</a></td>');
            else list.document.write('<td nowrap="nowrap" height="20" align="center" class="style6">' + (arrayData[0][i] || "&nbsp;") + '</td>');
            for (j = 1; j < titleCount; j++) {
                if ((j == 2 && _strSecondMenuFocus == "无线主机状态") || (j == 3 && _strSecondMenuFocus == "<% write(com_bandList); %>")||(j == 1 && _strSecondMenuFocus == "<% write(com_pppAcc);%>") || (j == 4 && _strSecondMenuFocus == "<% write(com_netAction); %>") || (j == 3 &&_strSecondMenuFocus == "连接限制") || (j == 1 && _strSecondMenuFocus == "<%write(com_ipsecInfo);%>") || (j == 1 && _strSecondMenuFocus == "<%write(com_control);%>") || (j == 1 && _strSecondMenuFocus == "<%write(com_policyroute);%>") || (j == 2 && _strSecondMenuFocus == "PPTP") || (j == 1 && _strSecondMenuFocus == "<% write(com_parentscontrol); %>")) {
                    ...
                    list.document.write('<td nowrap="nowrap" align="center" height="20"><input type="checkbox" name="chkAllow" ' + strTmp + ' onClick= "javascript:parent.SaveUserAllow(' + index + ',' + i.toString() + ')" value=' + arrayData[j][i] + '></td>');
                } else list.document.write('<td nowrap="nowrap" height="20" align="center" class="style6">' + arrayData[j][i] + '</td>');
            }
            ...
            if (chkbox) list.document.write('		<td nowrap="nowrap" height="20"></td>');
...
/* 是否3G接口 */
function is3GPort(index) {
	if (((index == 2) && (platform == "nv510W"))||((index==wanCount-1) && ((platform == "nv518W-Plus")||(platform == "nv518W-Lite")))) {
		return true;
	} else {
		return false;
	}
...
function isAPCliPort(index, wanCount) {
    if (((index == 3) && (platform == "nv510W")) || ((index == wanCount) && (platform == "nv520W"))||((index == wanCount) && (platform == "U1000N"))|| ((index == wanCount) && (platform == "nv518W")) || ((index == wanCount) && (platform == "nv518W-VPN")) || ((index == wanCount) && (platform == "nv512W"))|| ((index == wanCount) && (platform == "nv842W"))|| ((index == wanCount) && (platform == "nv518W-Plus"))|| ((index == wanCount) && (platform == "nv518W-Lite")) || ((index == wanCount) && (platform == "nv520Wv3")) || ((index == wanCount) && (platform == "nv842Wv3")))   {
		return true;
	} else {
		return false;
	}

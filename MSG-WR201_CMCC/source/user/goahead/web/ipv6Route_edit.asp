...
<% 
	...
	var edit_Name="";
	edit_Name=Form("id");
	if ( edit_Name!="")aspOutputIpv6RouteConfigOneInfo(edit_Name);//需要修改
%> 
/*
...
function save(){
	...
	if(checkData()){
		...
		Ipv6Route_Config.action="/goform/ConfigIpv6RouteConfig";
		Ipv6Route_Config.submit();
		f.RouteName.disabled = true;
	}
}
function checkData(){
	var f = Ipv6Route_Config;
        f.DesIP.value = f.DesIP.value.trim();
        f.GateWay.value = f.GateWay.value.trim();
        f.PNPL.value = f.PNPL.value.trim();
        if (!checkName(f.RouteName,11,"<%write(rou_routeName);%>",0)) return false;
        if (f.DesIP.value != "::"){
	    if (!checkIpv6(f.DesIP,"<%write(rou_destNet);%>",0)) return false;
	}
	if (!checkIpv6Sp(f.DesIP,"<%write(rou_destNet);%>")) return false;
	if (!checkIpv6(f.GateWay,"<%write(rou_gw);%>",1)) return false;
	if (!checkIpv6Sp(f.GateWay,"<%write(rou_gw);%>")) return false;
        if (!checkNumber(f.PNPL,0,127,"<%write(rou_destNetPreLen);%>",0)) return false;
        if (!checkNumber(f.Metric,0,7,"<%write(com_priority);%>",0)) return false;
        return true;
}
function Reset(){
	Ipv6Route_Config.reset();
	init();
}
//-->
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <td width="15"></td>
  <!--////内容-->
  <td><script language="javascript">showSecondTitle('<%write(ipv6RouteConfig);%>');</script>
  <table width="100%" border="0" height="450" cellspacing="0" cellpadding="0" align="center"  class="table_bg">
   <script language="javascript">showerror(errorstr);</script>
  <tr valign="top">
    <td><form action="/goform/"  name="Ipv6Route_Config" method="post">
...
<p>&nbsp;</p>
  ...
  <tr><td colspan="3" height="20"><input type="hidden" name="Action" value="add">&nbsp;</td></tr>
   ...
   <tr>
	   ...
	   <td><input name="RouteName" type="text" maxlength="11" size="12" onChange="showfiledtip(re_checkName(this,'<%write(rou_routeName);%>'),yutd1)"></td><td  id="yutd1"></td>
	   ...
	   <td ><input name="DesIP" type="text" size="40" maxlength="39" onKeyPress="ipv6Only()" onChange="" ></td><td  id="yutd2"></td>
	   </tr>
	    <tr>
	   <td align="right" height="25"><%write(rou_destNetPreLen);%></td>
	   <td ><img src="<%write(com_imgqt);%>" width="7" height="9"></td>
	   <td ><input name="PNPL" type="text" size="16" maxlength="3" onKeyPress="figureOnly()" onChange="showfiledtip(re_checkNumber(this,0,127,'<%write(rou_destNetPreLen);%>'),yutd3)" value="64"></td><td  id="yutd3"></td>
	   </tr>
	    <tr>
	   <td align="right" height="25"><%write(com_GateWayAddr);%></td>
	   <td ><img src="<%write(com_imgqt);%>" width="7" height="9"></td>
	   <td ><input name="GateWay" type="text" size="40" maxlength="39" onKeyPress="ipv6Only()" onChange="showfiledtip(re_checkIpv6(this,'<%write(com_GateWayAddr);%>',1),yutd4)"></td><td  id="yutd4"></td>
	   </tr>
	    <tr>
	   <td align="right" height="25"><%write(com_priority);%></td>
	   <td ><img src="<%write(com_imgqt);%>" width="7" height="9"></td>
	   <td class="helpfont"><input name="Metric" type="text" size="16" maxlength="1" onKeyPress="figureOnly()"  onChange="showfiledtip(re_checkNumber(this,0,7,'<%write(com_priority);%>'),yutd5)" value="0">&nbsp;<%write(rou_priTip);%></td><td  id="yutd5"></td>
	   </tr>
	    <tr>
	   ...
	   <option value='LAN0'> LAN</option>
		 ...
		 for(var i=1;i<=wanIfCount;i++){
				if (is3GPort(i,wanIfCount)) {
					...
					document.write('<option value="WAN'+i+'" '+ (i==1? 'selected="selected"':'')  + (i!=1? 'disabled="true"':'')+'>WAN'+i+'</option>');
				}
		 }
		if(PPTPClientList == 1)
		{
  ...
  <table width="80%" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    <td   height="30" align="center" valign="top"><input type="button" class="input-button" value="<%write(com_save);%>"  onClick="save()">&nbsp;&nbsp;&nbsp;<input type="button" class="input-button" value="<%write(com_reset);%>" onClick="Reset()">&nbsp;&nbsp;&nbsp;<input type="button" value="<%write(com_help);%>" class="input-button" onClick="window.open('<% write(com_helppage); %>#Route')">&nbsp;&nbsp;&nbsp;<input type="button" class="input-button" value="<%write(com_back);%>" onClick="GoBack_Page('ipv6Route.asp')">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
  </td></tr>
  </table></td>

 ...
 *
 * $Id: webs.c,v 1.19.14.1.4.2 2017/12/26 17:01:32 wu.xiaoyu Exp $
 */
...
#if (UTT_TOMAX == FYES) 
static char_t	websRealm[64] = T("ToMAX");	/* Realm name */
#elif(BRAND_LEVEL_ONE == FYES)
static char_t   websRealm[64] = T("Levelone"); /* Realm name */
#elif(MIDDLE_SOFT == FYES)
static char_t   websRealm[64] = T("Device");
#else
static char_t	websRealm[64] = T("UTT");	/* Realm name */
#endif
...
#if 0  
        static unsigned int acc_count; 
        printf("websAccept count: %x\n",++acc_count);
#endif 
...
static void websSocketEvent(int sid, int mask, int iwp)
{
	...
	printf("websSocketEvent count: %x\n",++count);
#endif
	wp = (webs_t) iwp;
	a_assert(wp);
 ...
 */
           ...
           if (wp->postData) {
 		        ...
 		        #if (WEB_SERVER == FYES)
 	                #endif      
 					        //if allocate mem faile do nothing left.					        
 					        wp->postData = tmp;					        	        
 					    }						  
 ...
 /*		wp->query = bstrdup(B_L, text);*/
                  //modify by jxy12@2004-05-19 for WebUI
 			      //if clen > 64k then think it is update the firmware and allocate 1.25 m memory
                 if(wp->clen > L64k) {
                    char_t *tmp = wp->postData;	
                    wp->postData = balloc(B_L, wp->clen);
                    if(!wp->postData)  {
          #if (WEB_SERVER == FYES)
          #endif
                        wp->clen = 0;
 ...
 */
 ...
 //add by brwang@2003-12-10 for WebUploadFile
 /******************************************************************************/
 /*
                                 ...
                                 if ((tmpEndStr = gstrchr(&tmpStartStr[17], '\"')) != NULL) {
                                     if ((tmpValLenByte = ((int)tmpEndStr - (int)&tmpStartStr[17])) > 0) {
                                         ...
                                         if ((tmpValLenByte = ((int)endPart - (int)part - 2)) > 0) {
                                             tmpValLenByte = (tmpValLenByte < (int)(MAX_LINE-strlen(tmpNameStr))) ? tmpValLenByte : (MAX_LINE-strlen(tmpNameStr)-1);
...
/***************************
 *当authResult=0,  判断是否有特殊需求
 *return value:
 *      1-- 有特殊需求，需跳过课正常访问
 *      0-- 无特殊需求
 */
#define MAXURL 28
int isHaveSpecialNeed(char_t **geturl)
{

    //printf("%s-%d,geturl = %s\n",__FUNCTION__,__LINE__,*geturl);
    int     i =0;
    const char*   specialusr[MAXURL]={
		    "/index.asp",
		    "/License.asp",
		    "/goform/UpdateLicenseFile",
		    "/css/uttwlan.css",
		    "/css/style.css",
		    "/images/logo.png",
		    "/images/tag1.gif",
    ...
    if(strstr(*geturl,"goform"))
    {
	return 1;
    }
    for( i=0; i<MAXURL; i++ )
    {
	if((strcmp(*geturl, specialusr[i] )) == 0)
	{
	    return 1;
	}
 ...
 */
	...
	if (url && url[0] == '/' && strlen(url) == 5)
	{
	    for (i=1; i<=4; i++)
		captcha = captcha * 10 + url[i]%16%10;
	    if (url[1] >= 'A' && url[1] <= 'Z')  /* wechat webauth */
	    {
		sprintf(tmp, "/goform/formWebAuthFastSubmit?phonecode=%04d", captcha);
		url = tmp;
	    }
	    else if (url[1] >= 'a' && url[1] <= 'z')  /* sms webauth */
	    {
		sprintf(tmp, "/goform/formWebAuthFastSubmit?weixincode=%04d", captcha);
		url = tmp;
	    }
	} 

	if (url && memcmp(url, "/wechatUrl", strlen("/wecharUrl")) == 0)
	{
	    sprintf(tmp, "/goform%s", url);
	    url = tmp;
	}
#endif
	/*
...
#if (UTT_LICENSE == FYES)
	LicenseProfile *prof=NULL;
	prof = (LicenseProfile *)ProfGetInstPointByIndex(MIB_PROF_LICENSE, 0);
	if(prof!=NULL){
	    if(prof->authResult!=0 || isHaveSpecialNeed(&url)) {
		wp->url = bstrdup(B_L, url);
		_LICENSEPRINT("url = %s,buf = %s, path = %s\n",bstrdup(B_L, url),buf,path);
	    } else{
		wp->url = buf=path=(char_t *)("/License.asp");
		wp->url = bstrdup(B_L, url);
		_LICENSEPRINT("wp-<url = %s\n",wp->url);
	    }
	}else{
	    wp->url = bstrdup(B_L, url);
	}
...
#define isgoodchar(s) (gisalnum((s)) || ((s) == '/') || ((s) == '_') || \
						((s) == '.')  || ((s) == '-') )
#define isgoodurlchar(s) (isgoodchar(s) || ((s) == '?') || ((s) == '&') || ((s) == '=') )/*url允许字符集*/
static void websParseRequest(webs_t wp)
{
 ...
 */
					...
					/*判断结束位置。
					  */
					if (gstricmp(cp, T("uri")) == 0) {
					    /*url路径，允许字符集不同于其他，否则将导致认证错误框的出现*/
					    while (isgoodurlchar(*npv)) {
						npv++;
					    }
					}
					else {
 ...
 */
		    ...
		    wp->httpHeadHost = bstrdup(B_L, value);
		}   
	}
}
...
void websSetEnv(webs_t wp)
{
	...
	websSetVar(wp, T("SERVER_PROTOCOL"), wp->protoVersion);
 	...
 	if (wp->flags & WEBS_MULTIPART_FORM) {
 		...
 		if (uploadedFileContent != NULL) {
           ...
           memcpy(wp->postData, uploadedFileContent, lenUploadedFile);
           //*(wp->postData + lenUploadedFile) = '\0';
  ...
  */
                     len = gstrlen(wp->query);
                     wp->query = brealloc(B_L, wp->query, (len + wp->lenPostData + 1) * sizeof(char_t));
...
char_t *websGetVarNull(webs_t wp, char_t *var, char_t *defaultGetValue, char_t *nullValue)
{
	sym_t	*sp;
	...
	if ((sp = symLookup(wp->cgiVars, var)) != NULL) {
		a_assert(sp->content.type == string);
		if (sp->content.value.string) {
                    if(strcmp(sp->content.value.string, "") == 0) {
			return nullValue;
                    } else {
			return sp->content.value.string;
                    }
 ...
 */
	...
	/*用于自定义登录界面,只有从登陆界面发来的请求才会朝下执行，否则均跳转到登录界面*/
	if (code == 401) {
	    if ((strstr(wp->url,"/index_login.asp") == NULL) || ((gstrcmp(wp->url,"/index_login.asp") == 0) && (wp->flags == (WEBS_AUTH_DIGEST | WEBS_ASP | WEBS_COOKIE)))) {
 ...
 */
		if (code == 401) {
            ...
            /* ...$$$ end */
#if (LOGIN_PAGE_DIY == FYES)
				char_t *timestamp = NULL;
				static char_t oldTimeStamp[64];
				timestamp = websGetVar(wp,T("timestamp"),T(""));
				if(strncmp(timestamp,oldTimeStamp,64) != 0) {/*第一次请求时，向浏览器输出需要认证的信息*/
				    strncpy(oldTimeStamp,timestamp,64);
#endif
				    websWrite(wp, 
...
/********************************************************************
 * 函数： websWriteBigBlock
 * 功能： 向页面输入比较大的数据块时选择此函数
 * 创建： 2014-02-13
 * 参数： 略
 * 返回： 略
 * 输出： 略
 * 作者： 赵冲
 ********************************************************************/ 
int websWriteBigBlock(webs_t wp, char_t *fmt, ...)
{
    int leftLen =0;
    int writeLen =0;
    ...
    va_start(p, fmt);
    r = vasprintf(&string_ptr, fmt, p);
    va_end(p);

    stringLength = strlen(string_ptr);
    n = stringLength/alloclen;
    for(m = 0; m < n; m ++) {    
	websWriteBlock(wp, string_ptr+writeLen, alloclen);	
	writeLen += alloclen;
    }
    leftLen =  stringLength - writeLen;
    if(leftLen > 0 ) {
	websWriteBlock(wp, string_ptr+writeLen, leftLen);
    }

...
int main(int argc, char **argv)
{
	...
	num = atoi(argv[1]);
	till = num+100;
	for(num;num<till;num++) {
	    memset(str, 0, sizeof(str));
	    sprintf(str,"%d",num);
    
	if((cipherStr = DES_Encrypt(str, str, &cipherLen)) == NULL) {
	    printf("des decrypt error.\n");
	    return -1;
	}
    if(cipherStr != NULL) {
	fprintf(fp,"%s %s \n",cipherStr,str);
	printf("%s-%d: cipherStr =%s, cipherLen =%d\n",__func__,__LINE__,cipherStr,cipherLen);
	if(DES_Decrypt(cipherStr,str,plainStr) == -1) {
	    printf("des decrypt error.\n");
	    return -1;
	}
	free(cipherStr);
...
#endif
#if 1
    char *mistr = NULL,*dest =NULL;
    int encryptlen=0;
    int baseLen = 0 ,i;
    plainLen = strlen(plain);
    mistr = getEncString(plain,plainLen,&encryptlen);
    if(mistr != NULL) {
	dest = (char *)malloc(encryptlen*2 + 1); 
	if(!dest)
	    return NULL;
	memset(dest,0,encryptlen+1);
	for(i=0;i<encryptlen;i++){
	    sprintf(dest+i*2,"%.2x",mistr[i]&0xFF);
	//    dest[2*i] =(mistr[i] & 0xF);
	  //  dest[2*i+1] =(mistr[i]>>4 & 0xF);
...
#if DEBUG
    printf("%s-%d: encryptlen =%d, mistr =%s\n",__func__,__LINE__,encryptlen,mistr);
#endif
#endif
    free(mistr);
    return dest;
...
int DES_Decrypt(char *cipher, char *keyStr,char *plain)
{
    ...
    char *mingstr = NULL;
    int decryptlen=0;
    mingstr = getDesString(dst2,len,&decryptlen);
#if DEBUG
    printf("%s-%d: decryptlen =%d, mingstr =%s\n",__func__,__LINE__,decryptlen,mingstr);
    printf("strlen(mingstr) =%d\n",strlen(mingstr));
#endif
    if(mingstr != NULL) {
	plain = mingstr;
	return decryptlen;
...
int DES_Encrypt(char *plainFile, char *keyStr,char *cipherFile)
{
    ...
    memset(buf, 0U, fileLen+1);
    char *dst1 = NULL;
    if((fread(buf,sizeof(char),fileLen,plain)) == 0) {
    ...
    char *mistr = NULL;
    int encryptlen=0;
    mistr = getEncString(buf,fileLen,&encryptlen);
#if DEBUG
    printf("%s-%d: encryptlen =%d, mistr =%s\n",__func__,__LINE__,encryptlen,mistr);
#endif
#endif
#if 1
    if(mistr != NULL) {
	dst1=base64_encode(mistr, encryptlen); /* the second parameter must accord with the first one */
#if DEBUG
	printf("%s-%d: strlen(dst1) =%d \ndst1 =%s\n",__func__,__LINE__,strlen(dst1),dst1);
#endif
    }
    if(dst1 != NULL) {
	if(fwrite(dst1,sizeof(char),strlen(dst1),cipher)!=strlen(dst1))
...
int DES_Decrypt(char *cipherFile, char *keyStr,char *plainFile)
{
    ...
    if(fileLen%4 != 0) {
	printf("%s-%d: read file len error.\n",__func__,__LINE__);
	fclose(cipher);
	fclose(plain);
	return -1;
    }
#if 1
    int len=0;
    char *cipherBlock1=NULL;
    char *dst2=NULL;
    cipherBlock1 = (char*)malloc(fileLen+1);
    if(cipherBlock1!=NULL) {
	memset(cipherBlock1,0,fileLen+1);
	fread(cipherBlock1,sizeof(char),fileLen,cipher);
    }
#if DEBUG
    printf("cipherBlock1 =%s\n",cipherBlock1);
#endif
    dst2=base64_decode(cipherBlock1,fileLen, &len);
#if 0
    while(dst2[len-1] == '=') {
	len -= 1;
    }
...
#endif
    if(mingstr != NULL) {
	if(fwrite(mingstr,sizeof(char),decryptlen,plain) != decryptlen)
	    printf("write error.\n");
	fwrite("\n",sizeof(char),1U,plain);
    } else {
...
static void getSoftVersion(char *softVer) 
{
    ...
    tblock = localtime(&t);
    snprintf(softVer,15,"%04d%02d%02d%02d%02d%02d",(tblock->tm_year+1900),(tblock->tm_mon+1),(tblock->tm_mday-1),
	    tblock->tm_hour,tblock->tm_min,tblock->tm_sec);
#if DEBUG
    printf("sec =%d, min =%d, hour =%d, dd =%d, mm =%d, yy =%d, wday =%d,yday =%d,isdst =%d,zone =%s\n",
...
#endif
#if 0
    DES_Encrypt("20130709134227.txt", "123445","2.txt");
    DES_Decrypt("tmp.url","123445","3.txt");
#endif
#endif

...
xmlnode_t * cwmp_xml_create_child_time_node(env_t * env ,  xmlnode_t * parentNode, const char * ns, const char * nodeName, const time_t *x_time)
{
    XmlElement * newNode;
    pool_t * pool = env->pool;
    char      tm_buf[CWMP_BUF_SIZE];
    struct    tm t;
    time_t    tt;
    ...
    else
    {
        ...
        t = *localtime(x_time);
        TRsnprintf(tm_buf, CWMP_BUF_SIZE, "%4d-%02d-%02dT%02d:%02d:%02d",
                t.tm_year + 1900,
                t.tm_mon + 1,
                t.tm_mday,
                t.tm_hour,
        ...
        XmlNodeSetNodeValue(pool , (xmlnode_t *)newNode, tm_buf);
...
#if (WECHAT_CWMP == FYES)
int  cwmp_add_parameter_to_list_by_nameValue(pool_t * pool,  parameter_list_t * pl, const char * name ,const char* value)
{
    parameter_t * pv = pool_pcalloc(pool, sizeof(parameter_t));
    if (!pv)
    {
        return NULL;
    }

    pv->name =  pool_pstrdup(pool, name);

    pv->value =  pool_pstrdup(pool, value);
    pv->value_length = strlen(value);
    ...
    {
        //parameter_t ** pp = XREALLOC(pl->parameters, pl->size * sizeof(parameter_t*), sizeof(parameter_t*) * (pl->size+CWMP_RENEW_SIZE));
        // pool_prealloc(env->pool, p, olds, news)
        parameter_t ** pp = pool_prealloc(pool,pl->parameters, pl->size * sizeof(parameter_t*), sizeof(parameter_t*) * (pl->size+CWMP_RENEW_SIZE));
        pl->parameters = pp;
        pl->size += CWMP_RENEW_SIZE;
    }
    pl->parameters[pl->count++] = pv;
    return CWMP_OK;
}
#endif
...
#else
            if (TRstrcasecmp(node->name, name) == 0)
#endif
            {
...
#if (WECHAT_CWMP == FYES)
	    else if(pn->type == TYPE_OBJECT && pn->detail){
		    rc = (*pn->detail)(env->cwmp, name, ppl,env->pool);
...
int  cwmp_parse_setparametervalues_message(env_t * env , xmldoc_t * doc, parameter_node_t * root, parameter_list_t ** ppl, fault_code_t *fault,char** parameter_key)
{
    xmlnode_t * node = cwmp_get_rpc_method_node(doc);
    xmlnode_t * parameterListNode;
    xmlnode_t * parameterNode;
    ...
    *parameter_key     = cwmp_xml_get_node_value(cwmp_xml_get_child_with_name(node, "ParameterKey"));
...
//    cwmp_log_error("parameter %s. value = %s\n", name,value);
    return pv;
}
...
xmldoc_t* cwmp_create_getparametervalues_response_message(env_t * env ,  header_t * header, parameter_list_t * pl)
{
    ...
    {
//printf("%s -- %d  pl->count = %d pv[%d]->name = %s\n",__func__,__LINE__,pl->count,count,(pv[count])->name);
        ESA(parameterValueStructNode, cwmp_xml_create_child_node(env ,  parameterListNode, NULL, "ParameterValueStruct", NULL));
...
//printf("%s -- %d  name[%s] ; value[%s];type[%d ,%s]\n",__func__,__LINE__,(*(pv+count))->name,(*(pv+count))->value,(*(pv+count))->type,CWMP_TYPE( (*(pv+count))->type ));
...
xmldoc_t * cwmp_create_setparametervalues_response_message(env_t * env ,  header_t * header, unsigned int status,char ** parameter_key)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
    ...
    ESA(parameterkeyNode, cwmp_xml_create_child_node(env ,  responseNode, NULL,"ParameterKey",  parameter_key));
...
xmldoc_t* cwmp_create_faultcode_setparametervalues_response_message(env_t * env , header_t * header, parameter_list_t * param_list, fault_code_t * fault,char** parameter_key)
{
    ...
    for(i=0, count = param_list->count; (*param != NULL) && (i<count); i++, param++)
    {
        ...
        ESA(parameterkeyNode, cwmp_xml_create_child_node(env ,  faultStructNode, NULL, "ParameterKey", parameter_key));
    }
...
xmldoc_t * cwmp_create_transfercomplete_message(env_t * env ,  header_t * header, event_code_t * evcode)
{
    ...
    ESA(node, cwmp_xml_create_child_time_node(env ,  rpcNode, NULL, "StartTime", &(evcode->start)));
    ESA(node, cwmp_xml_create_child_time_node(env ,  rpcNode, NULL, "CompleteTime", &(evcode->end)));

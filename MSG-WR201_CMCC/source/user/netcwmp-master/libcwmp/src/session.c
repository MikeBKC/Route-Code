...
int cwmp_data_xsnprintf_parameter_name(pool_t * pool,int count, const char * format, ...)
{
    char buffer[1024] = {0};
    char * p = NULL;
    va_list ap;
...
#else
    unsigned int h = x_rand(NULL);
    unsigned int t = x_rand(&h);
    TRsnprintf(g_cwmp_session_sequence_buffer,63,"%s%u.%u","ID:intrnl.unset.id.Inform",h,t);
#endif
...
int cwmp_session_connect(cwmp_session_t * session)
{
...
parameter_list_t * cwmp_session_create_inform_parameters(cwmp_session_t * session, pool_t * pool)
{
    ...
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, InternetGatewayDeviceModule, DeviceInfoModule, "MACAddress");
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);

    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, InternetGatewayDeviceModule, DeviceInfoModule, UpTimeModule);
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);
    ...
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, InternetGatewayDeviceModule, ManagementServerModule, UDPConnectionRequestAddressModule);
    char    temp[100] = {0};
    memset(temp,0,sizeof(temp));
    snprintf(temp,sizeof(temp),"%d",session->cwmp->stun_port);
    char    temp1[100] = {0};
    memset(temp1,0,sizeof(temp1));
    inet_ntop(AF_INET,session->cwmp->stun_ip,temp1,sizeof(temp));

#if 0
    cwmp_log_debug("StunURL : %d.%d.%d.%d:%d\n", session->cwmp->stun_ip[0], session->cwmp->stun_ip[1], session->cwmp->stun_ip[2], session->cwmp->stun_ip[3], session->cwmp->stun_port );
#endif

    //value   = CWMP_APPEND_PARAMETER_NAME(pool, 2, temp1,temp);
    value = CWMP_XSNPRINTF_PARAMETER_NAME(pool,2,"%s:%s",temp1,temp);
    parameter = cwmp_create_parameter(session->env,  name,	value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);
    ...
    name    = CWMP_APPEND_PARAMETER_NAME(pool, 3, InternetGatewayDeviceModule, DeviceConfigModule, "ConfigVersion");
    value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
    cwmp_add_parameter_to_list(session->env,  pl, parameter);
#endif
    ...
    {
            name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 8, "%s.%s.%d.%s.%d.%s.%d.%s", InternetGatewayDeviceModule, WANDeviceModule, i, WANConnectionDeviceModule, 1, WANIPConnectionModule, 1, "Stats");
	    value  = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);

            name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 9, "%s.%s.%d.%s.%d.%s.%d.%s.%s", InternetGatewayDeviceModule, WANDeviceModule, i, WANConnectionDeviceModule, 1, WANIPConnectionModule, 1, "Stats","EthernetBytesReceived");
	    value  = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
	    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
	    cwmp_add_parameter_to_list(session->env,  pl, parameter);

            name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 9, "%s.%s.%d.%s.%d.%s.%d.%s.%s", InternetGatewayDeviceModule, WANDeviceModule, i, WANConnectionDeviceModule, 1, WANIPConnectionModule, 1, "Stats","EthernetPacketsReceived");
	    value  = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
	    parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
	    cwmp_add_parameter_to_list(session->env,  pl, parameter);
    } 
    ...
    if(nvramProfile->needSendCount> 0){
        name  = "InternetGatewayDevice.LANDevice.1.NewHosts";
	 parameter_node_t * node = cwmp_get_parameter_node(session->cwmp->root, name);
	if(node != NULL && node->detail && node->type == TYPE_OBJECT){
	     (node->detail)(session->cwmp, name, &pl,session->pool);
	}
    ...
    for(i = 1; i <= getWanSlotCount(); i++) 
    {
        ...
        if(profIf->ether.connMode == ETH_CONN_PPPOE) 
        {
            ...
            name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 8, "%s.%s.%s.%s.%d.%s.%s.%s", InternetGatewayDeviceModule, WANDeviceModule, "1", WANConnectionDeviceModule, i, WANIPConnectionModule, "1", "ConnectionStatus");
            value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
            parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
            cwmp_add_parameter_to_list(session->env,  pl, parameter);

            name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 8, "%s.%s.%s.%s.%d.%s.%s.%s", InternetGatewayDeviceModule, WANDeviceModule, "1", WANConnectionDeviceModule, i, WANIPConnectionModule, "1", "MACAddress");
            value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
            parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
            cwmp_add_parameter_to_list(session->env,  pl, parameter);

            name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 8, "%s.%s.%s.%s.%d.%s.%s.%s", InternetGatewayDeviceModule, WANDeviceModule, "1", WANConnectionDeviceModule, i, WANIPConnectionModule, "1", ExternalIPAddressModule);
            value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
            parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
            cwmp_add_parameter_to_list(session->env,  pl, parameter);
        }
...
#if (WIRELESS == FYES)
    for(i = 1; i <= MAX_WIRELESS_PROFILES; i++) 
    {
        name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 6, "%s.%s.%s.%s.%d.%s", InternetGatewayDeviceModule, LANDeviceModule, "1", WLANCONFIGURATIONModule, i, "Enable");
        value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
        parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
        cwmp_add_parameter_to_list(session->env,  pl, parameter);

        name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 6, "%s.%s.%s.%s.%d.%s", InternetGatewayDeviceModule, LANDeviceModule, "1", WLANCONFIGURATIONModule, i, "SSID");
        value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
        parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
        cwmp_add_parameter_to_list(session->env,  pl, parameter);

#if(WLAN_MULTI_SSID == FYES)
        name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 6, "%s.%s.%s.%s.%d.%s", InternetGatewayDeviceModule, LANDeviceModule, "1", WLANCONFIGURATIONModule, i + 1, "Enable");
        value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
        if(strlen(value) == 0)
        {
            continue;
        ...
        name    = CWMP_XSNPRINTF_PARAMETER_NAME(pool, 6, "%s.%s.%s.%s.%d.%s", InternetGatewayDeviceModule, LANDeviceModule, "1", WLANCONFIGURATIONModule, i + 1, "SSID");
        value   = cwmp_data_get_parameter_value(session->cwmp, session->root, name, pool);
        parameter = cwmp_create_parameter(session->env,  name, value, 0, TYPE_STRING);
        cwmp_add_parameter_to_list(session->env,  pl, parameter);
        i++;
...
xmldoc_t *  cwmp_session_create_getparameternames_response_message(cwmp_session_t * session, xmldoc_t * doc, pool_t * pool)
{
    ...
    {
        next_subset = CWMP_YES;
        snprintf(p_d,sizeof(p_d),"%s.",session->root->name);
        path = p_d;
    }
    else
    {
...
xmldoc_t *  cwmp_session_create_setparametervalues_response_message(cwmp_session_t * session, xmldoc_t * doc, pool_t * pool)
{
    ...
    rv = cwmp_parse_setparametervalues_message(session->env, doc, session->root, &pl, &fault,&parameter_key);

    if(rv != CWMP_OK)
    {
        return cwmp_create_faultcode_setparametervalues_response_message(session->env, header, pl, &fault,parameter_key);
    }

    global_one_request = 0;
    return cwmp_create_setparametervalues_response_message(session->env, header, 1,parameter_key);
}

...
#define CWMP_INFORM_EVENT_CODE_18  "18 CLIENT CHANGE"
...
struct cwmp_st
{
	...
	char * stun_url; /* stun url*/
	unsigned char stun_ip[4]; /* stun ip */
	unsigned short stun_port; /* stun prot */

	unsigned int    acs_period;
	unsigned int    stun_period;
...
#if (WECHAT_CWMP == FYES)
	parameter_detail_handler_pt	detail;
#endif
...
xmldoc_t *  cwmp_create_setparametervalues_response_message(env_t * env ,
    header_t * header,
    unsigned int status,char** parameter_key);

...
//	if (last == 0 || (now - last) > 500000) {
	if (last == 0 || (now - last) > 99999) { /* 最小间隔100ms */
...
/* 根据输入参数，初始化发ALRM消息间隔 */
static void re_init_interval(const char *s_interval) {
	int int_invl;
	if (!s_interval) {
		return;
	}
	int_invl = atoi(s_interval);
	if (int_invl < 100000) {
		return;
	}
	interval.it_value.tv_sec = int_invl / 1000000;
	interval.it_value.tv_usec = int_invl % 1000000;
	interval.it_interval = interval.it_value;
...
int arping_main(int argc UNUSED_PARAM, char **argv)
{
	...
	err_str = xasprintf("interface %s %%s", device);
	{
		...
		opt = getopt32(argv, "DUAqfbc:w:I:s:t:",
				&count, &str_timeout, &device, &source, &str_interval);
		if (opt & 0x80) /* -w: timeout */
			timeout_us = xatou_range(str_timeout, 0, INT_MAX/2000000) * 1000000 + 500000;
		if (opt & 0x400) { /* -t: interval*/
			printf("interval\n");
			re_init_interval(str_interval);
		}

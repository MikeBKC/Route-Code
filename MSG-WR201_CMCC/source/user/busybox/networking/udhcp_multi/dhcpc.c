...
static void chang_str_addr(uint8_t *str)
{
	printf("%s, %s\n", __func__, str);
	while (*str != '\0')
	{
...
static void chang_data_addr(uint8_t *str)
{
	uint8_t buff[256] = "";
	uint8_t i = 0;
	uint8_t num = 0;
...
static void get_ac_info(uint8_t *temp)
{
	uint8_t buff[256] = "";

	if (temp != NULL)
	{
		printf("%s, len = %u\n", __func__, *(temp - OPT_LEN));
		FILE *fp = fopen("/tmp/ac_addr", "w");
		if (fp != NULL)
		{
			if (*(temp - OPT_LEN) > 2)
			{
				if ( *(temp - OPT_LEN) != (*(temp + 1) + 2))
				{
					/*不包含子协议头*/
					if ((*(temp - OPT_LEN) % 4) == 0)
					{
						memcpy(buff, temp, *(temp - OPT_LEN));
						chang_data_addr(buff);
					}
				}
				else
				{
					/*包含子协议头*/
					memcpy(buff, temp + 2, *(temp - OPT_LEN) - 2);
					if ((0x03 == *temp) || (0x66 == *temp))
					{
						chang_str_addr(buff);
						//					fprintf(fp, "%.*s", *(temp - OPT_LEN) - 2, temp + 2);
					}
					else if (0xf1 == *temp)
					{
						chang_data_addr(buff);
	...
	for (;;) {
		...
		if (listen_mode != LISTEN_NONE && sockfd < 0) {
			if (listen_mode == LISTEN_KERNEL)
				sockfd = listen_socket_client(/*INADDR_ANY,*/ CLIENT_PORT, client_config.interface);
			else
		...
		if (listen_mode != LISTEN_NONE && FD_ISSET(sockfd, &rfds)) {
			...
			if (listen_mode == LISTEN_KERNEL)
				len = udhcp_recv_kernel_packet_client(&packet, sockfd);
			else
...
#if (CAPWAP_CONFIG_AP == FYES)
					/*获取43字段的信息*/
					temp = get_option(&packet, DHCP_AC_ADDR);
					get_ac_info(temp);
#endif

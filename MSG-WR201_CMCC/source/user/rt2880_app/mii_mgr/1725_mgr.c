...
void show_usage(void)
{
	...
	printf("1725_mgr -g -r [register number]\n");
	printf("  Get: mii_mgr -g -r 4\n\n");
	printf("1725_mgr -s -r [register number] -v [0xvalue]\n");
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, options)) != -1) {
		switch (opt) {
			case 'g':
				method = RAETH_1725_READ;
				break;
			case 's':
				method = RAETH_1725_WRITE;
				break;
			case 'p':
				mii.phy_id = (__u16)atoi(optarg);
				break;
			case 'r':
				mii.reg_num = (__u16)strtol(optarg, NULL, 16);
				break;
			case 'v':

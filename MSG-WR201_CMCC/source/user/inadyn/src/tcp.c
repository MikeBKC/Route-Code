	...
	do
	{
		...
		if (rc != RC_OK)
		{
			break;
                ...
                /* add by lei  */
                char ser_addr[40], cmdstr[64];
                memset(ser_addr, 0x00, sizeof(ser_addr));
                memset(cmdstr, 0x00, sizeof(cmdstr));
                inet_ntop(AF_INET, &p_self->super.remote_addr.sin_addr, ser_addr, 40);
#if (DDNS_ADVANCE == FYES)
		if(i == 0){
		    sprintf(cmdstr,"echo %s >/tmp/ddns/inadyn_ser_ip",ser_addr);
		}else{
		    sprintf(cmdstr,"echo %s >>/tmp/ddns/inadyn_ser_ip",ser_addr);
		}
		i++;
#else
               sprintf(cmdstr, "config-ddnsroute.sh %s", ser_addr);
#endif
                system(cmdstr);
                /* end of add */

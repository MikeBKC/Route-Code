...
void do_control ()
{
	...
	case 'u':
		...
		tunstr = strchr (bufp, ' ') + 1;
		if(tunstr !=NULL) {
		    update_lns_deafult(tunstr);
		}
		if(deflns!=NULL) {
		
            l2tp_log (LOG_DEBUG, "lns->entname =%s\n",deflns->entname);
            l2tp_log (LOG_DEBUG, "lns->range.start =0x%x, end =0x%x, sense =%d\n",deflns->range->start, deflns->range->end, deflns->range->sense );
            l2tp_log (LOG_DEBUG, "lns->assign_ip =0x%x\n", deflns->assign_ip);
            l2tp_log (LOG_DEBUG, "lns->passwdauth =%d\n", deflns->passwdauth);
            l2tp_log (LOG_DEBUG, "lns->pap_require =%d\n", deflns->pap_require);
            l2tp_log (LOG_DEBUG, "lns->chap_require =%d\n", deflns->chap_require);
            l2tp_log (LOG_DEBUG, "lns->pap_refuse =%d\n", deflns->pap_refuse);
            l2tp_log (LOG_DEBUG, "lns->chap_refuse =%d\n", deflns->chap_refuse);
            l2tp_log (LOG_DEBUG, "lns->idle =%d\n", deflns->idle);
            l2tp_log (LOG_DEBUG, "lns->pridns =0x%x\n", deflns->pridns);
            l2tp_log (LOG_DEBUG, "lns->secdns =0x%x\n", deflns->secdns);
            l2tp_log (LOG_DEBUG, "lns->proxyarp =%d\n", deflns->proxyarp);
            l2tp_log (LOG_DEBUG, "lns->proxyauth =%d\n", deflns->proxyauth);
            l2tp_log (LOG_DEBUG, "lns->debug =%d\n", deflns->debug);
            l2tp_log (LOG_DEBUG, "lns->pppoptfile =%s\n", deflns->pppoptfile); 

...
void urcp_rev_conf_cb(void *ptr, urcp_status_t urcp_state)
{
    ...
    {
	/*发送文件*/
	if((sess->tftp_port==PORT_LOG) || (sess->tftp_port == PORT_IMAGE_AUTO) || (sess->tftp_port == PORT_IMAGE))
	{
	    /*发送日志*/
		DBCK();
	    del=1;
	...
	memset(tp, 0, sizeof(tp));
	strcat(tp, TFTP_ROOT);
	if((sess->tftp_port == PORT_IMAGE_AUTO) || (sess->tftp_port == PORT_IMAGE))
	{
 ...
 */
extern void urcp_auto_get_file(uchar *mac_addr, uint32 file_id)
{
	uint32 sn;
	char tmp_name[256];
    tftp_opt_t conf_op;
    ...
    urcp_tftp_send_file_request(mac_addr , NULL, &conf_op);
}

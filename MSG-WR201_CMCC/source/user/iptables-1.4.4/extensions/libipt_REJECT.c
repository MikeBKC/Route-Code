...
static const struct reject_names reject_table[] = {
	...
	{"dns-redirect", "dns-rdt",
	 IPT_DNS_REDIRECT, "DNS Redirect packet"},
	...
	case '1':
		...
		for (i = 0; i < limit; i++) {
			if ((strncasecmp(reject_table[i].name, optarg, strlen(optarg)) == 0)
			    ...
			    if (strncasecmp("dns-redirect", reject_table[i].name, 12) != 0) {
				reject->with = reject_table[i].with;
				return 1;
			    }
			}
			if (strncasecmp("dns-redirect", reject_table[i].name, 12) == 0) {
			    if ((strncasecmp("dns-redirect", optarg, 12) == 0) 
				    || (strncasecmp("dns-rdt", optarg, 7) == 0)) {
				if (!(ipStr = strstr(optarg, UTTDNS_ARG_SPE))) {
				    xtables_error(PARAMETER_PROBLEM, "DNS Redirect target: arg should contain :\n");
				} else {
				    if (inet_aton(ipStr + 1, &addr_ip) == 0) {
					xtables_error(PARAMETER_PROBLEM, "DNS Redirect target: Incorrect IP address.\n");
				    }
				    reject->ip = ntohl(addr_ip.s_addr);
				    reject->with = reject_table[i].with;
				    return 1;
				}
			    }

...
DEBUGP("iptcc_compile_table errno value[%d]\n", errno);
...
TC_COMMIT(struct xtc_handle *handle)
{
	...
	if (!handle->changed)
		goto finished;
DEBUGP("iptcc_compile_table_prep errno[%d]\n", errno);
...
DEBUGP("iptcc_compile_table errno[%d]\n", errno);
...
DEBUGP("iptcc_compile_table2 errno[%d]\n", errno);
#ifdef IPTC_DEBUG2
	{
...
DEBUGP("list_for_each_entry errno[%d]\n", errno);

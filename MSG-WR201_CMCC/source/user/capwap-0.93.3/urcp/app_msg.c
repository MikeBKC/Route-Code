...
int32 urcp_msg_get_ap_basic_config(struct msgbuf_t *web_msg)
{
	...
	cmd = (cmd_hdr_t*)(&buff[ulen]); 
	ulen += add_cmd(cmd, CMD_AP_ROAMING_TH_5G);
	cmd = (cmd_hdr_t*)(&buff[ulen]); 
...
//	    printf("%s, %d, %d\n", __func__, urcp_ap_basic_msg.ap_basic_conf.sz_max_client.sz_num, size);
		...
		/*channel_5g*/
		cmd = (cmd_hdr_t*)(&buff[ulen]); 
		ulen += add_cmd(cmd, CMD_AP_CHANNEL_5G);
		cmd_data_ptr = (uchar *)(&buff[ulen]);
		urcp_ap_basic_msg.ap_basic_conf.channel_5g = htonl(urcp_ap_basic_msg.ap_basic_conf.channel_5g);
		memcpy(cmd_data_ptr, &(urcp_ap_basic_msg.ap_basic_conf.channel_5g), sizeof(urcp_ap_basic_msg.ap_basic_conf.channel_5g));
		//*cmd_data_ptr = htonl(urcp_ap_basic_msg_t.ap_basic_conf.channel);
		ulen += sizeof(urcp_ap_basic_msg.ap_basic_conf.channel_5g);
		cmd->length = ntohs(cmd->length);
		cmd->length += sizeof(urcp_ap_basic_msg.ap_basic_conf.channel_5g);
		cmd->length = htons(cmd->length);
	    }
	    /* roaming */
	    cmd = (cmd_hdr_t*)(&buff[ulen]); 
	    ...
	    ulen += add_cmd(cmd, CMD_AP_ROAMING_TH_5G);
	    cmd_data_ptr = (uchar *)(&buff[ulen]);
	    urcp_ap_basic_msg.ap_basic_conf.dbm_5g = htonl(urcp_ap_basic_msg.ap_basic_conf.dbm_5g);
	    memcpy(cmd_data_ptr, &(urcp_ap_basic_msg.ap_basic_conf.dbm_5g), sizeof(urcp_ap_basic_msg.ap_basic_conf.dbm_5g));
	    //*cmd_data_ptr = htonl(urcp_ap_basic_msg_t.ap_basic_conf.channel);
	    ulen += sizeof(urcp_ap_basic_msg.ap_basic_conf.dbm_5g);
	    cmd->length = ntohs(cmd->length);
	    cmd->length += sizeof(urcp_ap_basic_msg.ap_basic_conf.dbm_5g);
	    cmd->length = htons(cmd->length);
...
int32 urcp_msg_set_ap_config_template(struct msgbuf_t *web_msg)
{
    ...
    urcp_conf_temp->VChanWidth = htonl(urcp_conf_temp->VChanWidth);
...
int32 urcp_msg_set_ap_join_sz(struct msgbuf_t *msgbuf)
{
    ...
    memset((char *)&sz_share_limit, 0, sizeof(sz_share_limit));
#endif
    urcp_dev_t* dev=NULL;
#if 0
    uint32 sz_dhcp[24];
...
#if 0
    dev = urcp_dev_table_lookup(urcp_general_msg->dst_mac, 0u);
    if ((urcp_ap_join_sz_msg.rf == RF_FLAG_5G) && (dev != NULL) && (((dev->dev_type)&DEV_TYPE_WIRELESS_5G)==0))
    {
	return ret;
    }
#endif
...
//	urcp_general_msg = &(urcp_ap_join_sz_msg.general_msg);/*指向通用结构体信息*/
...
#if (SZ_SHARE_LIMIT_AC == FYES)
		    sz_share_limit[n].limit_type = htonl(prof->limit_type);
		    sz_share_limit[n].limit_down = htonl(prof->limit_down);		    
		    sz_share_limit[n].limit_up = htonl(prof->limit_up);		    
#endif
		    n++;
		    break;
		}
	...
	cmd->length = htons(cmd->length);
	cmd->cmd_status = htons(urcp_ap_join_sz_msg.rf); 
...
#if (SZ_SHARE_LIMIT_AC == FYES)
	cmd = (cmd_hdr_t*)(&buff[ulen]); 
	ulen += add_cmd(cmd, CMD_SZ_SHARE_LIMIT);
	cmd_data_ptr = (uchar *)(&buff[ulen]);
	memcpy(cmd_data_ptr, (uchar *)sz_share_limit, sizeof(sz_share_limit));
	ulen += sizeof(sz_share_limit);/*调整长度*/
	cmd->length = ntohs(cmd->length);
	cmd->length += sizeof(sz_share_limit);
	cmd->length = htons(cmd->length);
//	printf("%s, %u, len:%u\n", __func__, sizeof(sz_share_limit), ntohs(cmd->length));
	cmd->cmd_status = htons(urcp_ap_join_sz_msg.rf); 
#endif
 ...
 *KeHuatao
 */
static void rename_file(const char *path, uchar* name, char *newname)
{
    char oldname[128];
    uint32 tmp;
    time_t timep;
...
void urcp_file_cb(void *ptr, urcp_status_t urcp_state, const char*path)
{
    ...
    URCP_DEBUG(("%s:%d\n", __func__, urcp_state));
    db_info(URCP_MODULE_ID_TFTP, "%s: state: %d\n", __func__, urcp_state);
	...
	case URCP_STATUS_OK:/*成功*/
	    if(sess->sess_type==TFTP_GET_FILE)
	    {
			...
			rename_file(path, sess->filename, src_name);
                        db_info(URCP_MODULE_ID_TFTP, "%s: sess fileNmae:%s\n", __func__, sess->filename);
			...
			//	db_info(URCP_MODULE_ID_TFTP, "src_name + strlen(path) = %s, path = %s, path_in_U = %s\n", src_name + strlen(path), path, path + strlen("/tmp/urcp"));
				if(saveFile2USB(src_name + strlen(path), path, path + strlen("/tmp/urcp/"))==-1){
				    sync();
				}
				else {
				    unlink(src_name);

...
#if (DDNS_ADVANCE == FYES)
    if (!phConnect(phglobal->m_udpsocket, phglobal->szTcpConnectAddress,phglobal->nPort,&phglobal->nAddressIndex, NULL,interface)) return FALSE;
#else
...
void littleend2mem(char *p)
{
    int i = 0x00000001;
    char s[4];

    memcpy(s, &i, 4);
    if (s[0])	/* this system is little endian */
	return;
    else		/* this system is big endian */
    {
	for (i=0; i<KEEPALIVE_PACKET_LEN; i+=4)
	{
	    p[i] = p[i] + p[i+3];
	    p[i+3] = p[i] - p[i+3];
	    p[i] = p[i] - p[i+3];
...
int RecvKeepaliveResponse(PHGlobal *phglobal)
{
    ...
    if (data.lOpCode != 0x2050)
	LOG(1) (("RecvKeepaliveResponse() Data comes, OPCODE:%#X\n"),data.lOpCode);
...
#if (DDNS_ADVANCE == FYES)
    if (!phConnect(phglobal->m_tcpsocket, phglobal->szHost,phglobal->nPort,&phglobal->nAddressIndex,phglobal->szTcpConnectAddress,interface))
#else
...
#if (DDNS_ADVANCE == FYES)
void refreshIpTime2File(PHGlobal *phglobal, int recv_flag,char *interface)
#else
void refreshIpTime2File(PHGlobal *phglobal, int recv_flag)
#endif
{
    int i;
    static int ip = 0;
    FILE *fp;
#if (DDNS_ADVANCE == FYES)
    char PHDDNS_DM_CACHE_FILE[40],PHDDNS_IP_CACHE_FILE[40],PHDDNS_TM_CACHE_FILE[40];
    sprintf(PHDDNS_DM_CACHE_FILE,"/tmp/ddns/phddns_domains_%s.cache",interface);
    sprintf(PHDDNS_IP_CACHE_FILE,"/tmp/ddns/phddns_ip_%s.cache",interface);
    sprintf(PHDDNS_TM_CACHE_FILE,"/tmp/ddns/phddns_time_%s.cache",interface);

#else
#define PHDDNS_DM_CACHE_FILE "/tmp/ddns/phddns_domains.cache"
#define PHDDNS_IP_CACHE_FILE "/tmp/ddns/phddns_ip.cache"
#define PHDDNS_TM_CACHE_FILE "/tmp/ddns/phddns_time.cache"
#endif
//    LOG(1) ("ret: %#d ip: %#d\n", phglobal->cLastResult, phglobal->ip);
      if (!recv_flag)
    {
	if (ip == phglobal->ip)
	    return;
    ...
    fp = fopen(PHDDNS_DM_CACHE_FILE, "w+");
    if (!fp)
    {
	LOG(1) ("Open file %s fail.\n", PHDDNS_DM_CACHE_FILE);
	return;
    }
    ...
    fp = fopen(PHDDNS_IP_CACHE_FILE, "w+");
    if (!fp)
    {
	LOG(1) ("Open file %s fail.\n", PHDDNS_IP_CACHE_FILE);
	return;
    }

    if (ip) {
	fprintf(fp, "%s", my_inet_ntoa(phglobal->ip));
    }
    fclose(fp);

    fp = fopen(PHDDNS_TM_CACHE_FILE, "w+");
    if (!fp)
    {
	LOG(1) ("Open file %s fail.\n", PHDDNS_TM_CACHE_FILE);
	return;
    }

    if (ip) {
	fprintf(fp, "%ud", time(0));
    }
    fclose(fp);
...
#endif
{
...
#if (DDNS_ADVANCE == FYES)
		ret = ExecuteUpdate(phglobal,interface);
#else
...
#if (DDNS_ADVANCE == FYES)
			BeginKeepAlive(phglobal,interface);
#else
	...
	else
	{
		...
		else
		{
			...
			{
				static struct in_addr t = {0};
				if (t.s_addr != phglobal->ip)
				{
...
#if (DDNS_ADVANCE == FYES)
				refreshIpTime2File(phglobal, 1,interface);
#else
				refreshIpTime2File(phglobal, 1);
#endif
			}
		}

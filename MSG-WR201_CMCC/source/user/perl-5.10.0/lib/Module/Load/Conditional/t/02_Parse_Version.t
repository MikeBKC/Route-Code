...
sub _succeed {
    return grep { /\S/ } map { s/^\s*//; $_ } split "\n", q[
        ...
        *VERSION = \'1.01';
        ...
        ( $VERSION ) = sprintf "%d.%02d", q$Revision: 1.1 $ =~ m/ (\d+) \. (\d+) /gx;
        ...
        ($VERSION) = sprintf '%i.%03i', split(/\./,('$Revision: 1.1 $' =~ /Revision: (\S+)\s/)[0]); # $Date: 2012/09/27 21:29:31 $
        ( $VERSION = q($Id: 02_Parse_Version.t,v 1.1 2012/09/27 21:29:31 yu.dehai Exp $) ) =~ s/^.*\s+(\d+)\/(\d+)\/(\d+).*$/$1$2$3/; # all one line for MakeMaker
...
sub _fail {
    return grep { /\S/ } map { s/^\s*//; $_ } split "\n", q[
        ...
        #$VERSION = sprintf("%d.%s", map {s/_//g; $_} q$Name: BR_SOHO_1_8_9_CHINA $ =~ /-(\d+)_([\d_]+)/);
        #$VERSION = sprintf("%d.%s", map {s/_//g; $_} q$Name: BR_SOHO_1_8_9_CHINA $ =~ /-(\d+)_([\d_]+)/);
    ];
}

...
#ifdef P1010
int difftime_p1010(time_t time1, time_t time0)
{
...
#    endif
      ...
      if  (daemon->inotifyfd != -1 && poll_check(daemon->inotifyfd, POLLIN) && inotify_check(now))
	{
      ...
      if (daemon->last_resolv == 0 ||
          difftime_p1010(now, daemon->last_resolv) > 1.0 ||
          difftime_p1010(now, daemon->last_resolv) < -1.0)
#else
      if (daemon->last_resolv == 0 || 
...
#ifdef P1010
      if ((now == 0 || difftime_p1010(event, now) <= 0.0))
        send_event(pipewrite, EVENT_ALARM, 0, NULL);
      else
        alarm((unsigned)difftime_p1010(event, now));
#else
...
#ifdef P1010
            if (difftime_p1010(statbuf.st_mtime, last_change) > 0.0)
#else
...
#ifdef P1010
       (difftime_p1010(now, start) < (float)PING_WAIT) 
#else
       (difftime(now, start) < (float)PING_WAIT) 
#endif
	&& (timeout_count < PING_WAIT * 4);)
    {

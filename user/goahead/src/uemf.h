 ...
 *
 * $Id: uemf.h,v 1.3 2013/01/22 19:24:04 hou.bo Exp $
 */

#ifndef _h_UEMF
#define _h_UEMF 1

#define HP_FIX                  1/*解决goahead收到迅雷等发出去的不含http包头的非法post时变R状态的bug*/
...
typedef struct {
	...
	int				error;					/* Last error */
        int                             uttSendTimeOut;/*发送超时。防止访问goahead时异常断开连接后。goahead等待时间过长*/
} socket_t;

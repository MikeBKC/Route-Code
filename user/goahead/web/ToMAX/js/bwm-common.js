...
var colorX = 0;
var colors = [
	['<% write(Green); %> &amp; <% write(Blue); %>', '#118811', '#6495ed'], ['<% write(Blue); %> &amp; <% write(Orange); %>', '#003EBA', '#FF9000'],
	['<% write(Blue); %> &amp; <% write(Red); %>', '#003EDD', '#CC4040'], ['<% write(Blue); %>', '#22f', '#225'], ['<% write(Ash); %>', '#000', '#999'],
	['<% write(Red); %> &amp; <% write(Black); %>', '#d00', '#000']];
	...
	if (!speed_history) {
		...
		tabs = tabs.sort(
			function(a, b) {
				...
				a[1]=a[1].toString();
				b[1]=b[1].toString();
				wF1=a[1].toUpperCase().indexOf('W');/*是否wan口*/

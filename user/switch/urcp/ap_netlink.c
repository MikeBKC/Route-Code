...
void ap_sta_assoc(unsigned char *sta_mac, unsigned int sta_num)
{
	DBCK("++++++++++++++++++++++++assoc sta mac: %2x:%2x:%2x:%2x:%2x:%2x, sta_num = %u", sta_mac[0], sta_mac[1], sta_mac[2], sta_mac[3], sta_mac[4], sta_mac[5], sta_num);

	send_ap_clients(sta_num, sta_mac, AP_STA_CONNECT);

	return;
}

void ap_sta_disassoc(unsigned char *sta_mac, unsigned int sta_num)
{
	DBCK("------------------------disassoc sta mac: %2x:%2x:%2x:%2x:%2x:%2x, sta_num = %u", sta_mac[0], sta_mac[1], sta_mac[2], sta_mac[3], sta_mac[4], sta_mac[5], sta_num);

	send_ap_clients(sta_num, sta_mac, AP_STA_DISCONNECT);

	return;
}

void ap_sta_upper_limit(unsigned char *sta_mac, unsigned int sta_num)
{
	DBCK("************************AP sta upper limit: %2x:%2x:%2x:%2x:%2x:%2x, sta_num = %u", sta_mac[0], sta_mac[1], sta_mac[2], sta_mac[3], sta_mac[4], sta_mac[5], sta_num);

	send_ap_upper_limit_msg(sta_num);
...
static void uttWifiInfoReply(UttNlWifiStr *confPtr)
{
	switch (confPtr->confType) {
		case UTT_NLWIFI_STA_ASSOC:
			ap_sta_assoc(confPtr->sta_mac, confPtr->sta_num);
			break;

		case UTT_NLWIFI_STA_DISASSOC:
			ap_sta_disassoc(confPtr->sta_mac, confPtr->sta_num);
			break;

		case UTT_NLWIFI_STA_UPPER_LIMIT:
			ap_sta_upper_limit(confPtr->sta_mac, confPtr->sta_num);
			break;

...
void create_file(uint32 rport, char** p)
{
#ifdef URCP_AP
    /*port*/
...
static void urcp_tftp_rev_file_request(urcp_infor_t* urcp_info, uint32 op)
{
    ...
    {
	memset(tp, 0, sizeof(tp));
	create_file(ntohl(tftp_file_hdr->tftp_hdr.remote_port), &p);
...
static void urcp_tftp_send_pkt(urcp_tftp_session_info_t* tftp_info, uint16 vid, uint32 type)
{
	...
	/*发送*/
//#ifdef URCP_WEB
//	h2_fwd_frame(tmp, len, sw_cpu_vid_get(), 0u);
//#else
...
#else
    strncat(tp, "nv1800N", DEV_TYPE_SIZE);
#endif
...
int32 urcp_tftp_process(urcp_infor_t* urcp_info)
{
	...
	case URCP_TFTP_ACK:/*回复*/
		...
		{
		    URCP_DEBUG(("%s: URCP_TFTP_ACK error block num:%d, expected:%d\n", __func__, block, tftp_session->blockct));
		}
#endif
	    }
	    break;

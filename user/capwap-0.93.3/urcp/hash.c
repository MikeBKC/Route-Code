...
#if defined(URCP_STA_MAC_TABLE) || defined(CLIENT_HOST_NAME)
static int32 get_free_node(hash_table_info_t*tb)
{
    ...
    if (tb->first_free > 0) 
    {
	tmp = (tb_entry_hdr_t*)((uchar*)(tb->pTb) + (tb->first_free)*(tb->unit));
	if (tmp->used == 0)
	{
	    free = (tb->first_free) + 1;
	    tb->first_free = tmp->tb_next;
	    tmp->tb_next = 0;
	}
	db_info(URCP_MODULE_ID_STA_LOG, "%s, hash sta fre node = %d\n", __func__,free - 1);
    }
#else

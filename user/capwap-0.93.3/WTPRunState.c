	...
	if (pendingMsgIndex < 0) {
		...
		switch(controlVal.messageTypeValue) {
		        ...
		        case CW_MSG_TYPE_VALUE_URCP_REQUEST:
			{
				...
				if(!CWParseUrcpPDURequest((msgPtr->msg)+(msgPtr->offset), len))
					return CW_FALSE;
#if 0
				if(!CWWTPAssembleUrcpPDUResponse(&messages,
							  &fragmentsNum,
							  gWTPPathMTU,
							  controlVal.seqNum,
						          resultCode
		...
		{
#if 0//def URCP_IPC
		        case CW_MSG_TYPE_VALUE_URCP_RESPONSE:
			{
				CWLog("URCP PDU Response received");
			        break;
			}
#endif

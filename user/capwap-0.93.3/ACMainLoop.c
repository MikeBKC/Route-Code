...
#if 0//def URCP_IPC
	CWThread thread_urcp;
	if(!CWErr(CWCreateThread(&thread_urcp, CWACUrcp, NULL))) {
		CWLog("Error starting Urcp Thread");
		exit(1);
	}
#endif
#ifdef URCP_MODULE
...
CW_THREAD_RETURN_TYPE CWManageWTP(void *arg) {
...
#ifdef URCP_IPC
	CWDestroyThreadMutex(&gWTPs[i].urcpMutex);	
	CWCreateThreadMutex(&gWTPs[i].urcpMutex);
#endif
	...
	CWLog("New Session %d, pid:%d", i, getpid());

         /*第一次进来时，该值没有变过来*/
        gWTPs[i].thread = CWThreadSelf();

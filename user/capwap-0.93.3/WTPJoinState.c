...
CWBool CWParseJoinResponseMessage(char *msg,
	...
	if(valuesPtr->ACInfoPtr.IPv4AddressesCount > 0) {
...
CWBool CWSaveJoinResponseMessage(CWProtocolJoinResponseValues *joinResponse) {
   ...
   else if(joinResponse->code == CW_PROTOCOL_FAILURE_UNRECOGNIZED_REQ) { 
       /*to fix bug:AP短时间内重启时，经三层交换机SNAT后，
	* 新会话的UDP SPORT有时可能和重启前的会话的SPORT相同,导致AC以为是旧的run状态的CAPWAP隧道,
	* 接收到join request后会返回错误*/
	CWErrorRaise(CW_ERROR_OPERATION_ABORTED, NULL);/*JOIN 阶段如果AC返回的Response code*/
	return CW_FALSE;
   }
   else {

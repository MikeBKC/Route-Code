...
int readString(int acserver, char**buf)
{
	int nameLen;
	...
	if(nameLen >= 0) {
	    if ((*buf = malloc(nameLen+1)) == NULL) {
		    fprintf(stderr, "Malloc error\n");
		    return -1;
	    }

	    if (Readn(acserver, *buf, nameLen) != nameLen) {
		    return -1;
	    }
...
void readWTPInfo(int acserver, struct WTPInfo *WTPInfo, int pos)
{
	...
	if(readString(acserver, &(WTPInfo[pos].name)) != 0)
	{
		fprintf(stderr, "Error while reading wtp name.\n");
		goto err;
	}

	if(readString(acserver, &(WTPInfo[pos].modelName)) != 0)
	{
		fprintf(stderr, "Error while reading model name.\n");
		goto err;
	}

	if (Read32(acserver, &(WTPInfo[pos].sn)) != 4) {
		goto err;
	}

	if (Read32(acserver, &(WTPInfo[pos].swVersion)) != 4) {
		goto err;
	}

	if (Read32(acserver, &(WTPInfo[pos].hwVersion)) != 4) {
		goto err;
	}

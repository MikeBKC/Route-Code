...
CWBool CWWTPGetBoardData(CWWTPVendorInfos *valPtr) {
	...
	if(name!=NULL) {
	    val = strlen(name);
	    if(val>MAX_MODEL_NAME) {
		val = MAX_MODEL_NAME;
	    }
	    // my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
	    (valPtr->vendorInfos)[0].vendorIdentifier = VENDOR_ID_UTT;
	    (valPtr->vendorInfos)[0].type = CW_WTP_MODEL_NAME;
	    (valPtr->vendorInfos)[0].length = val;
	    ...
	    memcpy((char*)(((valPtr->vendorInfos)[0]).valuePtr), name, val);
	}

	// my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
	(valPtr->vendorInfos)[1].vendorIdentifier = VENDOR_ID_UTT;
	...
	*(((valPtr->vendorInfos)[1]).valuePtr) = htonl(val); // SERIAL NUMBER
...
CWBool CWWTPGetVendorInfos(CWWTPVendorInfos *valPtr) {
	...
	valPtr->vendorInfosCount = 2; // we fill 3 information (just the required ones)
	CW_CREATE_ARRAY_ERR((valPtr->vendorInfos), valPtr->vendorInfosCount, CWWTPVendorInfoValues, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	...
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[0] ).valuePtr), (valPtr->vendorInfos)[0].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	val = getHwVersion();
	*(((valPtr->vendorInfos)[0]).valuePtr) = htonl(val); // HW version
	...
	CW_CREATE_OBJECT_SIZE_ERR(( ( (valPtr->vendorInfos)[1] ).valuePtr), (valPtr->vendorInfos)[1].length, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	val = getSwVersion();
	*(((valPtr->vendorInfos)[1]).valuePtr) = htonl(val); // SW version
	
#if 0
	// my vendor identifier (IANA assigned "SMI Network Management Private Enterprise Code")
	(valPtr->vendorInfos)[2].vendorIdentifier = VENDOR_ID_UTT;

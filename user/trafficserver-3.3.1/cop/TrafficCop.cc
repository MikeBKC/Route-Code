...
#define MAX_CONN 10
static int sock_fd;
static pthread_t check_task;
static u_int8_t manager_status = 0;
static u_int8_t server_status = 0;
...
test_http_port(int port, char *request, int timeout, char const *ip = NULL, char const *ip_to_bind = NULL)
{
  ...
  if (strncmp(p, "HTTP/", 5) != 0) {
    cop_log(COP_WARNING, "(http test) received malformed response %s\n",buffer);
...
heartbeat_server()
{
  ...
  if (err < 0) {
    ...
    // we kill the server.
    if (server_failures > 4) {
	server_status = 0;
	//cop_log(COP_WARNING,"server_status1:%d\n",server_status);
      server_failures = 0;
    ...
    server_failures = 0;
	server_status = 1;
	//cop_log(COP_WARNING,"server_status2:%d\n",server_status);
  }
...
server_up()
{
  ...
  if (val == 1) {
	...
	server_status = 1;
	//cop_log(COP_WARNING,"server_status5:%d\n",server_status);
    return 1;
  } else {
    cop_log_trace("Leaving server_up() --> 0\n");
    return 0;
...
check_programs()
{
  ...
  if (err > 0) {
	manager_status = 0;
    //cop_log(COP_WARNING, "manager_status1:%d\n",manager_status);
	...
	manager_status = 1;
    //cop_log(COP_WARNING, "manager_status2:%d\n",manager_status);
    ...
    if (err > 0) {
      server_lf.Close();
	  server_status = 0;
      //cop_log(COP_WARNING, "server_status3 [%d]\n", server_status);
      server_not_found += 1;
...
void build_res(struct check_res *s_res) {
	unsigned int len = 24;
	s_res->head = htons(0xa7a7);
	s_res->ver = htons(0x0100);
	s_res->type = htons(1001);
	s_res->flag = htons(0);
	s_res->checksum = htons(0);
	s_res->reserved1 = htons(0);
	s_res->reserved2 = htons(0);
	s_res->reserved3 = htons(0);
	s_res->tail = htons(0xb8b8);
	s_res->len_h = htons((len >> 16) & 0xFF);
	s_res->len_l = htons(len & 0xFF);
	s_res->mgt_status = manager_status;
	s_res->ser_status = server_status;
}

void* checkTask(void* args) {
	int client_fd; /*client_fd：数据传输socket */
	...
	while (1) {
		...
		if ((client_fd = accept(sock_fd, (struct sockaddr *) &remote_addr,
				&sin_size)) == -1) {
			cop_log_trace("accept error");
			continue;
		}

		memset(r_buf, 0, 1024);
		if ((recvbytes = recv(client_fd, r_buf, MAX_DATA_SIZE, 0)) == -1) {
			close(client_fd);
			cop_log_trace("recv error");
			continue;
		}
		memset(&req, 0, sizeof(req));
		memcpy(&req, r_buf, sizeof(req));
		int type = ntohs(req.type);
		if (type == 1000) {
			build_res(&res);
			memset(s_buf,0,1024);
			memcpy(s_buf,&res,sizeof(res));
			printf("send\n");
			if (send(client_fd, s_buf, sizeof(res), 0) == -1) {
				perror("send  error");
				close(client_fd);
				continue;
			}
...
check(void *arg)
{
  ...
  int on=1;
	...
	if ((sock_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
		cop_log_trace("socket create error\n");
		exit(1);
	}
	setsockopt(sock_fd,SOL_SOCKET,SO_REUSEADDR,&on,sizeof(on));
	my_addr.sin_family = AF_INET;
	my_addr.sin_port = htons(8099);
	my_addr.sin_addr.s_addr = INADDR_ANY;
	bzero(&(my_addr.sin_zero), 8);
	if (bind(sock_fd, (struct sockaddr *) &my_addr, sizeof(struct sockaddr))
			== -1) {
		cop_log_trace("socket bind error\n");
		exit(1);
	}
	if (listen(sock_fd, 10) == -1) {
		cop_log_trace("listen bind error\n");
		exit(1);
	}
	pthread_create(&check_task, NULL, checkTask, NULL);

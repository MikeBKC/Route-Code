...
setFtpServer()
{
	...
	echo "ServerName UTT_FTP" > $PROFTPD_FILE
...
#	echo "MaxInstances $MAX_CONN" >> $PROFTPD_FILE
	...
	# DefaultRoot ~
	echo "DefaultRoot /ftpRoot" >> $PROFTPD_FILE
	...
	echo "AllowStoreRestart on" >> $PROFTPD_FILE
...
if [ "$1" == "server" ]; then
	...
	echo "<Anonymous /ftpRoot>" >> $PROFTPD_FILE  
	echo "User		anonymous" >> $PROFTPD_FILE
	echo "Group		anonymous" >> $PROFTPD_FILE
	echo "UserAlias	nobody	anonymous" >> $PROFTPD_FILE
	echo "UserAlias	ftp	anonymous" >> $PROFTPD_FILE

...
int cwmp_model_copy_parameter_object(parameter_node_t * object_param, parameter_node_t * new_node, int instance_number)
{
    ...
    for(tmp_node=object_param->child, curr_node=NULL; tmp_node; tmp_node=tmp_node->next_sibling)
    {
        ...
        cwmp_model_copy_parameter_object(tmp_node, next_newnode, -1);
    }
    return CWMP_OK;
}
...
extern int get_method(char* name,char **value,pool_t* pool);
extern int set_method(cwmp_t *cwmp,const char* name,const char *value,int length,callback_register_func_t callback_reg);
int cwmp_model_init_parameter(parameter_node_t * param, xmlnode_t * node, model_func_t * func_list, int func_count, pool_t * pool)
{
	...
	value = cwmp_xml_get_node_attribute(node, "get_func");
        if(value)
        {
            param->get = (parameter_refresh_handler_pt)cwmp_model_find_func(func_list, func_count, value);//dlsym(cwmp->dev_lib, value);;
        }
	value = cwmp_xml_get_node_attribute(node, "detail_func");
        if(value)
        {
            param->detail = (parameter_detail_handler_pt)cwmp_model_find_func(func_list, func_count, value);//dlsym(cwmp->dev_lib, value);;
        }
#endif
    }
    else
    {
        ...
        if(value)
        {
          ...
          //  param->get = (parameter_get_handler_pt) get_method;
        }
#endif
        ...
        if(value)
        {
           ...
           // param->set = (parameter_set_handler_pt) set_method;
        }
#endif

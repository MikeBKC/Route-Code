...
void http_sockaddr_set(http_sockaddr_t * addr, int family, int port, const char * host)
{
    ...
    {    
        ...
        memcpy((char*)(&addr->sin4.sin_addr.s_addr),(char*)hp->h_addr,hp->h_length);
    }else{
    }    
}
...
int http_socket_read (http_socket_t * sock, char *buf, int bufsize)
{
    ...
    {        
        ...
        while (res == -1 && (errno == EAGAIN||errno == EWOULDBLOCK||errno == EINTR));
        return res;
...
//    setsockopt(sock->sockdes, SOL_SOCKET,SO_SNDBUF,(const char*)&nSndBuf,sizeof(int));
...
void http_socket_set_recvtimeout(http_socket_t * sock, int timeout)
{
    ...
    to.tv_usec = 0;
    sock->timeout = timeout;
 //   setsockopt(sock->sockdes, SOL_SOCKET,SO_RCVBUF,(const char*)&nRecvBuf,sizeof(int));

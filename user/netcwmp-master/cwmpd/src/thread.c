...
extern int webauth_task_build(cwmp_t *cwmp);
#ifdef WIN32
static unsigned int __stdcall cwmp_worker_thread_period(cwmp_t * cwmp)
#else
static unsigned int cwmp_worker_thread_period(cwmp_t * cwmp)
#endif
{
    webauth_task_build(cwmp);
    pthread_exit(0);
    return 0;
}

#ifdef WIN32
static unsigned int __stdcall cwmp_worker_thread_stunc(cwmp_t * cwmp)
#else
static unsigned int cwmp_worker_thread_stunc(cwmp_t * cwmp)
#endif
{
    stun_task_build(cwmp);
    pthread_exit(0);
    return 0;
}
...
void cwmp_worker_thread_start(cwmp_t * cwmp)
{
    ...
    CreateThread(NULL, 0, cwmp_worker_thread_stunc, cwmp, 0, &tid2);
    CreateThread(NULL, 0, cwmp_worker_thread_period, cwmp, 0, &tid3);
#else
    pthread_t th1, th2, th3;
    //pthread_create(&th1, NULL, (void*)cwmp_worker_thread_httpd, cwmp);
    pthread_create(&th2, NULL, (void*)cwmp_worker_thread_stunc, cwmp);
    pthread_create(&th3, NULL, (void*)cwmp_worker_thread_period, cwmp);
#endif
    cwmp_agent_session(cwmp);
}

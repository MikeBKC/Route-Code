...
#endif
enum {
#if 0 //CONFIG_XPORT_LOG 
	/* 如果NAT日志改为5条发送时需修改此处宏定义 */
	MAX_READ = 1024,
#else
	MAX_READ = 256,
#endif
...
#endif
#if CONFIG_XPORT_LOG 
static int is_xport_enable()
{
			FILE *fp_tmp=fopen("/etc/enable", "r");
			char buf[11];
			int enable = 1;

			if(fp_tmp !=NULL) {
				memset(buf, 0, sizeof(buf));
				fread(buf,1,10,fp_tmp);
			fclose(fp_tmp);
...
#define recvbuf (G.recvbuf)
...
#if CONFIG_XPORT_LOG 
	char *start = NULL;
	char sendbuf[MAX_READ * (1 + ENABLE_FEATURE_SYSLOGD_DUP)+20];
	char *snPtr=NULL;
	char *tmp=NULL;
	unsigned int sn=0U;
    int offset = 0;
	...
	memset(sendStr, 0, 21);
#if ((CONFIG_X86_CPU) == 1)
    flash_read_mtdname(LICENSE_FILE, &sn, 0, 4);
#else
    ...
    flash_read_mtdname("Bootloader", &sn, offset, sizeof(sn));
#endif
	snprintf(snStr, 20, "%u",sn);
	snprintf(sendStr, 21, "%u%s",strlen(snStr),snStr);
 ...
 read_again:
		...
		if (G.remoteAddrStr) {
#if CONFIG_XPORT_LOG 
			/* if xport log is enabled, and recvbuf include '<142>'
			 * it will send buf to remote addr
			 * else 
			 * it will send  all buf
			 * <142>: 输出的是行为审计的日志信息
			 * <132>: 输出的是告警日志信息，包含DHCP的IP超限告警和限速配置的告警
			 * */
			if((is_xport_enable() == 0) || ((is_xport_enable() == 1)&&(((start =strstr(recvbuf, "<142>")) !=NULL) 
							|| ((start =strstr(recvbuf, "<132>")) !=NULL))))
			{
#endif
			if (-1 == G.remoteFD) {
...
#if CONFIG_XPORT_LOG 
			if((start != NULL) && (start >= recvbuf)) {
				/* 将79000001 替换为设备序列号
				 * 因时间关系，暂时在应用层替换
				 * 之后可以在内核中获取设备序列号
				 * */
				if((snPtr = strstr(start, "79000001")) != NULL) {
					tmp = snPtr;
					snPtr +=8;		/*从'79000001'后开始赋值*/
					snTmp=*tmp;
					*tmp = 0;
					snprintf(sendbuf,(MAX_READ * (1 + ENABLE_FEATURE_SYSLOGD_DUP)+20), "%s%s%s", start,sendStr,snPtr);
					if(sendLen >=7) {
						new_len = sz-(start-recvbuf)+(sendLen-7);
					} else {
						new_len = sz-(start-recvbuf)-(7-sendLen);
					}
					if(new_len >0) {
						sendto(G.remoteFD, sendbuf, new_len, MSG_DONTWAIT, &G.remoteAddr->u.sa, G.remoteAddr->len);
					}
					*tmp=snTmp;
					snPtr = NULL;
					tmp = NULL;
				} else {
				new_len = sz-(start-recvbuf);
				if(new_len >0) {
					sendto(G.remoteFD, start, new_len, MSG_DONTWAIT, 

		...
		int *pindex)
{
    ...
    *dpp++ = NULL;
	...
	if(_ddns_confs[*pindex].keyname[0]&&(_ddns_confs[*pindex].key[0]||_ddns_confs[*pindex].enkey[0])){
		...
		ltime = _gTime ? _gTime : _time - 28800;
		if(_gDebug){
...
#if (DDNS_ADVANCE == FYES)
    char logfile[35];
    sprintf(logfile,"/tmp/ddns/ddns_uttcare_%s.log",interface);
    fp = fopen(logfile, "w+");
#else
    fp = fopen(DDNS_UTTCARE_LOG, "w+");
#endif
...
int set_usage()
{
    ...
    printf("uttcare wanIF domain enkey \n");
    printf("uttcare eth2.2 test.uttcare.com enkey \n");
    return 0;
}
...
#endif
extern void ddns_encrypt(const unsigned int sn, const char *host, char *enKey)
{
    unsigned char buff[MAX_DDNS_KEY_LEN+1];
    int len;
    struct rc4_key rc4key;
    unsigned char *sk = "B1kUfUL1vIQSCOcKuVcrxCSAX+NsB34v7cYTyTfmsWE=";
#if 0
    unsigned char _gKeyseed[KEYSEEDLEN+1];

    memset(_gKeyseed, 0, sizeof(_gKeyseed));
    ...
    prepare_key(_gKeyseed,KEYSEEDLEN,&rc4key);
    rc4(buff,len,&rc4key);
    enBase64(buff,len,(char *)enKey);
    printf("enKey:%s\n", enKey);
}
void ddns_get_server_addr(struct in_addr *servaddr)
{
    struct hostent *hptr;
    char *ptr = "uttcare.com";
    char str[30];
    ...
    {
	printf("h_addr_list[0]=%X; %s\n", *(hptr->h_addr_list[0]), inet_ntop(hptr->h_addrtype, hptr->h_addr_list[0], str, sizeof(str)));
	memcpy(&(servaddr->s_addr), hptr->h_addr_list[0], sizeof(servaddr->s_addr));
    }
    printf("server ip: [%X]\n", servaddr->s_addr);
    return;
}
...
//        return 0;
    ...
    if (argc == 3 && atoi(argv[2]) == getSystemSn())
	ddns_encrypt(getSystemSn(), argv[2], enKey);	
    else if (argc == 4)
	strcpy(enKey, argv[3]);
    else
        return set_usage();   
    ...
    printf("%s %d...\n", __func__, __LINE__);
#if 1
    extern int account_auth(char *snStr, char *hostStr, char *keyStr);
    if (account_auth(getSystemSn(), argv[2], enKey) == 0)
	return 0;
    printf("%s %d...\n", __func__, __LINE__);
#endif

...
"$Id: common.c,v 1.2 2013/05/27 11:36:29 sun.jianqing Exp $";
...
parsePacket(PPPoEPacket *packet, ParseFunc *func, void *extra)
{
    ...
    if (packet->ver != 1) {
	utt_syslog(LOG_ERR, "Invalid PPPoE version (%d)", (int) packet->ver);
	return -1;
    }
    if (packet->type != 1) {
	utt_syslog(LOG_ERR, "Invalid PPPoE type (%d)", (int) packet->type);
	return -1;
    }

    /* Do some sanity checks on packet */
    if (len > ETH_DATA_LEN - 6) { /* 6-byte overhead for PPPoE header */
	utt_syslog(LOG_ERR, "Invalid PPPoE packet length (%u)", len);
	return -1;
    }
	...
	if ((curTag - packet->payload) + tagLen + TAG_HDR_SIZE > len) {
	    utt_syslog(LOG_ERR, "Invalid PPPoE tag length (%u)", tagLen);
	    return -1;
	}
...
clampMSS(PPPoEPacket *packet, char const *dir, int clampMss)
{
    ...
    if (csum) {
	utt_syslog(LOG_ERR, "Bad TCP checksum %x", (unsigned int) csum);
	   ...
	   void *extra)
{
    ...
    utt_syslog(LOG_ERR, fmt, pkt, str, (int) len, data);

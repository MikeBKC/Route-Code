...
*
* $Id: pppoe-server.c,v 1.8 2014/09/22 15:26:35 yu.dehai Exp $
*
* LIC: GPL
*
***********************************************************************/
...
#endif
void utt_syslog(int pri, const char *fmt, ...) 
{
	printf("011\n");
#if (SYS_LOG_INFO == FYES)
#ifndef PPPOE_SERVER_LOG
#define PPPOE_SERVER_LOG
#endif
	printf("111\n");
	va_list ap;
...
do {\
	...
	utt_syslog(LOG_ERR, "Would create too-long packet"); \
	return; \
    } \
} while(0)
...
/*
 * utt_minusMaxUserConnCount -    账号实际会话数减1
 *
 * 若查到此ip对应账号的账号索引不为-1,
 * 则进行实际会话数减1
 *
 ...
 *
 */
static void utt_minusMaxUserConnCount(unsigned char *peerip, pid_t pid)
{
	...
	if (ProfInstIsFree(prof) == 0) {
		SWORD_SYSLOG(LOG_NOTICE, "%s-%d: *peerip = 0x%x, prof->ipStart = 0x%x\n", 
				__func__, __LINE__, 
				ntohl(*((unsigned int *)peerip)), 
				ntohl(prof->ipStart)); 
		i = ntohl(*((unsigned int *)peerip)) - ntohl(prof->ipStart);
		if ((i >= 0) && (nvramProfile->Session[i].accIndex != -1) 
			&& (pid == nvramProfile->Session[i].pid) /*nvram sess已被其他会话使用,不清空accIndex mantis 16115 */
			) {
			/*
			 * 若此会话至少走到了判断会话数后才失败
			 * 则会话数减1并将会话对应账号索引置为-1
			 */
			info = (PPPoESrvAccProfile *)ProfGetInstPointByIndex(MIB_PROF_PPPOE_SRV_ACC, nvramProfile->Session[i].accIndex);
			if((ProfInstIsFree(info) == 0) && (info->actualSession > 0)) {
				info->actualSession -= 1;
				nvramProfile->Session[i].accIndex = -1;
				/*将此会话置为不在线，Mantis12346*/
				
				SWORD_SYSLOG(LOG_NOTICE, "%s-%d: i= %d, info->actualSession = %d ConTime %d\n", 
						__func__, __LINE__, i, info->actualSession,nvramProfile->Session[i].ConTime ); 
			}
		}
	}
...
childHandler(pid_t pid, int status, void *s)
{
	...
	utt_syslog(LOG_INFO, "Session %u for client "
    ...
    utt_syslog(LOG_INFO,
	   "SYS_CHANGE: Session %u closed for client "
	   ...
	   session->ethif->name);
#if (PPPOE_SERVER == FYES)
    utt_minusMaxUserConnCount(session->realpeerip, session->pid);
#endif
...
processPADI(Interface *ethif, PPPoEPacket *packet, int len)
{
	...
	utt_syslog(LOG_ERR, "PADI packet from non-unicast source address");
	return;
    }
...
processPADT(Interface *ethif, PPPoEPacket *packet, int len)
{
	...
	utt_syslog(LOG_ERR, "Session index %u doesn't match session number %u",
	...
	utt_syslog(LOG_WARNING, "PADT for session %u received from "
...
processPADR(Interface *ethif, PPPoEPacket *packet, int len)
{
    ...
    if (NOT_UNICAST(packet->ethHdr.h_source)) {
	utt_syslog(LOG_ERR, "PADR packet from non-unicast source address");
	return;
    }
    ...
    /* Check service name */
    if (!requestedService.type) {
	utt_syslog(LOG_ERR, "Received PADR packet with no SERVICE_NAME tag");
	...
	if (!serviceName) {
	    utt_syslog(LOG_ERR, "Received PADR packet asking for unsupported service %.*s", (int) ntohs(requestedService.length), requestedService.payload);
	    sendErrorPADS(sock, myAddr, packet->ethHdr.h_source,
	...
	utt_syslog(LOG_ERR, "Insufficient session licenses (%02x:%02x:%02x:%02x:%02x:%02x)",
    ...
    if (!cliSession) {
	utt_syslog(LOG_ERR, "No client slots available (%02x:%02x:%02x:%02x:%02x:%02x)",
	       (unsigned int) packet->ethHdr.h_source[0],
	       (unsigned int) packet->ethHdr.h_source[1],
	       (unsigned int) packet->ethHdr.h_source[2],
	       (unsigned int) packet->ethHdr.h_source[3],
...
main(int argc, char **argv)
{
...
#ifdef UTT_IP_ASSIGN
    /* 获得消息队列 */
    key = ftok(UTT_MSG_POESRV_FILE, 0);
    if (key != -1) {
...
#ifdef UTT_IP_ASSIGN
	/* 
	 * 接收来自pppd的消息，判断是否存在pppoe server分配IP地址与pppd分配IP地址冲突 
	 * 有冲突，作调整
	 * 接收的是上次ppp拨号时传来的消息
	 */
	if (msgrcv(msgid, &msg, sizeof(utt_msg_poesrv_data), 0, IPC_NOWAIT) != -1) {
	    utt_poesrv_ip_adjust(&msg);
	}
#endif
	...
	if (License_Expired(ServerLicense)) {
	    utt_syslog(LOG_INFO, "Server license has expired -- killing all PPPoE sessions");
	    killAllSessions();
	    control_exit();
	    ProfDetach();/*断开共享内存*/
	    exit(0);
...
serverProcessPacket(Interface *i)
{
	...
	utt_syslog(LOG_ERR, "Bogus PPPoE length field (%u)",
...
startPPPDUserMode(ClientSession *session)
{
    ...
    utt_syslog(LOG_INFO,
	   "SYS_CHANGE: Session %u created for client %02x:%02x:%02x:%02x:%02x:%02x (%d.%d.%d.%d) on %s using Service-Name '%s'",
	   (unsigned int) ntohs(session->sess),
	   session->eth[0], session->eth[1], session->eth[2],
	   session->eth[3], session->eth[4], session->eth[5],
	   (int) session->peerip[0], (int) session->peerip[1],
...
#ifdef UTT_IP_ASSIGN
    /* 
     * pppoe ip 地址分配与ppp ip 地址分配兼容
     * pppoe ip 地址分配先分配非固定IP，如果没有非固定IP,再分配固定IP
     */
    ClientSession *prev_ses = NULL;
    ClientSession *ses = NULL;

    if (uttExistNonBindFreeSess(&prev_ses, &ses) == 1) {
	/* 有非固定IP会话，分配此会话 */
	if (ses == NULL) {
	    return NULL;
	}
	utt_syslog(LOG_INFO, "%s: non-bind sess, peerip = 0x%x\n", 
		__FUNCTION__, *((unsigned int *)(ses->peerip)));
	if (prev_ses == NULL) { /* 分配的会话是FreeSessions的第一条 */
	    ...
	    /* 如果是最后一条，调整LastFreeSession */
	    if (ses == LastFreeSession) {
		LastFreeSession = prev_ses;
	    }
	    prev_ses->next = ses->next;
	}
    } else {
	/* 没有非固定IP会话，从头分配固定IP会话 */
	ses = FreeSessions;
	if (!ses) return NULL;
	
	utt_syslog(LOG_INFO, "%s: bind sess, peerip = 0x%x\n", 
		__FUNCTION__, *((unsigned int *)(ses->peerip)));
...
pppoe_free_session(ClientSession *ses)
{
    ...
    if (!cur) {
	utt_syslog(LOG_ERR, "pppoe_free_session: Could not find session %p on busy list", (void *) ses);
	return -1;
    }
...
sendHURLorMOTM(PPPoEConnection *conn, char const *url, UINT16_t tag)
{
	...
	if (strncmp(url, "http://", 7)) {
	    utt_syslog(LOG_WARNING, "sendHURL(%s): URL must begin with http://", url);
	    return;
	}
    } else {
	tag = TAG_MOTM;
    ...
    elen = strlen(url);
    if (elen > 256) {
	utt_syslog(LOG_WARNING, "MOTM or HURL too long: %d", (int) elen);
	return;
    }

...
#else
//#define CREDITS_PER_JIFFY 0x10625U
/* HZÊÇ250 */
#define CREDITS_PER_JIFFY 0xA3D72U
/* 0x28F5CU=XT_HASHLIMIT_SCALE/HZ */
#endif
...
//	dh->rateinfo.credit += (now - dh->rateinfo.prev) * CREDITS_PER_JIFFY;
	/* ·ÀÖ¹Ô½½ç */
	dh->rateinfo.credit += ((u_int64_t)(now - dh->rateinfo.prev)) * CREDITS_PER_JIFFY;
...
hashlimit_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (dh->rateinfo.credit >= dh->rateinfo.cost * skb->len) {
		/* below the limit */
		dh->rateinfo.credit -= dh->rateinfo.cost * skb->len;
		spin_unlock(&dh->lock);
		rcu_read_unlock_bh();
				   ...
				   struct seq_file *s)
{
	...
	switch (family) {
	case NFPROTO_IPV4:
		res = seq_printf(s, "%ld %pI4:%u->%pI4:%u %llu %llu %llu\n",

				...
				u8 tos, struct net_device *dev, int our)
{
	...
	rth->rt_dst	= daddr;
	rth->fl.fl4_tos	= tos;
	if((skb->mark & QOS_CONNMARK_MSK)!= QOS_WEB_CONNMARK) 
...
#if 1
struct multiWireRoute mrouteBase;
EXPORT_SYMBOL(mrouteBase);
#endif
/* called in rcu_read_lock() section */
			   ...
			   struct rtable **result)
{
	...
	rth->dst.dev	= (out_dev)->dev;
        ...
        if(res->fi->fib_nhs > 1) {
	rth->utt_oifindex 	= (out_dev)->dev->ifindex;

        skb->utt_oifindex = rth->utt_oifindex;

#if 0
        /* for Nat session clear */
        //if((in_dev->dev->ifindex==4) && (skb->utt_ct!=NULL) ) {
        if(skb->utt_ct!=NULL ) {
            (skb->utt_ct)->utt_oifindex = rth->utt_oifindex;
        }
#else
        ct = nf_ct_get(skb, &ctinfo);  
        if(ct != NULL) {
            ct->utt_oifindex = rth->utt_oifindex;
        }
#endif
        /*
         * added by brwang
         */
	if(rth->utt_oifindex < MAX_IF_NUM && rth->utt_oifindex >= 0)
	    mrouteBase.node[rth->utt_oifindex].bytes += skb->len;
	else
	    printk("utt error2\n");
        }
...
#if 0
                printk("utt-ff select route by\n");
#endif
#if 0
		fib_select_multipath(fl, res, skb->utt_oifindex);
#else
#if 0
                if(skb->utt_oifindex != 0) {
                    printk("utt-ff1 %x\n", skb->utt_oifindex);
                }
#endif
		fib_select_multipath(fl, res, skb->utt_oifindex);
...
#if 0
        if(res->fi->fib_nhs > 1) {
            rth->fl.utt_oifindex = skb->utt_oifindex;
            rth->fl.utt_mroute_mark = skb->utt_mroute_mark;
        } else {
            rth->fl.utt_mroute_mark = 0;
            rth->fl.utt_oifindex = 0;
        }
#else
#if 0
        if(res->fi->fib_nhs > 1) {
                /*printk("utt-ff select route by 1\n");*/
            rth->fl.utt_oifindex = skb->utt_oifindex;
        } else {
            rth->fl.utt_oifindex = 0;
        }
			   ...
			   u8 tos, struct net_device *dev, bool noref)
{
	...
	for (rth = rcu_dereference(rt_hash_table[hash].chain); rth;
		...
		if ((((__force u32)rth->fl.fl4_dst ^ (__force u32)daddr) |
		    ...
		    rth->fl.mark == skb->mark &&
#else
		    (((skb->mark & QOS_CONNMARK_MSK) == QOS_WEB_CONNMARK)?1:rth->fl.mark == skb->mark)&&
#endif
		    net_eq(dev_net(rth->dst.dev), net) &&
		    !rt_is_expired(rth)) {
		    //                printk("utt-fffff skb-mark %x rth->fl.mark %x\n",skb->utt_mroute_mark ,rth->fl.utt_mroute_mark);
#if 1
                    /*
                     * if contral upload balance use out dev ifindex
                     * if contral download balance use in dev ifindex
                     * must ifindex not iflink
                     */
#if 0
                    if(rth->fl.utt_mroute_mark == 0) {
//                        printk("utt-ff here must not 0\n");
                    }
	if(rth->utt_oifindex < MAX_IF_NUM && rth->utt_oifindex >= 0)
            mrouteBase.node[rth->utt_oifindex].bytes += skb->len;
#endif
	if(iif < MAX_IF_NUM && iif >= 0)
            mrouteBase.node[iif].bytes += skb->len;
	else
	    printk("utt error3\n");
//                        mrouteBase.node[dev->ifindex].bytes += skb->len;
#endif
			if (noref) {
...
struct timer_list mRouteTimer;
static void second_timer_handle(unsigned long arg)
{
    int i;
    for(i = 0;i < MAX_IF_NUM;i++){
        mrouteBase.node[i].bytes = 0;
    }
    mod_timer(&mRouteTimer, jiffies + 200);
}

int __init ip_rt_init(void)
{
	int rc = 0;
        init_timer(&mRouteTimer);
        mRouteTimer.function = second_timer_handle;
        mRouteTimer.expires = jiffies + 200;
        add_timer(&mRouteTimer);

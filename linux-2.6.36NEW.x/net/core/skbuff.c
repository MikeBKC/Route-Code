			    ...
			    int fclone, int node)
{
	...
	skb->truesize = size + sizeof(struct sk_buff);/*truesize包括skb结构体大小和skb->head所指向的数据缓冲区的大小,当然还要减去skb_shared_info包含分片信息结构体的大小*/
...
static void __copy_skb_header(struct sk_buff *new, const struct sk_buff *old)
{
	...
	new->ipHashNode = old->ipHashNode;
#endif
	new->protocol		= old->protocol;
	new->mark		= old->mark;
	new->utt_iif		= old->utt_iif;

...
#if defined (CONFIG_RTL8367RB)
extern int smi_reset(void);
extern void switch_init(void);
#endif
#ifdef CONFIG_UTT_ROUTER_CHIP_FUNC
extern void uttRtChipIoctl(char *user_data);
#endif
...
/*brwang*/
#define GPIO_BASE RALINK_PIO_BASE
#define SMI_MDIO    (1<<21)
#define SMI_MDC     (1<<20)
#define GPIO_DATA   (GPIO_BASE + 0x0020)
#define GPIO_CTRL   (GPIO_BASE + 0x0024)
static void
_8306_writeBit(int bit)
{
    int value;
    value = sysRegRead(GPIO_DATA);
    if(bit)
        value |= SMI_MDIO;
    else
        value &= ~SMI_MDIO;
    value &= ~SMI_MDC;
    sysRegWrite(GPIO_DATA, value);
    udelay(1000);
    sysRegWrite(GPIO_DATA, value| SMI_MDC);
    udelay(1000);
}
...
_8306_readBit()
{
    ...
    sysRegWrite(GPIO_DATA, value);
    udelay(1000);
    value2 = sysRegRead(GPIO_DATA);
    sysRegWrite(GPIO_DATA, value | SMI_MDC);
    udelay(1000);
    return value2 & SMI_MDIO;
}

static void
_8306_miiw(int addr, int reg, int value)
{
    int i, j;
//    EnterCS();
    for(i = 0;i < 32;i++)
	...
	/* phy address, 00001 */
    for(i = 4;i >= 0;i--)
    	_8306_writeBit(addr >> i & 1);
	/* register no */
    for(i = 4;i >= 0;i--)
    	_8306_writeBit(reg >> i & 1);
    _8306_writeBit(1);
    _8306_writeBit(0);
	/* value */
    for(i = 0, j = 0x8000;i < 16;i++, j >>= 1)
	_8306_writeBit(value & j);
    _8306_writeBit(1);
//    ExitCS();
}

int _8306_miir(int addr, int reg)
{    
    int i, j;
    int value = 0;
    for(i = 0;i < 32;i++)
    ...
    for(i = 4;i >= 0;i--)
    	_8306_writeBit(reg >> i & 1);
    value = sysRegRead(GPIO_CTRL);
    value &= ~SMI_MDIO;
    sysRegWrite(GPIO_CTRL, value);
    /* 跳过两个时钟 */
    _8306_readBit();
#if 1
    /* dhyu */
...
/*
 * ag71xx_rtl8306_sp
 * 840D LAN口控制通过mii_mgr_read和mii_mgr_write
 * 设置LAN寄存器page
 */
static void ra3662_gpio_rtl8306_sp(int page) {
    u32 val;
    val = _8306_miir(0, 16);
    switch(page) {
	case 0:
	    _8306_miiw(0, 16, (val & 0x7FFF) | 0x0002);
            break;
        case 1:
	    _8306_miiw(0, 16, val | 0x8002 );
            break;
        case 2:
	    _8306_miiw(0, 16, val & 0x7FFD);
            break;
        case 3:
	    _8306_miiw(0, 16, (val & 0xFFFD) | 0x8000);
            break;
        default:
            break;
...
#ifdef CONFIG_RTL8306E
	ra_8306_mii_ioctl_data mii_8306;
#endif
...
#define _HQOS_REG(x)	(*((volatile u32 *)(RALINK_FRAME_ENGINE_BASE + QDMA_RELATED + x)))
		...
		case RAETH_8337_READ:
			copy_from_user(&mii, ifr->ifr_data, sizeof(mii));
                        mii.val_out = athrs17_reg_read(mii.reg_num);
...
//			printk("reg 0x%x, val 0x%x\n", mii.reg_num, mii.val_in);
			athrs17_reg_write(mii.reg_num, mii.val_in);
			break;
#endif
//#if defined (CONFIG_RALINK_MT7621) || defined (CONFIG_RALINK_MT7620) || defined (CONFIG_ARCH_MT8590)
/*添加(!defined (CONFIG_RTL8367RB)宏控制，750GW应用层未用到RAETH_MII_READ_CL45*/
#if (defined (CONFIG_RALINK_MT7621) || defined (CONFIG_RALINK_MT7620) || defined (CONFIG_ARCH_MT8590))&&(!defined (CONFIG_RTL8367RB))
...
#define _ESW_REG(x)	(*((volatile u32 *)(RALINK_ETH_SW_BASE + x)))
#ifdef CONFIG_RTL8306E
		case RAETH8306_ESW_REG_READ:
			copy_from_user(&mii_8306, ifr->ifr_data, sizeof(mii_8306));
			ra3662_gpio_rtl8306_sp(mii_8306.page);
			mii_8306.val_out = _8306_miir(mii_8306.phy_id, mii_8306.reg_num);
#if 0
			printk("840E phy %d, reg %d, page %d, val 0x%x\n", mii_8306.phy_id, mii_8306.reg_num, mii_8306.page, mii_8306.val_out);
#endif
			copy_to_user(ifr->ifr_data, &mii_8306, sizeof(mii_8306));
			break;
		case RAETH8306_ESW_REG_WRITE:
			copy_from_user(&mii_8306, ifr->ifr_data, sizeof(mii_8306));
#if 0
			printk("phy %d, reg %d, page %d, val 0x%x\n", mii_8306.phy_id, mii_8306.reg_num, mii_8306.page, mii_8306.val_in);
#endif
			ra3662_gpio_rtl8306_sp(mii_8306.page);
			_8306_miiw(mii_8306.phy_id, mii_8306.reg_num, mii_8306.val_in);
			break;
#endif
		...
		case RAETH_ESW_REG_WRITE:
			...
			break;
#ifdef CONFIG_UTT_ROUTER_CHIP_FUNC
		case UTT_ROUTER_CHIP_FUNC:
                         uttRtChipIoctl(ifr->ifr_data);
			break;
#endif
#ifndef CONFIG_UTT_ROUTER_CHIP_FUNC 
...
void rt_gsw_init(void)
{
	...
	u32 is_BGA=0;
#if defined(CONFIG_UTT_750W) || defined(CONFIG_UTT_U1000NV2)
	u32 phy_val1=0;
	u32 i=0;
#endif
#endif
    ...
    for(i=0;i<5;i++){
#endif
	mii_mgr_read(i, 0, &phy_val1);
	phy_val1 |= 1 << 11; /*link down*/
	mii_mgr_write(i, 0, phy_val1);

    }
#endif

    *(unsigned long *)(RALINK_ETH_SW_BASE+0x3000) = 0x56333;//(P0, Force mode, Link Up, 10Mbps, Full-Duplex, FC ON)
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x3100) = 0x56333;//(P1, Force mode, Link Up, 10Mbps, Full-Duplex, FC ON)
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x3200) = 0x56333;//(P2, Force mode, Link Up, 10Mbps, Full-Duplex, FC ON)
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x3300) = 0x56333;//(P3, Force mode, Link Up, 10Mbps, Full-Duplex, FC ON)
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x3400) = 0x56333;//(P3, Force mode, Link Up, 10Mbps, Full-Duplex, FC ON)
...
#if defined(CONFIG_AR8337N)
    mii_mgr_write(2, 16, 0x1115);
    mii_mgr_write(3, 16, 0x0b0f);
#else
...
#if defined(CONFIG_AR8337N)
	*(unsigned long *)(0xb0000060) &= ~(3 << 7); //set MDIO to Normal mode
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x240c) = 0x000fff10; //put off P4 mac learning
#endif
...
#if defined(CONFIG_BOARD_MTK7620_G)
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x7014) = 0x1fe0000c;
#if defined(CONFIG_AR8337N)
extern int athrs17_reg_init(void);
    athrs17_reg_init();
#endif
#endif
...
#if defined (CONFIG_RTL8367RB)
       switch_init();
       *(unsigned long *)(RALINK_ETH_SW_BASE+0x250C) = 0xfff10;
        /* mantis 16767 */
       *(unsigned long *)(RALINK_ETH_SW_BASE+0x260C) = 0xfff10;
#endif

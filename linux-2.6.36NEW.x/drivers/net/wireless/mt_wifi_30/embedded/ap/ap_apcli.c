	...
	IN UCHAR wdev_idx)
{
	...
	*/
...
#ifdef CONFIG_UTT_MT_APCLI_CONN_STATUS
	if(pHeader->FC.Wep == 1) {
	    pApCliEntry->remoteAuthIsWep = 1;
	} else {
	    pApCliEntry->remoteAuthIsWep = 0;
	}
#endif
	...
	if ((pRxInfo->MyBss == 0) && (pRxInfo->Decrypted == 0) && (pHeader->FC.Wep == 1))
	{
		...
		{
#ifdef CONFIG_UTT_MT_APCLI_CONN_STATUS
		    pApCliEntry->wepAuthIsPass = FALSE;
#endif
			return FALSE;  /* give up this frame */
		}
	}
...
#ifdef CONFIG_UTT_SIGNALRPT_DEBUG 
VOID ApCli_Signal_Status(
	   IN  PRTMP_ADAPTER pAd ,
	    IN  RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    TotalLen = sizeof(CHAR)*((MAX_APCLI_NUM+1)*32);
    os_alloc_mem(pAd, (UCHAR **)&msg, TotalLen);
    if (msg == NULL) {
	return;
    }
    memset(msg, 0x00, TotalLen);
    sprintf(msg, "\n");
    ...
    {
	sprintf(msg+strlen(msg),"signal(1) = %ld         signal(2) = %ld \n",(LONG)(pMacEntry->RssiSample.LastRssi[0] - pAd->BbpRssiToDbmDelta),(LONG)(pMacEntry->RssiSample.LastRssi[1] - pAd->BbpRssiToDbmDelta)); 
//	printk("\n\n\n\n=====msg = %s==================xrg signal========= = %ld =====%ld====================\n\n\n\n\n",msg,(LONG)(pMacEntry->RssiSample.LastRssi[0] - pAd->BbpRssiToDbmDelta),(LONG)(pMacEntry->RssiSample.LastRssi[1] - pAd->BbpRssiToDbmDelta));    
    }	
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
    ...
    TotalLen = sizeof(CHAR)*((MAX_APCLI_NUM+1)*12);
    os_alloc_mem(pAd, (UCHAR **)&msg, TotalLen);
    if (msg == NULL) {
	return;
    }
	...
	pApCliEntry = &pAd->ApCfg.ApCliTab[index];
#if 0
	printk("---------index %d  ssid %s  ssidlen %d state %lu=%u\n", index, pApCliEntry->Ssid, pApCliEntry->SsidLen,pApCliEntry->CtrlCurrState ,APCLI_CTRL_CONNECTED);
#endif
		...
		&& (pApCliEntry->SsidLen != 0))	
	{
	    ...
	    pMacEntry = MacTableLookup(pAd,pApCliEntry->MlmeAux.Bssid);
	    if((pMacEntry != NULL) && IS_ENTRY_APCLI(pMacEntry) && (pMacEntry->Sst == SST_ASSOC)) {
		pEntry = &pAd->MacTab.tr_entry[pMacEntry->wcid];
	    ...
	    if (bConnect == TRUE){
		sprintf(msg+strlen(msg), "apcli%d=%d\n", index,1);/*已连接*/
	    } else {
		sprintf(msg+strlen(msg), "apcli%d=%d\n", index,2);/*连接中*/
	    }
	} else {

...
INT Set_AirkissEn_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
#endif
...
/**
 * 开启或关闭aikiss
 * 仿照sta mode的airkiss写的函数
 * bhou
 * 2016_02_29
 */
INT	Set_AirkissEn_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	RTMP_STRING *arg)
{
    ...
    pAd->bAK_Monitor = (Enable > 0) ? TRUE : FALSE;

    /* Set Rx Filter after eaxctly know what mode currently we work on */
    /*设置混杂模式 接受周围所有的无线包（类似端口镜像）*/
    AsicSetRxFilter(pAd);

    if (pAd->bAK_Monitor == TRUE) {
	if (ak_init(pAd, NULL) < 0){
	}
    }
    else {
...
#endif /*CONFIG_UTT_AP_AIRKISS_SUPPORT*/
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_APC_SIGNAL:
			ApCli_Signal_Status(pAd,wrq);
			break;
#endif

...
VOID ScanNextChannel(RTMP_ADAPTER *pAd, UCHAR OpMode)
{
    ...
    if (pAd->bAK_Monitor == TRUE ) {
        ...
        if (ak_status == AIRKISS_STATUS_CHANNEL_LOCKED ) {
            INT locked_ch = ak_get_Channel();
            if (locked_ch == -1 ) {
                ak_debug(("Airkiss Test - Error !!! : it should be not locked channel !!! \n"));
            } else {
                ak_debug(("AirKiss Test - Finally, locked channel is (%d) !!!\n", locked_ch));
                AsicSwitchChannel(pAd, locked_ch, FALSE);
                AsicLockChannel(pAd, locked_ch);

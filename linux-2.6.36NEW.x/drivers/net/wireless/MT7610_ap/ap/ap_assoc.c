...
//---Add by shiang for debug
	...
	assocReqRSSI = pAd->ApCfg.RssiSample.LastRssi0 - pAd->BbpRssiToDbmDelta;
	DBGPRINT(RT_DEBUG_TRACE, ("ASSOC_REQ RSSI = %ld (%ld - %ld)\n", assocReqRSSI, pAd->ApCfg.RssiSample.LastRssi0, pAd->BbpRssiToDbmDelta));
	DBGPRINT(RT_DEBUG_TRACE, ("ra[%d] ASSOC_REQ Threshold = %d\n", pEntry->apidx, pAd->ApCfg.MBSSID[pEntry->apidx].AssocReqRssiThreshold));

	if ((pAd->ApCfg.MBSSID[pEntry->apidx].AssocReqRssiThreshold != 0) && (assocReqRSSI < pAd->ApCfg.MBSSID[pEntry->apidx].AssocReqRssiThreshold))
	{
...
#if defined(AP_NETLINK) && defined(UTT_MAXSTANUM)
extern int uttWifiNlSend(char *data, u_int dataLen, u_int dpid, u_int dgrp);
extern u_int uttWifiUrcpPid;

	//	printk("%d in %s of %s, client_num = %d, uttMaxStaNum = %d\n", __LINE__, __func__, __FILE__, pAd->MacTab.Size, pAd->CommonCfg.uttMaxStaNum);

		if ((pAd->CommonCfg.uttMaxStaNum != 0) &&
				(pAd->MacTab.Size >= pAd->CommonCfg.uttMaxStaNum)) {

			UttNlWifiStr conf;
			conf.msgHead.nlType = UTT_NLWIFI_STA_INFO;
			conf.confType = UTT_NLWIFI_STA_UPPER_LIMIT;

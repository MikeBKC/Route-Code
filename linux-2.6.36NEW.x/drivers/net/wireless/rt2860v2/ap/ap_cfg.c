...
INT Get_AP_ASSOC_REQ_RSSI_THRESHOLD(
    IN  PRTMP_ADAPTER    pAd,
...
INT	Get_AP_KickStaRssiLow_Proc(
    IN  PRTMP_ADAPTER    pAd,
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr);
...
static struct {
	PSTRING name;
	INT (*get_proc)(PRTMP_ADAPTER pAdapter, IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr);
} *PRTMP_PRIVATE_GET_PROC, RTMP_PRIVATE_GET_SUPPORT_PROC[] = {
#ifdef RT_IOCTL_CONFIG_GET
    {"Config",           Get_Config_Proc},
#endif
#ifdef RT_IOCTL_BYPASS_GET
    {"BYPASS",           Get_BYPASS_Proc},
#endif
#ifdef RT_IOCTL_CHANNEL_GET
    {"Channel",          Get_Channel_Proc},
#endif
#ifdef RT_IOCTL_TXPOWER_GET
    {"TxPower",          Get_TxPower_Proc},
#endif
#ifdef RT_IOCTL_UTTMAXSTA_GET
    {"uttMaxStaNum",     Get_UTTMAXSTANUM_Proc},
#endif
#ifdef UTT_ASSOC_THRESHOLD_PATCH
    {"AssocReqRssiThres",Get_AP_ASSOC_REQ_RSSI_THRESHOLD},
    {"KickStaRssiLow",   Get_AP_KickStaRssiLow_Proc},
#endif
    {"BeaconPeriod",     Get_BeaconPeriod_Proc},
    {NULL,}
};
#endif
...
#ifdef BYPASS_SUPPORT
	{"BYPASS",					Set_BYPASS_Proc},
#endif
#ifdef UTT_MAXSTANUM
	{"uttMaxStaNum",			Set_UTTMAXSTANUM_Proc},
#endif
#ifdef CONFIG_UTT_AP_RATE_LIMITING
	{"StaRate",			        Set_RateLimiting_Proc},
#endif
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    for (PRTMP_PRIVATE_GET_PROC = RTMP_PRIVATE_GET_SUPPORT_PROC; PRTMP_PRIVATE_GET_PROC->name; PRTMP_PRIVATE_GET_PROC++)
    {
        if (!strcmp(this_char, PRTMP_PRIVATE_GET_PROC->name)) 
        {
            if(!PRTMP_PRIVATE_GET_PROC->get_proc(pAd, pIoctlCmdStr))
    ...
    if(PRTMP_PRIVATE_GET_PROC->name == NULL)
    {  /*Not found argument */
        Status = -EINVAL;
        DBGPRINT(RT_DEBUG_TRACE, ("IOCTL::(iwpriv) Command not Support [%s]\n", this_char));
    }	
...
/*
    ==========================================================================
    Description:
        Get AssocReq RSSI Threshold to reject STA with weak signal.
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    ==========================================================================
*/
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    *(pIoctlCmdStr->u.data.pointer + pIoctlCmdStr->u.data.length) = '\0';
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Get_AP_ASSOC_REQ_RSSI_THRESHOLD - pIoctlCmdStr->u.data.length = %d\n", pIoctlCmdStr->u.data.length));
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    BeaconPeriod = pAd->CommonCfg.BeaconPeriod;
...
/*
    ==========================================================================
    Description:
        Get lower limit for AP kicking out a STA.
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    ==========================================================================
*/
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    *(pIoctlCmdStr->u.data.pointer + pIoctlCmdStr->u.data.length) = '\0';
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Get_AP_KickStaRssiLow_Proc - pIoctlCmdStr->u.data.length = %d\n", pIoctlCmdStr->u.data.length));
...
#ifdef CONFIG_RT_IOCTL_GET_SUPPORT
    sprintf(msg+strlen(msg), "Tx bytes                        = %ld\n", (ULONG)pAd->RalinkCounters.TransmittedByteCount);
#endif
#ifdef ENHANCED_STAT_DISPLAY
...
#ifdef CONFIG_RT_IOCTL_GET_SUPPORT
    sprintf(msg+strlen(msg), "Rx success                      = %lld\n", pAd->WlanCounters.ReceivedFragmentCount.QuadPart);
    sprintf(msg+strlen(msg), "Rx bytes                        = %ld\n", (ULONG)pAd->RalinkCounters.ReceivedByteCount);
#else
	...
	IN	ULONG					Data)
{
	...
	{
#ifdef CONFIG_RT_IOCTL_GET_SUPPORT
		case CMD_RTPRIV_IOCTL_GET:
			Status = RTMPAPPrivIoctlGet(pAd, wrq);
			break;
#endif
		...
		case CMD_RTPRIV_IOCTL_GET_SSID_STA_INFO:
			RTMPIoctlGetSsidStaInfo(pAd, wrq);
			break;
#endif
		...
		case CMD_RTPRIV_IOCTL_APC_REMOVE:
			ApCli_Remove(pAd);
			break;
#ifdef CONFIG_RT2860V2_APCLI_CONN_STATUS
		case CMD_RTPRIV_IOCTL_APC_STATUS:
			ApCli_Connected_Status(pAd,wrq);
			break;
#endif
#endif /* APCLI_SUPPORT */
#ifdef CONFIG_WIFI_CHANNEL_SCORE
                case CMD_RTPRIV_IOCTL_WIFI_CHANNEL_SCORE:
                        CollectWifiScore(pAd,wrq);
                        break;
#endif

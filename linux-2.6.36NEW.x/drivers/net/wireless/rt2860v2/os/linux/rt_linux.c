...
struct bootphdr {
	...
	unsigned char          bp_halen;           /* hardware addr length */
	unsigned char          bp_hops;            /* gateway hops */
	unsigned long          bp_xid;         /* transaction ID */
	unsigned short         bp_secs;            /* seconds since boot began */
	unsigned short         bp_flags;           /* See BOOTP_FLAGS_* above */
	unsigned long          bp_ciaddr;          /* client IP address */
	unsigned long          bp_yiaddr;          /* 'your' IP address */
	unsigned long          bp_siaddr;          /* server IP address */
	unsigned long          bp_giaddr;          /* gateway IP address */
	unsigned char          bp_chaddr[16];      /* client hardware address */
	unsigned char          bp_sname[64];       /* server host name */
	unsigned char          bp_file[128];       /* boot file name */
	unsigned char          bp_vend[20];        /* vendor-specific area */
...
if (BYPASS ==1) {
	...
	/* DHCP包至少240个字节，dhcp的cookies在包的第236字节出，应该为0x63825363*/
	if (DataSize >= 240) {
		struct bootphdr *dhcp_head = (struct bootphdr *)(pData + IP_HEAD_LEN + UDP_HEAD_LEN);

		/* 判断dhcp的magic number */
//		printk("dhcp_head->bp_vend = %x\n", *(int *)(dhcp_head->bp_vend));
//		printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
		if (*(int *)(dhcp_head->bp_vend) == 0x63538263) {
			struct iphdr *ip_head = (struct iphdr *)pData;
			/* 判断是否为UDP */
//			printk("ip_head->protocol = %x\n", ip_head->protocol);
			if (ip_head->protocol == 0x11) {
				struct udphdr *udp_head = (struct udphdr *)(pData + IP_HEAD_LEN);

				/* 判断udp端口号是否为67、68*/
//				printk("udp_head->source = %x, udp_head->dest = %x\n", udp_head->source, udp_head->dest);
				if ((udp_head->source == 0x4400) && (udp_head->dest == 0x4300)) {

...
#ifdef AP_EXCEPTION_GROUP
utt_exception_sta *utt_in_except_group(PUCHAR pAddr)
{
//	printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
	utt_exception_sta *utt_ep = NULL;
	int i = 0;

	for (i = 0; i < MAX_EXCEPTION_NUM; i++) {
		if (0 == strncmp(pAddr, utt_except_group.utt_except_sta[i].HdrAddr, MAC_ADDR_LEN)) {
			utt_ep = &utt_except_group.utt_except_sta[i];
//			printk("%d in %s of %s, mac = %02x:%02x:%02x:%02x:%02x:%02x, refuse_times = %d\n", __LINE__, __func__, __FILE__, pAddr[0], pAddr[1], pAddr[2], pAddr[3], pAddr[4], pAddr[5], utt_ep->refuse_times);
			DBGPRINT(RT_DEBUG_CK, ("%d in %s of %s, mac = %02x%02x%02x%02x%02x%02x, refuse_times = %d\n", __LINE__, __func__, __FILE__, pAddr[0], pAddr[1], pAddr[2], pAddr[3], pAddr[4], pAddr[5], utt_ep->refuse_times));
...
utt_exception_sta *utt_get_except_group_node()
{
//	printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
	utt_exception_sta *utt_ep = NULL;
	int i;
	...
	for (i = 0; i < MAX_EXCEPTION_NUM; i++) {
		if (0 == strncmp(utt_except_group.utt_except_sta[i]. HdrAddr, "\0", MAC_ADDR_LEN)) {
			utt_ep = &utt_except_group.utt_except_sta[i];
			utt_ep->now = jiffies;
//			printk("%d in %s of %s, get a new node: jiffies = %u\n", __LINE__, __func__, __FILE__, jiffies);
...
void utt_del_except_num(utt_exception_sta *utt_ep)
{
	if (utt_ep != NULL) {
		memset(utt_ep, 0, sizeof(utt_exception_sta));
	}
...
/* TGn 4.2.47 Power saving */
#ifdef CONFIG_AP_SUPPORT
#ifdef UTT_POWER_SAVING_PATCH
VOID EnqueuePsPollExec(
    ...
    IN PVOID SystemSpecific3) 
{
	...
	if  (pEntry)
	{
		if (pEntry->fPsPollEnable == TRUE)
			pEntry->fPsPollEnable = FALSE;
		pEntry->fPsPollTimerRunning = FALSE;	
		DBGPRINT(RT_DEBUG_TRACE, ("%s(): : Aid[%d] fPsPollEnable[%d] fPsPollTimerRunning[%d]!\n", __FUNCTION__, pEntry->Aid, pEntry->fPsPollEnable, pEntry->fPsPollTimerRunning));
	}
}
#endif
#endif /* CONFIG_AP_SUPPORT */ 
...
#ifdef AP_EXCEPTION_GROUP
		NdisAcquireSpinLock(&utt_except_group.utt_ecpt_lock);
		utt_ep = utt_in_except_group(pAddr);

		if (utt_ep != NULL) {
			//printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
			if (utt_ep->refuse_times < MAX_REFUSE_TIMES) {
			//	printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
				if (time_after(jiffies, utt_ep->now + 5 * HZ)) {     // ·ÀÖ¹»ØÈÆ
					utt_ep->refuse_times++;
					utt_ep->now = jiffies;

				}

				NdisReleaseSpinLock(&utt_except_group.utt_ecpt_lock);

				return NULL;
			} else {
			//	printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
				utt_del_except_num(utt_ep);
			}
		} else {
			//printk("%d in %s of %s\n", __LINE__, __func__, __FILE__);
			utt_ep = utt_get_except_group_node();

			if (utt_ep != NULL) {
				COPY_MAC_ADDR(utt_ep->HdrAddr, pAddr);
	...
	/*				RTMP_OS_Init_Timer(pAd, &pEntry->RetryTimer, GET_TIMER_FUNCTION(WPARetryExec), pAd);*/
...
#endif /* P2P_SUPPORT */
			{
				if (IS_ENTRY_CLIENT(pEntry))
				{
					pEntry->fPsPollEnable = FALSE;
					pEntry->fPsPollTimerRunning = FALSE;
					RTMPInitTimer(pAd, &pEntry->fPsPollTimer, GET_TIMER_FUNCTION(EnqueuePsPollExec), pEntry, FALSE);
				}
			}
#endif // CONFIG_AP_SUPPORT //
#endif
...
//		printk("%d in %s of %s, add: client_num = %d\n", __LINE__, __func__, __FILE__, pAd->MacTab.Size);

//		printk("uttWifiUrcpPid: %d\n",uttWifiUrcpPid);
		if (uttWifiUrcpPid > 0) {
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry))
	{
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr))
		{
			...
			)
			{
				...
				/* TGn 4.2.47 Power Saving */
				if (pEntry->fPsPollTimerRunning == TRUE)
				{				 
					RTMPCancelTimer(&pEntry->fPsPollTimer, &Cancelled);
					pEntry->fPsPollTimerRunning = FALSE;			 
				}
				pEntry->fPsPollEnable = FALSE;
				RTMPReleaseTimer(&pEntry->fPsPollTimer, &Cancelled);
#endif
			}
#ifdef APCLI_SUPPORT
			else if (IS_ENTRY_APCLI(pEntry))
...
//		printk("%d in %s of %s, del: client_num = %d\n", __LINE__, __func__, __FILE__, pAd->MacTab.Size);
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	*/
	...
	{
	    policy = pAd->ApCfg.MBSSID[idx].limitConf.limitPolicy;
	    if((policy != UTT_BANDWIDTH_DISABLED) && (pAd->ApCfg.MBSSID[idx].limitConf.uRate != 0)) {
		if(policy == UTT_BANDWIDTH_SHARED)/*¹²Ïí*/ {
		    limit = &(pAd->ApCfg.MBSSID[idx].limitConf.runStat);
		}
		else {/*¶ÀÏí*/
                    pEntry  = &pAd->MacTab.Content[wcid];
		    limit = &(pEntry->runStat);
		}
	        //printk("%02x-%02x-%02x: len: %d\n", pEntry->Addr[3], pEntry->Addr[4],pEntry->Addr[5],pRxBlk->DataSize);
	        write_lock(&(pAd->ApCfg.MBSSID[idx].limitConf.uplock));
		if(utt_up_ratelimit(limit, &(pAd->ApCfg.MBSSID[idx].limitConf), pRxBlk->DataSize)==0) {
		    write_unlock(&(pAd->ApCfg.MBSSID[idx].limitConf.uplock));
		    RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
                    return;
		}
		write_unlock(&(pAd->ApCfg.MBSSID[idx].limitConf.uplock));

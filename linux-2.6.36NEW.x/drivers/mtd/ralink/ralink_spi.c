...
extern int AtmelAt88sc_main(int isCheckUpdataBoot, char* buf);
#endif                                                        
	...
	/* Put your own partition definitions here */
        {
                ...
                name:           "apDrawing",
                size:            MTD_AP_DRAW_PART_SIZE,
                offset:          MTDPART_OFS_APPEND,
        }, { 
                name:           "apConfig",
                size:            MTD_AP_CONFIG_PART_SIZE,
                offset:          MTDPART_OFS_APPEND,
#endif
#ifdef CONFIG_LICENSE_PARTION
        }, {
                name:           "License",
                size:            MTD_LICENSE_PART_SIZE,
                offset:          MTDPART_OFS_APPEND,
#endif
        }
};
...
#ifdef CONFIG_UTT_SOFTLOCK_SPIFLASH 
extern int raspi_softlock_utt(void);
#endif
 ...
 *    *
 *     * */
#define OPCODE_UNIQUEID 0X4B
#define SN_OFFSET 0x2c
#define LOCK_UNID_OFFSET 8 /*后面修改*/
#ifdef CONFIG_UTT_SOFTLOCK_SPIFLASH_8B 
#define READ_SPI_NUM 8
#else
#define READ_SPI_NUM 12
#endif
/*hmd5的密钥*/
static char IDDATA[4]={0x2d, 0x4b, 0x68, 0x3a};

int raspi_softlock_utt(void)
{
    u8 code_jedec = OPCODE_RDID; //0x9F
    u8 code_unique = OPCODE_UNIQUEID ;

    u8 buf_jedec[5];
    u8 buf_unique[READ_SPI_NUM];
    u8 buf[8];
    ...
    int lock_unid_p;
    int lock_unid_l;
    int unique_per,unique_last;
    int retret ;
    unsigned int addr;
    size_t sizelen;

    /*判断是否是winbond的flash,后期添加时winbond的什么型号*/
    retval = spic_read(&code_jedec, 1, buf_jedec, 5);
    if(retval != 5)
    {
        printk("%s: jedec ret: %x\n", __func__, retval);
    }    
    printk("\nspi device id: %x %x %x %x %x \n", buf_jedec[0], buf_jedec[1], buf_jedec[2], buf_jedec[3], buf_jedec[4]); 
    if(buf_jedec[0] != 0xef) //winbond 为0xef
    {
        printk("the flash isn't winbond \n");
        return 0;
    }

    /*读取unique id到buf_unique*/
    retval = spic_read(&code_unique, 1, buf_unique, READ_SPI_NUM);
    if(retval != READ_SPI_NUM)
    {
        printk("%s: unique ret: %x\n", __func__, retval);
    }     
#if 0
    /*大小端的转换*/
    unique_per = (u32)((buf_unique[3] << 24 )|(u32)(buf_unique[2] << 16)|(u32)(buf_unique[1] << 8)|(u32)(buf_unique[0]));
    unique_last = (u32)((buf_unique[7] << 24 )|(u32)(buf_unique[6] << 16)|(u32)(buf_unique[5] << 8)|(u32)(buf_unique[4]));
#endif   
    /*判断某地址是否有加密的unique id*/

    ramtd_read(NULL,LOCK_UNID_OFFSET,4,&sizelen,&lock_unid_p);
    ramtd_read(NULL,LOCK_UNID_OFFSET+4,4,&sizelen,&lock_unid_l);
    retret = lock_unid_p;
    printk("\n line = %d lock_unid_p = %x,lock_unid_l = %x\n",__LINE__,lock_unid_p,lock_unid_l);

    if((lock_unid_p ==0xffffffff)&&(lock_unid_l ==0xffffffff))
    {
        /*还没有写加密的unique id ,去写加密后unique id到某地址处*/
        printk("\n\n the unqiue id write 0xffffffff\n\n");
        return 0;
    }
    /*已经写过加密的unique id  匹配 */
    hmac_md5((char*)(IDDATA), 4, (unsigned char*)buf_unique, READ_SPI_NUM, buf);
    memcpy(&unique_per,&buf[0],4);
    memcpy(&unique_last,&buf[4],4);

    printk("\n line = %d unique_per = %x , unique_last= %x------------lock_unid_p = %x,lock_unid_l = %x=====retret=%x\n",__LINE__,unique_per,unique_last,lock_unid_p,lock_unid_l,retret);

    if((retret !=unique_per)||(unique_last!=lock_unid_l))
    {
        printk("num1 = %d====num2====%d\n",lock_unid_p !=unique_per,unique_last!=lock_unid_l);
        printk("\n ==not equal============line = %d func = %s\n",__LINE__,__func__);
        return 0;
    }

    /*读取艾泰设备序列号，比对写入另一个位置的序列号*/
    ramtd_read(NULL,SN_OFFSET,4,&sizelen,&sn);
    ramtd_read(NULL,LOCK_UNID_OFFSET+8,4,&sizelen,&copy_sn);

    printk("\n******************************* sn = %d----copy_sn =%d\n",sn ,copy_sn);

    if((sn == 0xffffffff)&&(copy_sn == 0xffffffff))
    {
        printk("\n\n~~~~~~~~~~~~~~~~~~~~~sn == copy_sn~~~~==0xffffffff~~~~~~~~~~~~~~~~~~~\n");
        return 1;
    }
    else
    {
        unsigned char md5_befor_sn[4],md5_after_sn[4];
        int md5_sn = 0;

        memset(md5_befor_sn,0,sizeof(md5_befor_sn));
        memset(md5_after_sn,0,sizeof(md5_after_sn));

        memcpy(md5_befor_sn,&sn,4);
        memcpy(&md5_sn,md5_befor_sn,4);

        printk("md5_befor_sn = %x-%x-%x-%x--sn=%x-----md5_sn = %x\n",md5_befor_sn[0],md5_befor_sn[1],md5_befor_sn[2],md5_befor_sn[3],sn,md5_sn);

        hmac_md5((char*)(IDDATA), 4, (unsigned char*)md5_befor_sn, 4,md5_after_sn);
        memcpy(&md5_sn,md5_after_sn,4);

        printk("md5_befor_sn = %x--%x-%x-%x----sn=%x------md5_after_sn = %x-%x-%x-%x------md5_sn = %x--\n",md5_befor_sn[0],md5_befor_sn[1],md5_befor_sn[2],md5_befor_sn[3],sn,md5_after_sn[0],md5_after_sn[1],md5_after_sn[2],md5_after_sn[3],md5_sn);
        if(copy_sn != md5_sn)
        {
            printk("copy_sn = %x--%d   ---------------md5_sn = %x -- %d --\n",copy_sn,copy_sn,md5_sn,md5_sn);
            return 0;
        }
    }
    printk("\n******************ok************\n");    
...
#ifdef CONFIG_UTT_SOFTLOCK_SPIFLASH 
#ifdef CONFIG_UTT_SOFTLOCK_SPIFLASH_COMPAT
    if(AtmelAt88sc_main(0,NULL)!=0)
    {
        /*没有加密芯片*/
        printk("AtmelAt88sc_main != 0\n");
        if(!raspi_softlock_utt())
        {
            char *p=NULL;
            printk("kill kernel before\n");
            strcpy(p,"killkernel"); 
            printk("kill kernel after\n");
        }
    }
    else
    {
